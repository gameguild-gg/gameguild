...
* **BREAKING:** Renamed `GameGuild.Audit` module to `GameGuild.Compliance.Audit` to reflect its true purpose as a compliance and regulatory audit module
  - Updated namespace from `GameGuild.Audit` to `GameGuild.Compliance.Audit` across all source files
  - Module now properly categorized under `GameGuild.Compliance` alongside `GameGuild.Compliance.KYC` and `GameGuild.Compliance.FERPA`
  - Supports SOC2, ISO 27001, GDPR, and HIPAA compliance features including:
    - Compliance evidence packages for regulatory submissions
    - Tamper-evident audit logs with cryptographic hash chains (WORM storage)
    - Field-level data access auditing with PII masking
    - Scheduled audit exports to SFTP, S3, Azure Blob Storage
    - Digital signatures and chain-of-custody tracking
  - Updated all project references and solution file
  - **Migration:** Update any code referencing `GameGuild.Audit` to `GameGuild.Compliance.Audit`
