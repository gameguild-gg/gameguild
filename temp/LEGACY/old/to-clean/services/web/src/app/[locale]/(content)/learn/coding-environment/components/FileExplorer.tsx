    ...
    <div className="border-t border-gray-200 dark:border-gray-600 h-48">
      <div
        className={`flex items-center ${
          mode === 'light' ? 'bg-gray-100 border-gray-300' : mode === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-black border-gray-600'
        }`}
      >
        {showLeftArrow && (
          <button
            onClick={() => scroll('left')}
            className={`p-2 ${
              mode === 'light' ? 'text-gray-600 hover:bg-gray-200' : mode === 'dark' ? 'text-gray-400 hover:bg-gray-700' : 'text-yellow-300 hover:bg-gray-800'
            }`}
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
        )}
        <div ref={tabsRef} className="flex overflow-x-auto scrollbar-hide flex-1" onScroll={checkForArrows}>
        ...
        {showRightArrow && (
          <button
            ...
            className={`p-2 ${
              mode === 'light' ? 'text-gray-600 hover:bg-gray-200' : mode === 'dark' ? 'text-gray-200 hover:bg-gray-700' : 'text-yellow-300 hover:bg-gray-800'
            }`}
          >
