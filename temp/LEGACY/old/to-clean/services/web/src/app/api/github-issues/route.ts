    ...
    if (user && user !== 'all') {
      filteredIssues = filteredIssues.filter(
        (issue) =>
          ...
          (issue.requested_reviewers && issue.requested_reviewers.some((r) => r.login === user)),
      );
    }

    if (labels.length > 0) {
      filteredIssues = filteredIssues.filter((issue) => labels.every((label) => issue.labels && issue.labels.some((l) => l.name === label)));
    }
