...
import { PreviewCallout } from '@/components/content/editor/plugins/preview-components/preview-callout';
import { PreviewButton } from '@/components/content/editor/plugins/preview-components/preview-button';
    ...
    if (node.type === 'text') {
      ...
      if (node.format) {
        ...
        if (node.format & 64) {
          // Code
          textContent = (
            <code key={`code-${node.version || Math.random()}`} style={inlineStyles} className="bg-muted px-1 py-0.5 rounded text-sm">
              {textContent}
            </code>
          );
        }
