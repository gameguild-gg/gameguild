...
export default function Page(): React.JSX.Element {
  ...
  const runServerActionTest = async () => {
    ...
    try {
      ...
      const result = await TenantService.getUserTenants(session.api.accessToken);
  ...
  const testTokenRefresh = async () => {
    ...
    try {
      ...
      console.log('🧪 [TEST] Session details:', {
        ...
        hasAccessToken: !!session?.api.accessToken,
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <p>
                  ...
                  <Badge variant={session.api.accessToken ? 'default' : 'destructive'}>{session.api.accessToken ? 'Present' : 'Missing'}</Badge>
                </p>
                <p>
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <pre className="text-xs bg-muted p-2 rounded-md overflow-auto max-h-20">
                    {session.api.accessToken ? `${session.api.accessToken.substring(0, 50)}...` : 'No access token'}
                  </pre>
                </div>
                <div className="space-y-2">
                  <p className="text-sm">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm">
                  ...
                  Access: {session.api.accessToken?.length || 0} chars, Refresh: {(session as any).refreshToken?.length || 0} chars
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button onClick={runApiTest} disabled={loading || !session.api.accessToken} variant="outline">
          ...
          {/* Action Buttons */}
          <div className="flex gap-2">
            <Button onClick={() => (window.location.href = '/dashboard')} disabled={!session.api.accessToken}>
              Go to Dashboard
            </Button>
