...
const Hexagon: React.FC<HexagonProps> = ({ isActive, onClick }) => (
  <div
    className={`w-14 h-14 m-[1px] relative cursor-pointer transition-colors duration-300 ${isActive ? 'bg-green-800' : 'bg-lime-300'}`}
    onClick={onClick}
    style={{
...
const CatSilhouette: React.FC = () => (
  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-16 h-16 fill-current text-black">
...
export default function HexagonalGrid() {
  ...
  const handleHexagonClick = (row: number, col: number) => {
    const newActiveHexagons = activeHexagons.map((r, i) => r.map((h, j) => (i === row && j === col ? !h : h)));
        ...
        {activeHexagons.map((row, rowIndex) => (
          ...
          >
            {row.map((isActive, colIndex) => (
              <Hexagon key={`${rowIndex}-${colIndex}`} isActive={isActive} onClick={() => handleHexagonClick(rowIndex, colIndex)} />
            ))}
          </div>
        ))}
        <CatSilhouette />
