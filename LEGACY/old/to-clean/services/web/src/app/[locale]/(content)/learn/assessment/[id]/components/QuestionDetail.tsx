...
const QuestionDetail: React.FC<QuestionDetailProps> = ({ question, assessmentId, userId, role, courseId, moduleId, mode }) => {
    ...
    <>
      ...
      <div className="mb-4">
        ...
        <pre className="bg-gray-100 p-4 rounded-md overflow-x-auto">
          <code>{question.initialCode[question.codeName[0]?.id || '']}</code>
        </pre>
      </div>
      <div className="mb-4">
        <Link
        ...
        >
          <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Access Code Environment</button>
        </Link>
      </div>
    </>
  );
        ...
        <div className="mb-4">
          {question.answers.map((answer) => (
            <div key={answer.id} className="flex items-center mb-2">
              <Checkbox id={`answer-${answer.id}`} checked={selectedAnswers.includes(answer.id)} onCheckedChange={() => handleCheckboxChange(answer.id)} />
  ...
  return (
    <div
      className={`bg-white p-6 rounded-lg shadow-md ${
        mode === 'dark' ? 'bg-gray-800 text-gray-100' : mode === 'high-contrast' ? 'bg-black text-yellow-300' : ''
      }`}
    >
      {question.type === 'code'
        ? renderCodeQuestion(question as CodeQuestionv1_0_0)
        : question.type === 'multiple-choice'
          ? renderMultipleChoiceQuestion(question as MultipleChoiceQuestionv1_0_0)
          : question.type === 'essay'
