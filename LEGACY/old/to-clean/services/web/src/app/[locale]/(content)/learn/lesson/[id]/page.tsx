...
export default function LessonPage({ params }: { params: { id: string } }) {
  ...
  const [originalContent, setOriginalContent] = useState<{
    [key: number]: string | { content: string } | { title: string; content: string }[];
  }>({});
              ...
              {isEditing ? (
                <div>
                  ...
                  <div className="mt-2 space-x-2">
                    <Button onClick={() => handleSave(index, content, true)} variant="default">
                      Save
                    </Button>
                    <Button onClick={() => handleCancelEdit(index)} variant="outline">
                      Cancel
                    </Button>
                  </div>
                </div>
            ...
            <CardContent className="p-6">
              {isEditing ? (
                <div>
                  ...
                  />
                  <div className="mt-2 space-x-2">
                    <Button onClick={() => handleSave(index, contentItem, true)} variant="default">
                      Save
                    </Button>
                    <Button onClick={() => handleCancelEdit(index)} variant="outline">
                      Cancel
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-8">
          <Link href={`/learn/course/${courseId}?userId=${userId}&role=${role}`}>
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Return to Course</button>
          </Link>
        </div>
      </div>
    </div>
