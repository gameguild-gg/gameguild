...
import type { StudentSubmission, UserListQuestionv1_0_0 } from '@/lib/interface-base/user.list.question.v1.0.0';
...
const saveStateToLocalStorage = (state: any, courseId: string | null, moduleId: string | null, assessmentId: string | null) => {
  ...
  const updatedCodeFiles = state.codeFiles.filter((file) => state.globalCodeFiles.some((globalFile) => globalFile.id === file.id));
...
export default function CodeEditorPanel({
      ...
      if (assignment.type === 'code') {
        ...
        initialFiles = codeQuestion.codeName.map(({ id, name }) => {
          ...
          return {
            ...
            name,
            language,
            content: codeQuestion.rules.includes('addinput') ? insertInputs(content, codeQuestion.inputs[id] || [], language, 0) : content,
            history: { past: [], future: [] },
          };
        });
      }
    ...
    const handleInternalOutput = () => {
      if (currentFile.language === 'javascript') {
        ...
        const executeAndCapture = async () => {
          ...
          if (assignment.rules.includes('outputs: y') && !newAbortController.signal.aborted) {
            const results = compareOutputs([capturedOutput], assignment.outputs);
            setTestResults(results);
          }
        };
  ...
  const onRenameFile = (oldName: string, newName: string) => {
    const updatedCodeFiles = codeFiles.map(
      (file) => (file.name === oldName ? { ...file, name: newName, language: getLanguageFromExtension(newName) } : file), // Updatelanguage here
    );
    setCodeFiles(updatedCodeFiles);

    const updatedGlobalCodeFiles = globalCodeFiles.map(
      (file) => (file.name === oldName ? { ...file, name: newName, language: getLanguageFromExtension(newName) } : file), // Update language here
    );
    setGlobalCodeFiles(updatedGlobalCodeFiles);
    ...
    return (
      <div
        className={`h-full overflow-auto p-4 ${
          mode === 'light' ? 'bg-white text-gray-800' : mode === 'dark' ? 'bg-gray-800 text-gray-200' : 'bg-black text-white'
        }`}
      >
  ...
  const handleCloseTab = (index: number) => {
    ...
    const updatedGlobalCodeFiles = globalCodeFiles.map((file) => (file.id === closedFile.id ? { ...file, content: closedFile.content } : file));
    setGlobalCodeFiles(updatedGlobalCodeFiles);
  ...
  return (
    <div className={`h-screen ${mode === 'light' ? 'bg-gray-100 text-gray-800' : mode === 'dark' ? 'bg-gray-900 text-gray-200' : 'bg-black text-white'}`}>
      <PanelGroup direction="horizontal" onLayout={handlePanelLayout}>
        ...
        <PanelResizeHandle
          className={`transition-colors ${
            mode === 'light' ? 'bg-gray-300 hover:bg-gray-400' : mode === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-yellow-500 hover:bg-yellow-400'
            ...
            <Panel minSize={50}>
              <div className="flex flex-col h-full">
                ...
                <div className="flex-grow flex flex-col overflow-hidden">
                  <CodeTabs files={codeFiles} activeIndex={activeFileIndex} onTabChange={handleTabChange} mode={mode} onCloseTab={handleCloseTab} />
                    ...
                    {codeFiles.length > 0 ? (
                      ...
                      <div className="flex-grow overflow-auto flex items-center justify-center p-4">
                        ...
                        >
                          ...
                          <AlertDescription>No files are currently open. Select a file from the file explorer to start coding.</AlertDescription>
                        </Alert>
                      </div>
                    )}
                  </div>
      ...
      <SubmissionWarningDialog
        ...
        mode={mode}
      />
      <SettingsModal isOpen={isSettingsOpen} onClose={handleCloseSettings} onConfirm={handleConfirmSettings} mode={mode} initialSettings={settings} />
      <ConfirmSubmissionDialog isOpen={isConfirmDialogOpen} onClose={() => setIsConfirmDialogOpen(false)} onConfirm={handleConfirmSubmit} mode={mode} />
...
const ProblemsTab = ({ assignment, mode }: { assignment: CodeQuestionv1_0_0; mode: string }) => {
  ...
  const renderInputs = () => {
    if (Array.isArray(assignment.inputs)) {
      return (
        <ul className="list-disc list-inside">
          {assignment.inputs.map((input, index) => (
            <li key={index} className={mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}>
      ...
      return <p className={mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}>No inputs available</p>;
    }
  };

  return (
    <div>
      <h2 className={`text-xl font-bold mb-2 ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}`}>Problem Details</h2>
      {assignment.rules.includes('inputs: y') && (
        <div className="mb-4">
          <h3 className={`text-lg font-semibold ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}`}>Inputs:</h3>
          {renderInputs()}
        </div>
      )}
      {assignment.rules.includes('outputs: y') && !hideTestOutputs && (
        <div>
          <h3 className={`text-lg font-semibold ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}`}>
            Expected Outputs:
          </h3>
          <ul className="list-disc list-inside">
...
const OutputTab = ({ output, mode }: { output: string; mode: string }) => {
  ...
  return (
    <div>
      <h2 className={`text-xl font-bold mb-2 ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-white'}`}>Output</h2>
      {bitmapContent ? (
        <div className="mt-4">
        ...
        <pre
          className={`p-2 rounded ${mode === 'light' ? 'bg-gray-100 text-gray-800' : mode === 'dark' ? 'bg-gray-800 text-gray-200' : 'bg-black text-white'}`}
        >
