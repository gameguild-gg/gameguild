    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md w-96">
        ...
        <div className="mb-4">
          ...
          <input type="text" id="userId" value={userId} onChange={(e) => setUserId(e.target.value)} className="w-full px-3 py-2 border rounded" required />
        </div>
        ...
        <Link href={`/learn/coding-environment?id=0&type=sandbox&userId=${userId}`}>
          {' '}
          {/* Adicione o link para o sandbox */}
          <button className="w-full bg-gray-300 text-gray-700 py-2 rounded hover:bg-gray-400">Try Sandbox</button>
        </Link>
      </form>
    </div>
  );
