      ...
      <div className="max-w-5xl mx-auto">
              ...
              <div className="space-y-4">
                ...
                {module.idLessons.map((lessonId) => {
                  ...
                  return lesson ? (
                    <div key={lesson.id} className="border rounded-md">
                      <Link href={`/learn/lesson/${lesson.id}?userId=${userId}&role=${role}&courseId=${params.id}&moduleId=${module.id}`}>
                        {' '}
                        {/* Added userId and courseId */}
                ...
                {module.idAssessments.map((assessmentId) => {
                  ...
                  if (!displayDate && assessment.scoreTotal && assessment.scoreTotal.length > 0) {
                    ...
                    const futureScoreDates = assessment.scoreTotal.filter((score) => new Date(score.maxDate) > now).map((score) => new Date(score.maxDate));
                  ...
                  return (
                    <div key={assessment.id} className="border rounded-md flex">
                      ...
                      {displayDate && ( // Conditionally render the date if it exists
                        <div className={`ml-4 p-2 rounded-lg flex items-center ${dateBgColor} ${dateTextColor}`}>{displayDate.toLocaleDateString()}</div>
                      )}
                    </div>
                  );
                })}
        ...
        <div className="fixed bottom-4 left-4">
          <Link href={`/learn/courses?userId=${userId}`}>
            <button className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Return to Course Selection</button>
          </Link>
        </div>
      </div>
    </div>
