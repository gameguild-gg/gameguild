...
export default function TestResultsTab({ testResults, mode, hideTestOutputs }: TestResultsTabProps) {
  ...
  return (
    <div>
      <h2 className={`text-xl font-bold mb-4 ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-yellow-300'}`}>Test Results</h2>
      {testResults.length > 0 ? (
        <>
          {testResults.map((result, index) => (
            <div key={index} className={`mb-4 p-4 rounded ${getColorClass(result.passed)}`}>
              <div className="flex items-center mb-2">
                <span className={`font-bold mr-2 ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-yellow-300'}`}>
                  Test #{index + 1}:
                </span>
          ...
          <p className={`mt-4 font-bold ${mode === 'light' ? 'text-gray-800' : mode === 'dark' ? 'text-gray-200' : 'text-yellow-300'}`}>
