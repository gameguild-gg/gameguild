  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`${getDialogStyles()} p-6 rounded-lg`} aria-describedby="confirm-submission-description">
        <DialogHeader>
          <DialogTitle className={`text-xl font-bold ${mode === 'high-contrast' ? 'text-yellow-300' : ''}`}>Confirm Submission</DialogTitle>
          <DialogDescription
            id="confirm-submission-description"
            className={`${mode === 'light' ? 'text-gray-600' : mode === 'dark' ? 'text-gray-400' : 'text-yellow-200'}`}
          >
            Are you sure you want to submit your assignment?
          </DialogDescription>
        </DialogHeader>
        <DialogFooter className="mt-6 flex justify-end space-x-2">
          <Button variant="outline" onClick={onClose} className={getButtonStyles('secondary')}>
            No
          </Button>
          <Button onClick={onConfirm} className={getButtonStyles('primary')}>
            Yes
          </Button>
        </DialogFooter>
      </DialogContent>
