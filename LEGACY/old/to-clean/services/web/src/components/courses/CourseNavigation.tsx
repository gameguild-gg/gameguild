...
export function CourseNavigation({ course, isOpen, className = '', onNavigate }: CourseNavigationProps) {
    ...
    <nav className={`${className}`}>
      <Link
        ...
        className={`block text-blue-600 hover:underline font-semibold py-2 px-3 rounded-md ${
          pathname === `/course/${course.slug}` ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-200'
        }`}
      >
        Course Home
      </Link>
      {course.chapters.map((chapter) => (
        <div key={chapter.id} className="mt-4">
          <details className="group" open={chapter.lectures.some((lecture) => pathname === `/course/${course.slug}/${lecture.slug}`)}>
            <summary
              className={`font-semibold cursor-pointer list-none flex items-center justify-between p-3 rounded-md ${
                chapter.lectures.some((lecture) => pathname === `/course/${course.slug}/${lecture.slug}`) ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-200'
              }`}
            >
