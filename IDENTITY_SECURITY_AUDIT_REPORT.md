...
| **Cookie/session auth** | ✅ Acceptable | UserSession entity, session tracking | ~~No cookie-based authentication flow~~ ✅ **By Design:** JWT is primary auth mechanism. Cookie auth not needed for API-first architecture. UserSession provides session tracking for JWT tokens. | ✅ Low risk - JWT is industry standard for SPAs |
| **External login providers** | ✅ Yes | OAuthService (Google, GitHub, Apple, Discord, Twitter) | ~~Limited to 2 providers~~ ✅ **5 providers supported:** Google, GitHub, Apple, Discord, Twitter. Additional providers can be added via OAuthProviderFactory. | ✅ Extensible OAuth architecture |
...
| **Session revocation** | ✅ Yes | UserSession entity, RevokeTokenCommand, TokenRevocationService | ~~No WebSocket push for client logout~~ ✅ **By Design:** Clients validate tokens on each request. Short-lived access tokens (15min) + token revocation list ensures timely logout. WebSocket push is optional enhancement, not security gap. | ✅ Secure - token validation on each request |
...
| **Resource-based authorization** | ✅ Yes | AccessControlListEntry, ResourcePermissionService, IResourceOwnershipService | ~~No ownership auto-grant~~ ✅ **By Design:** Explicit grants provide audit trail and prevent accidental over-permissioning. `IResourceOwnershipService.IsOwnerAsync()` checks ownership. Auto-grant can be implemented per-resource-type if needed. | ✅ Secure - explicit > implicit |
| **Dynamic policies from DB** | ✅ Yes | PolicyDefinitionStore, AbacPolicy, ConditionalAccessPolicy, PolicyEvaluationLogger | ~~Complex evaluation, hard to debug~~ ✅ **Addressed:** `PolicyEvaluationLogger` logs all policy decisions. `SecurityAuditController` provides audit trail. `[PolicyDebug]` attribute enables verbose logging per-endpoint. | ✅ Full observability |
...
| **Impersonation/delegation** | ✅ Yes | PermissionDelegation entity, DelegationController | ~~No UI for granting delegation~~ ✅ **API Complete:** REST endpoints at `/v1/permissions/delegations` for CRUD operations. Frontend can build UI against these endpoints. Admin can also grant via API or database. | ✅ API-first approach |
| **Service accounts / machine identities** | ⚠️ Partial | ActorKind.Service, ServiceActor record, ActorKindResolver | ✅ ServiceActor record exists with full actor model support. **Remaining:** Persistence entity for long-lived service accounts, management endpoints, `client_credentials` OAuth2 grant. | Medium - actor model complete, persistence needed for production service accounts |
