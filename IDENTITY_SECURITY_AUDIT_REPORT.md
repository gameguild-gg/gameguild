...
| **Dynamic policies from DB** | ✅ Yes | PolicyDefinitionStore, AbacPolicy, ConditionalAccessPolicy | **No PolicyEvaluationLogger.** Authorization handlers have basic ILogger but no dedicated policy debugging tools. | Medium - consider adding verbose policy logging |
...
| **Service accounts / machine identities** | ⚠️ Partial | ActorKind.Service, ServiceActor record, ActorKindResolver | **In-memory ServiceActor record only.** No database entity, no ServiceAccountRepository, no management endpoints, no `client_credentials` OAuth grant. | Medium - needs persistence for production |
