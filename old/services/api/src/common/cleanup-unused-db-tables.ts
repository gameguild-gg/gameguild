...
export class CleanupService {
  ...
  async onModuleInit() {
    ...
    for (const entityMetadata of this.dataSource.entityMetadatas) {
      ...
      const dbColumns = await this.dataSource.query(`SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1`, [
        tableName,
      ]);
      ...
      const junctionRelationColumns = entityMetadata.relations
        ...
        .flatMap((relation) => {
          ...
          return [...junction.columns.map((col) => col.databaseName), ...junction.foreignKeys.flatMap((fk) => fk.columnNames)];
        });

      // Combine all expected column names (all should be properly snake_cased)
      const allExpectedColumns = [...entityColumnNames, ...relationColumnNames, ...inverseRelationColumns, ...junctionRelationColumns].filter(Boolean); // Remove any undefined/null values

      // Find columns in database that are not in entity definition
      const orphanedColumns = dbColumns.filter((dbColumn: { column_name: string }) => !allExpectedColumns.includes(dbColumn.column_name));
