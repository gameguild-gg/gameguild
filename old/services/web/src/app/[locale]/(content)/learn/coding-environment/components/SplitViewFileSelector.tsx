...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@/components/learn/ui/dialog';
...
export function SplitViewFileSelector({ isOpen, onClose, onSelectFile, files, mode }: SplitViewFileSelectorProps) {
  ...
  const getButtonStyles = (isActive: boolean) => {
    ...
    switch (mode) {
      case 'light':
        return isActive ? `${baseStyles} bg-blue-500 text-white hover:bg-blue-600` : `${baseStyles} bg-gray-200 text-gray-800 hover:bg-gray-300`;
      case 'dark':
        return isActive ? `${baseStyles} bg-blue-600 text-white hover:bg-blue-700` : `${baseStyles} bg-gray-700 text-gray-200 hover:bg-gray-600`;
      case 'high-contrast':
        return isActive
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`${getDialogStyles()} p-6 rounded-lg`}>
        <DialogHeader>
          <DialogTitle className={`text-xl font-bold ${mode === 'high-contrast' ? 'text-yellow-300' : ''}`}>Select File for Split View</DialogTitle>
          <DialogDescription className={`${mode === 'light' ? 'text-gray-600' : mode === 'dark' ? 'text-gray-400' : 'text-yellow-200'}`}>
