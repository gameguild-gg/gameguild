...
const FileExplorer = ({
  ...
  const handleToggleFolder = (folderPath: string) => {
    setExpandedFolders((prev) => (prev.includes(folderPath) ? prev.filter((path) => path !== folderPath) : [...prev, folderPath]));
  };

  const getFileStateClasses = (file: File, mode: string) => {
    const isOpen = codeFiles.some((f) => f.name.split('/').pop() === file.name.split('/').pop());
    const isModified = globalCodeFiles.some((f) => f.name.split('/').pop() === file.name.split('/').pop() && f.content !== '');
    ...
    return (
      <div key={path} className="relative">
        ...
        >
          ...
          {isFolder && <ChevronDown className={`ml-auto transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />}
        </div>
        {isFolder && isExpanded && (
  ...
  return (
    <div
      className={`p-2 overflow-y-auto h-full scrollbar-thin ${mode === 'light' ? 'scrollbar-thumb-gray-400 scrollbar-track-gray-200' : mode === 'dark' ? 'scrollbar-thumb-gray-600 scrollbar-track-gray-800' : 'scrollbar-thumb-yellow-500 scrollbar-track-gray-900'}`}
    >
        ...
        <PanelGroup direction="vertical">
          <Panel minSize={10}>
            <div className="h-full overflow-hidden">
              <CustomScrollbar mode={mode} className="h-full">
                <div
                  className={`p-4 max-h-[calc(100%-8px)] overflow-y-auto custom-scrollbar ${
                    mode === 'light'
                ...
                >
                  ...
                  <ReactMarkdown
                    className={`prose max-w-none ${mode === 'light' ? 'prose-gray' : mode === 'dark' ? 'prose-invert' : 'prose-yellow prose-invert'}`}
                  >
          ...
          <Panel minSize={10}>
            <div className="h-full overflow-hidden border-t border-gray-200 dark:border-gray-600">
              <CustomScrollbar mode={mode} className="h-full">
                <FileExplorer files={fileStructure} mode={mode} handleOpenFile={handleOpenFile} globalCodeFiles={globalCodeFiles} codeFiles={codeFiles} />
              </CustomScrollbar>
            </div>
          </Panel>
        </PanelGroup>
