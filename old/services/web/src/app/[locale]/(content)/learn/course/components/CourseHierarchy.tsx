...
export default function CourseHierarchy({ structure, onSelectSection }: CourseHierarchyProps) {
      ...
      return (
        <li key={fullPath} className="mb-2">
          <div className="flex items-center">
            {isExpandable && (
              <button onClick={() => toggleItem(fullPath)} className="mr-2">
                {isExpanded ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
              </button>
            )}
            <span className="cursor-pointer hover:text-blue-600" onClick={() => onSelectSection(fullPath)}>
              {key}
            </span>
          </div>
          {isExpanded && isExpandable && <ul className="ml-6 mt-2">{renderTree(value, `${fullPath}/`)}</ul>}
        </li>
      );
    });
  };
  ...
  return (
    <nav className="w-64 bg-white shadow-md p-4">
      ...
      <ul>{structure && renderTree(structure)}</ul>
    </nav>
  );
}
