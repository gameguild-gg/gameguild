          ...
          {Object.entries(exercises).map(([id, exercise]) => (
            <tr key={id} className="hover:bg-gray-50">
              <td className="px-4 py-3">
                <button onClick={() => onExerciseClick(id)} className="text-blue-600 hover:text-blue-800 font-medium">
                  {exercise.title}
                </button>
              </td>
              <td className="px-4 py-3 text-sm text-gray-500">{exercise.description.split('\n')[0]}</td>
              <td className="px-4 py-3 text-sm text-gray-500">{exercise.type}</td>
            </tr>
          ))}
        </tbody>
      </table>
