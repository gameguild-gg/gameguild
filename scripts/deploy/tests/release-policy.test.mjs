...
const prVerifyWorkflowPath = new URL(
...
test("manual releases are an administrator-only audited hotfix path", async () => {
  ...
  assert.match(
    workflow,
    /Only a repository administrator may promote a production hotfix/,
  );
  assert.match(
    workflow,
    /The hotfix branch must contain the latest main commit/,
  );
...
test("live Forgejo and Devtron automation owns normal production releases", async () => {
  const releaseWorkflow = await readFile(releaseWorkflowPath, "utf8");
  const prVerifyWorkflow = await readFile(prVerifyWorkflowPath, "utf8");

  assert.doesNotMatch(
    releaseWorkflow,
    /\n\s+push:\s*\n\s+branches:\s*\[main\]/,
  );
  assert.doesNotMatch(prVerifyWorkflow, /candidate-api:/);
  assert.doesNotMatch(prVerifyWorkflow, /candidate-web:/);
  assert.doesNotMatch(
    prVerifyWorkflow,
    /\.\/\.github\/actions\/build-candidate/,
  );
});
