...
describe("LearnerActivities", () => {
  it("lists every graded and participatory activity with persisted status", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Knowledge check" }),
    ).toBeInTheDocument();
    expect(screen.getByText("9 / 10")).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Playable build" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Production reflection" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Module survey" }),
    ).toBeInTheDocument();
