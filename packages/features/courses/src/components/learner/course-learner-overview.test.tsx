  ...
  assessments: [
    {
      id: "assessment-1",
      courseId: "course-1",
      title: "AI behavior quiz",
      type: "Quiz",
      dueAt: "2026-08-05T23:59:00Z",
      maxScore: 10,
    },
  ],
  submissions: [],
...
describe("CourseLearnerOverview", () => {
  it("shows cohort, next event, deadline, progress, and learner actions", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Advanced Game AI" }),
    ).toBeInTheDocument();
    ...
    ).toHaveAttribute("href", "/courses/advanced-game-ai/content");
    expect(
      screen.getByRole("link", { name: "View activities" }),
    ).toHaveAttribute("href", "/courses/advanced-game-ai/activities");
    expect(
      screen.getByRole("link", { name: "Open community" }),
    ).toHaveAttribute("href", "/courses/advanced-game-ai/community");
  });
});
