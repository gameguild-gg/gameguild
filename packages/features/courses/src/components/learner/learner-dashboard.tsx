  ...
  if (courses.length === 0) {
    return (
      <div className="space-y-8">
        ...
        <section className="flex min-h-72 flex-col items-center justify-center border-y py-12 text-center">
  ...
  return (
    <div className="space-y-8">
      ...
      <section
        ...
        className="grid overflow-hidden border-y bg-muted/30 lg:grid-cols-[minmax(0,1fr)_20rem]"
      >
        <div className="p-6 lg:p-8">
          <p className="text-xs font-medium uppercase text-primary">
            Continue learning
          </p>
          <h2 id="continue-heading" className="mt-2 text-2xl font-semibold">
            {active.title}
          </h2>
          <p className="mt-2 text-sm text-muted-foreground">
            {active.currentItem?.title ?? "Open the classroom to continue."}
          </p>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Only courses tied to your authenticated enrollment are listed.
          </p>
        </div>
        <div className="divide-y border-y">
          {courses.map((course) => (
            <Link
              ...
              className="grid gap-4 py-5 transition-colors hover:bg-muted/40 sm:grid-cols-[minmax(0,1fr)_8rem_9rem] sm:items-center sm:px-3"
            >
              <div className="min-w-0">
                <h3 className="truncate font-medium">{course.title}</h3>
                <p className="mt-1 truncate text-sm text-muted-foreground">
                  {course.currentItem?.title ??
                    `${course.completedItems} of ${course.totalItems} complete`}
                </p>
              </div>
              <div>
                <p className="text-sm font-medium">{course.overallProgress}%</p>
