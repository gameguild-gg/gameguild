...
export const INSERT_DIVIDER_LEXICAL_COMMAND: LexicalCommand<void> = createCommand(
...
export function DividerPlugin() {
  ...
  useEffect(() => {
    if (!editor.hasNodes([DividerLexicalNode])) {
      throw new Error("DividerPlugin: DividerLexicalNode not registered on editor")
    }
