...
function $getAdjacentNodeFromCollapsedSelection(
  isBackspace: boolean,
): LexicalNode | null {
  ...
  return isBackspace
    ? $getPreviousNodeFrom(anchorNode)
    : $getNextNodeFrom(anchorNode);
}
 ...
 */
export function $wouldDeletionAffectNodeKey(
  nodeKey: NodeKey,
  isBackspace: boolean,
): boolean {
  const selection = $getSelection();
  ...
  if ($isRangeSelection(selection)) {
    if (!selection.isCollapsed()) {
      ...
      for (const n of nodes) {
        const children =
          "getChildren" in n &&
          typeof (n as { getChildren?: unknown }).getChildren === "function"
            ? (
                n as unknown as {
                  getChildren: () => Array<{ getKey: () => string }>;
                }
              ).getChildren()
            : [];
