...
export function MermaidPlugin() {
  ...
  const { MermaidEditor } = useLexicalSurfaceAdapters()

  useEffect(() => {
    if (!editor.hasNodes([MermaidLexicalNode])) {
      throw new Error("MermaidPlugin: MermaidLexicalNode not registered on editor")
    }
  ...
  const handleSave = (data: MermaidData) => {
    editor.update(() => {
      const node = $createMermaidLexicalNode(data.code, data.type as MermaidDiagramType)
  ...
  return isModalOpen && MermaidEditor ? (
    <MermaidEditor
      initialData={{
        code: "graph TD\n    A[Start] --> B[End]",
        type: "flowchart",
