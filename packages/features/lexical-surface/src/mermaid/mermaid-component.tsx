...
export function MermaidLexicalComponent({
  code,
  diagramType,
  theme,
  themeMode,
  ...
  size,
  nodeKey,
}: MermaidLexicalComponentProps): React.JSX.Element {
  ...
  const { MermaidEditor, MermaidViewer } = useLexicalSurfaceAdapters()
      ...
      editor.registerCommand(
        ...
        (event: MouseEvent) => {
          ...
          if (
            ...
            // Check if the click target is an interactive control (zoom buttons, fullscreen etc.)
            if (
  ...
  const setData = (data: MermaidData) => {
    editor.update(() => {
      const node = $getNodeByKey(nodeKey)
      if ($isMermaidLexicalNode(node)) {
        node.setCode(data.code)
        node.setDiagramType(data.type as MermaidDiagramType)
  ...
  // MermaidData object for the viewer and editor
  const mermaidData = useMemo<MermaidData>(() => ({
    code,
    type: diagramType,
    theme,
    themeMode,
    ...
    caption,
    size,
  }), [code, diagramType, theme, themeMode, title, caption, size])

  return (
    <>
      {isEditable && isModalOpen && MermaidEditor && (
        <MermaidEditor
        ...
        >
          <div className="p-4">
            {MermaidViewer ? (
              ...
              <pre className="overflow-auto whitespace-pre-wrap text-sm">{code}</pre>
            )}
          </div>
        </div>
      </div>
