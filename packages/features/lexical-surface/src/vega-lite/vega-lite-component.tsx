...
export function VegaLiteLexicalComponent({
  ...
  const { VegaLiteEditor, VegaLiteViewer } = useLexicalSurfaceAdapters()
  ...
  return (
    <>
      {isEditable && isModalOpen && VegaLiteEditor && (
        <VegaLiteEditor
        ...
        >
          <div className="p-4">
            {VegaLiteViewer ? (
              ...
              <pre className="overflow-auto whitespace-pre-wrap text-sm">{spec}</pre>
            )}
          </div>
        </div>
      </div>
