...
import { useNodesDeleteProtection } from "../../shared/lexical/node-delete-protection";
...
function getMouseInfo(event: MouseEvent): {
  ...
  if (isHTMLElement(target)) {
    ...
    const isOutside = !(
      codeDOMNode || target.closest<HTMLElement>("div.lexical-code-action-menu")
    );
...
function CodeActionMenuContainer({ anchorElem }: { anchorElem: HTMLElement }) {
  ...
  const [pendingDeleteKey, setPendingDeleteKey] = useState<NodeKey | null>(
    null,
  );
  ...
  const onMouseMove = useCallback(
    (event: MouseEvent) => {
      ...
      if (found) {
        const { y: anchorY, right: anchorRight } =
...
export function CodeActionMenuPlugin({
  ...
  useEffect(() => {
    if (!anchorElem && typeof document !== "undefined")
      setResolvedAnchor(document.body);
  }, [anchorElem]);
