...
import {
  $createAdmonitionLexicalNode,
  AdmonitionLexicalNode,
} from "./admonition-node";
...
export const INSERT_ADMONITION_LEXICAL_COMMAND: LexicalCommand<
  AdmonitionPayload | undefined
> = createCommand("INSERT_ADMONITION_LEXICAL_COMMAND");
...
export function AdmonitionPlugin() {
  ...
  useEffect(() => {
    if (!editor.hasNodes([AdmonitionLexicalNode])) {
      throw new Error(
        "AdmonitionPlugin: AdmonitionLexicalNode not registered on editor",
      );
    }
