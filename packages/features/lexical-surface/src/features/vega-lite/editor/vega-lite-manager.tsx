...
export function VegaLiteManager({
  ...
  const handleFileUpload = async (
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!isCSV && !isJSON) {
        alert(
          `Arquivo "${filename}" não é CSV ou JSON. Por favor, envie apenas arquivos .csv ou .json`,
        );
        continue;
      }
      ...
      try {
        ...
        if (isJSON) {
          alert(
            `Erro ao ler arquivo "${filename}". Verifique se é um JSON válido.`,
          );
        } else {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Envie arquivos CSV ou JSON para usar no seu gráfico Vega-Lite. Os
            arquivos serão salvos junto com a especificação.
          </DialogDescription>
        </DialogHeader>
          ...
          {totalCount > 0 ? (
            ...
            <div className="text-center py-8 text-muted-foreground border rounded-md border-dashed">
              ...
              <p className="text-xs mt-1">
                Envie arquivos CSV ou JSON para usar nos gráficos
              </p>
            </div>
          )}
        </div>
