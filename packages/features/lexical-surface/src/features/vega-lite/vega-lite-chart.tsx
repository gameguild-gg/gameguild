...
export async function renderVegaChart(
  ...
  console.log(
    "VegaChart Renderer: Starting render with layout:",
    layout,
    "theme:",
    theme,
  );
  ...
  try {
    ...
    try {
      console.log(
        "VegaChart Renderer: Attempting to import Vega-Lite and themes...",
      );
      const vegaLiteImport = await import("vega-lite").catch(() => null);
      const vegaImport = await import("vega").catch(() => null);
      const vegaThemesImport = await import("vega-themes").catch(() => null);

      if (!vegaLiteImport || !vegaImport) {
        console.log(
          "VegaChart Renderer: Vega-Lite not available, showing placeholder",
        );
        throw new Error("Vega-Lite not available");
      }

      console.log(
        "VegaChart Renderer: Vega-Lite imported successfully, compiling spec...",
      );
      ...
      if (vegaThemesImport && theme !== "default" && THEME_MAP[theme]) {
        ...
        if (themeConfig) {
          ...
          if (isDarkTheme && baseThemeName !== "dark") {
            console.log(
              "VegaChart Renderer: Creating dark version of theme:",
              baseThemeName,
            );
          ...
          if (overrides) {
            console.log(
              "VegaChart Renderer: Applying manual theme overrides for:",
              theme,
            );
            console.log(
              "VegaChart Renderer: Override values:",
              JSON.stringify(overrides, null, 2),
            );
            finalThemeConfig = applyThemeOverrides(finalThemeConfig, overrides);
            console.log(
              "VegaChart Renderer: Final theme config:",
              JSON.stringify(finalThemeConfig, null, 2),
            );
          } else {
            console.log(
              "VegaChart Renderer: No overrides found for theme:",
              theme,
            );
          }
      ...
      try {
        ...
        console.error(
          "VegaChart Renderer: Error during view.runAsync():",
          runError,
        );
        throw runError;
      }
      ...
      setTimeout(async () => {
        ...
        console.log(
          "VegaChart Renderer: Container children count:",
          container.children.length,
        );

        if (container.children.length === 0) {
          console.error(
            "VegaChart Renderer: No children in container after render!",
          );
          console.log(
            "VegaChart Renderer: Trying alternative rendering approach...",
          );
          ...
          try {
            ...
            console.log(
              "VegaChart Renderer: Got SVG string, length:",
              svgString.length,
            );
            ...
            if (container.firstElementChild) {
              ...
              if (layout === "square") {
                ...
                console.log(
                  "VegaChart Renderer: Applied fixed square styles to SVG",
                );
              } else {
                ...
                console.log(
                  "VegaChart Renderer: Applied fixed rectangular styles to SVG",
                );
              }
            ...
            try {
              ...
              console.error(
                "VegaChart Renderer: Canvas approach failed:",
                canvasError,
              );
              ...
              try {
                ...
                view
                  ...
                  .catch((err: any) => {
                    console.error(
                      "VegaChart Renderer: Retry render failed:",
                      err,
                    );
                  });
              } catch (initError) {
                console.error(
                  "VegaChart Renderer: Re-initialization failed:",
                  initError,
                );
              }
            }
        ...
        if (container.firstElementChild) {
          ...
          console.log(
            "VegaChart Renderer: Applying styles for layout:",
            layout,
          );
          ...
          if (layout === "square") {
            ...
            svgElement.style.maxHeight = "400px";
            console.log(
              "VegaChart Renderer: Applied fixed square layout centering",
            );
          } else {
            // Rectangular layout - force fixed dimensions
            svgElement.style.width = "100%";
            ...
            svgElement.style.maxHeight = "300px";
            console.log(
              "VegaChart Renderer: Applied fixed rectangular layout styling",
            );
          }
          ...
          if (rect.width === 0 || rect.height === 0) {
            ...
            console.log(
              "VegaChart Renderer: Applied fallback dimensions for layout:",
              layout,
            );
          }
          ...
          console.log(
            "VegaChart Renderer: SVG dimensions:",
            svgElement.getBoundingClientRect(),
          );
          console.log(
            "VegaChart Renderer: SVG innerHTML length:",
            svgElement.innerHTML.length,
          );
        } else {
      ...
      console.log(
        "VegaChart Renderer: Vega error, showing placeholder:",
        vegaError.message,
      );
