...
import { QuizEditorSettingsMenu } from "./quiz-editor-settings-menu";
import type { QuizEditorSettings } from "./use-quiz-editor-settings";

const HTML_SCROLL_LOCK_CLASS = "quiz-surface-editor-scroll-lock";
...
function workspaceStyle(size: QuizEditorModalSize): CSSProperties {
  if (size === "compact") {
    return {
  ...
  return createPortal(
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={typeof title === "string" ? title : "Quiz editor"}
      ...
      >
        <header className="flex min-h-14 items-center justify-between gap-4 border-b border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <QuizEditorSettingsMenu settings={settings} />
            <button
              type="button"
              onClick={onClose}
              className="inline-flex h-8 w-8 items-center justify-center rounded transition-colors hover:bg-gray-200 dark:hover:bg-gray-800"
