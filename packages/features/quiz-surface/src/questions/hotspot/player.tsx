...
import type { HotspotEntry } from "@game-guild/quiz"
import type { HotspotLearnerEntry } from "@game-guild/quiz"
...
export function HotspotRenderer({
  ...
  const { url: imageUrl, loading: imageLoading, error: imageError } = useResolvedAssetUrl(entry.imageAssetUri)
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {imageUrl ? (
          <img src={imageUrl} alt="Hotspot question" className="block w-full" draggable={false} />
        ) : (
          <div className="flex min-h-48 items-center justify-center text-sm text-muted-foreground">
            {imageLoading ? "Loading image..." : imageError ? "Image unavailable" : "No image selected"}
          </div>
        )}
