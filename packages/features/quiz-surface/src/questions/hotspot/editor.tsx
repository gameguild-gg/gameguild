...
import { X, Plus, Crosshair, Trash2, ImageIcon } from "lucide-react"
import { AssetPickerDialog, useResolvedAssetUrl } from "@game-guild/assets/react"
import { Button } from "@game-guild/ui/components/button"
import { Input } from "@game-guild/ui/components/input"
import { Label } from "@game-guild/ui/components/label"
import { createQuizEntityId, type HotspotEntry, type HotspotPoint } from "@game-guild/quiz"
...
export function HotspotEditor() {
  ...
  const imageAssetUri = watch("imageAssetUri")
  const { url: imageUrl } = useResolvedAssetUrl(imageAssetUri)
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      setValue("imageWidth", image.naturalWidth)
      setValue("imageHeight", image.naturalHeight)
    }
    image.src = imageUrl
  }, [imageUrl, setValue])
  ...
  return (
    <div className="space-y-5">
      ...
      {!imageAssetUri ? (
        <button
          type="button"
          className="w-full rounded-md border-2 border-dashed p-8 text-center transition-colors hover:border-primary"
          onClick={() => setPickerOpen(true)}
        >
          <ImageIcon className="mx-auto mb-3 h-8 w-8 text-muted-foreground" />
          <span className="text-sm font-medium">Choose a hotspot image</span>
        </button>
      ) : (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={() => { setValue("imageAssetUri", null); setValue("hotspots", []); setSelectedId(null) }}
      ...
      <AssetPickerDialog
        ...
        title="Choose hotspot image"
        description="Upload an image or select one from the asset library."
        accept="image/*"
        kinds={["image"]}
        onSelect={(selection) => {
          const asset = Array.isArray(selection) ? selection[0] : selection
          if (!asset) return
          setValue("imageAssetUri", asset.uri)
          setValue("hotspots", [])
          setSelectedId(null)
        }}
      />
