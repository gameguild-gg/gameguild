...
export interface EssayEntry extends QuizEntryBase {
  ...
  correctAnswer?: SerializedRichTextPayload
...
export function createQuizEntityId(): string {
...
export function createSingleChoiceEntry(stem = ""): SingleChoiceEntry {
  const options = ["Paris", "London", "Berlin", "Madrid"].map((text) => ({
    id: createQuizEntityId(),
    text,
  }))
  return {
    type: QuizEntryType.SingleChoice,
...
export function createMultipleChoiceEntry(stem = ""): MultipleChoiceEntry {
  const options = ["2", "3", "4", "7"].map((text) => ({
    id: createQuizEntityId(),
    text,
  }))
  return {
    ...
    stem,
    options,
    correctOptionIds: [options[0]!.id, options[1]!.id, options[3]!.id],
    settings: createDefaultSettings(),
  }
}
...
export function createMatchingEntry(stem = ""): MatchingEntry {
  return {
    ...
    stem,
    pairs: [
      { id: createQuizEntityId(), left: "France", right: "Paris" },
      { id: createQuizEntityId(), left: "Japan", right: "Tokyo" },
      { id: createQuizEntityId(), left: "Brazil", right: "Brasília" },
    ],
    settings: createDefaultSettings(),
  }
}
...
export function createOrderingEntry(stem = ""): OrderingEntry {
  return {
    ...
    stem,
    items: [
      { id: createQuizEntityId(), text: "World War I", correctPosition: 0 },
      { id: createQuizEntityId(), text: "World War II", correctPosition: 1 },
      { id: createQuizEntityId(), text: "Moon Landing", correctPosition: 2 },
    ],
    settings: createDefaultSettings(),
  }
}
...
export function createCategorizationEntry(stem = ""): CategorizationEntry {
  const fruitId = createQuizEntityId()
  const vegetableId = createQuizEntityId()
  return {
    type: QuizEntryType.Categorization,
    stem,
    categories: [
      { id: fruitId, name: "Fruits" },
      { id: vegetableId, name: "Vegetables" },
    ],
    items: [
      { id: createQuizEntityId(), text: "Apple", correctCategoryIds: [fruitId] },
      { id: createQuizEntityId(), text: "Carrot", correctCategoryIds: [vegetableId] },
      { id: createQuizEntityId(), text: "Banana", correctCategoryIds: [fruitId] },
    ],
    settings: createDefaultSettings(),
  }
}
...
export function createNumericEntry(stem = ""): NumericEntry {
  return {
    ...
    stem,
    variables: [
      { id: createQuizEntityId(), name: "x", min: 1, max: 10, decimals: 0 },
      { id: createQuizEntityId(), name: "y", min: 1, max: 10, decimals: 0 },
    ],
    formula: "x^2 + y",
