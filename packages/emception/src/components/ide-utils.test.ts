...
import { buildFileTree, buildSDL3Args, detectsSDL, fileName, inferLanguage, isSourceFile, isTextFile, toWorkspaceFsPath } from './ide-utils';
...
describe('buildSDL3Args', () => {
  ...
  it('includes the emscripten browser JS library for main-loop support', () => expect(args).toContain('/usr/lib/emscripten/src/lib/library_browser.js'));
  it('enables SINGLE_FILE=1 to embed wasm as base64', () => {
    const idx = args.indexOf('SINGLE_FILE=1');
    expect(idx).toBeGreaterThan(-1);
    expect(args[idx - 1]).toBe('-s');
  });
  it('enables ALLOW_MEMORY_GROWTH=1', () => {
    const idx = args.indexOf('ALLOW_MEMORY_GROWTH=1');
    expect(idx).toBeGreaterThan(-1);
    expect(args[idx - 1]).toBe('-s');
  });
  it('sets ERROR_ON_UNDEFINED_SYMBOLS=0 to allow unresolved camera/sensor stubs', () => {
    const idx = args.indexOf('ERROR_ON_UNDEFINED_SYMBOLS=0');
    expect(idx).toBeGreaterThan(-1);
    expect(args[idx - 1]).toBe('-s');
  });
