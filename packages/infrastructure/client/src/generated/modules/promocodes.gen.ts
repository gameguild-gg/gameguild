/**
 * @game-guild/client - Promocodes Module
 *
...
export class PromocodesModule {
  ...
  async getPromoCodes(query?: {
    ...
    skip?: number;
    take?: number;
  }): Promise<Result<Types.CommerceProductsPagedResult, ApiError>> {
    ...
    // Validate response
    if (result.ok) {
      const validatedData = safeParse(Types.CommerceProductsPagedResultSchema, result.data, 'response');
...
export function createPromocodesModule(client: ApiClient): PromocodesModule {
  return new PromocodesModule(client);
}
