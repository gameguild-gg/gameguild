...
export class UsersModule {
  ...
  async getUsers(query?: {
    ...
    Result<Types.PagedResultOfGameGuildIdentityUsersUserDto, ApiError>
  > {
    const url = "/v1/users";
    ...
    if (result.ok) {
      const validatedData = safeParse(
        Types.PagedResultOfGameGuildIdentityUsersUserDtoSchema,
        result.data,
        "response",
      );
