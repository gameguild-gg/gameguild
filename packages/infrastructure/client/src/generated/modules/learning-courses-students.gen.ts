...
export class LearningCoursesStudentsModule {
  ...
  async postCoursesStudentsMessage(
    ...
    Result<Types.LearningCoursesSendCourseStudentMessageOutput, ApiError>
  > {
    const url = `/v1/courses/${courseId}/students/message`;

    // Validate request body
    const validatedBody = safeParse(
      Types.LearningCoursesSendCourseStudentMessageInputSchema,
      body,
      "request",
    );
    ...
    if (result.ok) {
      const validatedData = safeParse(
        Types.LearningCoursesSendCourseStudentMessageOutputSchema,
        result.data,
        "response",
      );
...
export function createLearningCoursesStudentsModule(
  client: ApiClient,
): LearningCoursesStudentsModule {
