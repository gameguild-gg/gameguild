...
export class TenantsModule {
  ...
  async getTenants(query?: {
    ...
    Result<Types.PagedResultOfGameGuildIdentityTenantsTenant, ApiError>
  > {
    ...
    if (result.ok) {
      const validatedData = safeParse(
        Types.PagedResultOfGameGuildIdentityTenantsTenantSchema,
        result.data,
        "response",
      );
  ...
  > {
    ...
    if (result.ok) {
      const validatedData = safeParse(
        Types.PagedResultOfGameGuildIdentityTenantsTenantAuditLogEntrySchema,
        result.data,
        "response",
      );
      return { ok: true, data: validatedData };
