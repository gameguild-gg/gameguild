/**
 * @game-guild/client - Taxjurisdictions Module
 *
...
export class TaxjurisdictionsModule {
  ...
  async getTaxJurisdictions(): Promise<Result<Array<Types.CommercePaymentsTaxRate>, ApiError>> {
    const url = '/v1/tax-jurisdictions';
  ...
  async getTaxJurisdictions1(jurisdictionId: string): Promise<Result<Types.CommercePaymentsTaxJurisdiction, ApiError>> {
    const url = `/v1/tax-jurisdictions/${jurisdictionId}`;
...
export function createTaxjurisdictionsModule(client: ApiClient): TaxjurisdictionsModule {
  return new TaxjurisdictionsModule(client);
}
