 ...
 */
export function getOAuthExchangeToken(provider: OAuthProviderWithMethods): OAuthProviderWithMethods['exchangeToken'] {
  return typeof provider.exchangeToken === 'function' ? provider.exchangeToken : undefined;
}

/**
 * Type-safe accessor for getAuthorizeUrl method.
 */
export function getOAuthAuthorizeUrl(provider: OAuthProviderWithMethods): OAuthProviderWithMethods['getAuthorizeUrl'] {
  return typeof provider.getAuthorizeUrl === 'function' ? provider.getAuthorizeUrl : undefined;
}

/**
 * Type-safe accessor for handleCallback method.
 */
export function getOAuthHandleCallback(provider: OAuthProviderWithMethods): OAuthProviderWithMethods['handleCallback'] {
  return typeof provider.handleCallback === 'function' ? provider.handleCallback : undefined;
}
