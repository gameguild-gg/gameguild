...
public class PermissionServiceTenantTests : IDisposable {
  ...
  public async Task HasTenantPermissionAsync_ExpiredPermission_ReturnsFalse() {
    ...
    var permission = new TenantPermission { UserId = userId, TenantId = tenantId, ExpiresAt = DateTime.UtcNow.AddDays(-1) };
  ...
  public async Task HasTenantPermissionAsync_DeletedPermission_ReturnsFalse() {
    ...
    var permission = new TenantPermission { UserId = userId, TenantId = tenantId };
    permission.AddPermission(PermissionType.Read);
  ...
  public async Task GetTenantPermissionsAsync_WithPermissions_ReturnsCorrectList() {
    ...
    var userId = Guid.NewGuid();
    var tenantId = Guid.NewGuid();
    var expectedPermissions = new[] { PermissionType.Read, PermissionType.Comment, PermissionType.Vote };
  ...
  public async Task RevokeTenantPermissionAsync_ExistingPermissions_RemovesSpecified() {
    ...
    var userId = Guid.NewGuid();
    var tenantId = Guid.NewGuid();
    var grantedPermissions = new[] { PermissionType.Read, PermissionType.Comment, PermissionType.Vote };
    var revokePermissions = new[] { PermissionType.Comment };
  ...
  public async Task JoinTenantAsync_ExistingMembership_ReactivatesIfNeeded() {
    ...
    var expiredMembership = new TenantPermission { UserId = userId, TenantId = tenantId, ExpiresAt = DateTime.UtcNow.AddDays(-1) };
