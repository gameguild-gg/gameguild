// @emception/react — Phase 6.2 React surface.
//
// Provides:
...
//     it just memoizes the builder + cleans up on unmount. Phase 7.2
...
type EventHandlerProps = {
    [E in EmceptionEventName as `on${Capitalize<CamelCase<E>>}`]?: (
        payload: EmceptionEventMap[E],
    ) => void;
};
...
const KNOWN_EVENT_NAMES: readonly EmceptionEventName[] = Object.keys(
    EVENT_DOM_NAMES,
 ...
 * Computed once.
 */
const ATTRIBUTE_PROP_KEYS: ReadonlyArray<keyof ViewConfigInput> = [
    'preset', 'manifestUrl', 'workspace', 'source', 'seedUrl', 'buildUrl',
    'seedPolicy', 'autorun', 'canvas', 'showHidden', 'showSolution',
    'stdin', 'stdout', 'stderr',
];
...
export const EmceptionRun = forwardRef<EmceptionRunHandle, EmceptionRunProps>(
    function EmceptionRun(props, ref) {
        ...
        // skipped — those scenarios should set `api` directly via Phase
        // 7.2's `createEmception` and pass the result through `props.api`.
        ...
        useEffect(() => {
            ...
            for (const name of KNOWN_EVENT_NAMES) {
                ...
                const handler = (rest as Record<string, unknown>)[propKey] as
