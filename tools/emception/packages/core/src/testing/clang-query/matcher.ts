/**
 * Clang-query matcher engine (Phase 5.4 — runtime-agnostic half).
 *
...
class Parser {
    ...
    parse(): ParsedCall {
        ...
        if (this.i !== this.src.length) {
            throw new TestFailureError(
                `clang-query matcher: trailing input at position ${this.i} in ${JSON.stringify(this.src)}`,
            );
        }
        return top;
    }
    ...
    private parseCall(): ParsedCall {
        ...
        if (this.peek() !== '(') {
            throw new TestFailureError(
                `clang-query matcher: expected '(' after '${name}' at position ${this.i}`,
            );
        }
        this.i++; // consume '('
        ...
        while (true) {
            ...
            if (c === ')') { this.i++; return { kind: 'call', name, args }; }
            throw new TestFailureError(
                `clang-query matcher: expected ',' or ')' at position ${this.i}`,
            );
        }
    }
...
const FACTORIES: Record<string, MatcherFactory> = {
    ...Object.fromEntries(
        Object.entries(KIND_MATCHERS).map(([name, spec]) => [name, kindFactory(spec)]),
    ),
...
function compileCall(call: ParsedCall): CompiledMatcher {
    ...
    if (!factory) {
        throw new TestFailureError(
            `clang-query matcher: unsupported matcher '${call.name}'. ` +
            `Supported: ${Object.keys(FACTORIES).sort().join(', ')}.`,
        );
    }
    const inner = call.args
        .filter((a): a is ParsedCall => a.kind === 'call')
        .map(compileCall);
