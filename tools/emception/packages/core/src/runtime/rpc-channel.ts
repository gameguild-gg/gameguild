...
import { RequestCorrelator, CorrelatorDisposedError } from './request-correlator.js';
...
export class RpcChannel<TRequest = unknown, TResponse = unknown> {
     ...
     */
    async request(
        build: (id: number) => TRequest,
        transfer?: unknown[],
        label?: string,
    ): Promise<TResponse> {
        if (this.disposed) {
            throw new CorrelatorDisposedError(
                'Cannot issue request: RpcChannel already disposed.',
            );
        }
    ...
    notify(message: TRequest, transfer?: unknown[]): void {
        if (this.disposed) {
            throw new CorrelatorDisposedError(
                'Cannot notify: RpcChannel already disposed.',
            );
        }
