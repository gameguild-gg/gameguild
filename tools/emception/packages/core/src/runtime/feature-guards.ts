// Phase 7.7 — runtime feature guards.
//
...
// (Phase 1.7 SSR safety): we recognise the shapes by their structural
...
export function assertCanvasUnsupported(canvas: unknown, runtime: RuntimeLabel = 'this runtime'): void {
    ...
    if (!looksLikeCanvas(canvas)) {
        throw new RuntimeFeatureUnavailableError(
            `RunOptions.canvas was provided to ${runtime} but the value is not a Canvas-shaped object`,
        );
    }
    throw new CanvasUnavailableError(
        `Canvas output is unavailable in ${runtime}. Use @emception/browser (web worker + OffscreenCanvas) for SDL / GUI presets.`,
    );
}
 ...
 */
export function assertNoBrowserOnlyFeatures(
    opts: { canvas?: unknown; stdin?: StdinInput } | undefined,
    runtime: RuntimeLabel = 'this runtime',
): void {
    if (!opts) return;
