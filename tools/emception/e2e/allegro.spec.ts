/**
 * End-to-end test: Allegro 5 C++ canvas path.
 *
 * Selects the "C++ Allegro 5 — Bouncing Ball" preset, clicks "Compile & Run",
 * and verifies:
 *   1. Status transitions through "Compiling..." → "allegro done (…s)".
 *   2. Terminal shows "allegro detected" and "rendering in canvas tab".
 *   3. The canvas element is visible with non-zero dimensions.
 *
 * Compilation links against precompiled Allegro static libs; allow generous
...
test.describe('Allegro 5 C++ canvas', () => {
    test('Allegro 5 demo compiles and renders to canvas', async ({ page }) => {
        // Allegro uses clang+wasm-ld (not emcc) but links many static archives — allow 15 min.
        ...
        // ── Select the allegro preset ─────────────────────────────────────────
        console.log('Selecting cpp-allegro preset…');
        await workspacePicker(page).selectOption('cpp-allegro');
        // Wait for the preset switch to settle (editor remounts).
        await page.waitForTimeout(1_000);

        // Verify allegro source in workspace state (more reliable than Monaco CDN loading).
        ...
        if (finalStatus && /compilation failed/.test(finalStatus)) {
            dumpLogs(logs, 'ALLEGRO COMPILE FAILED');
            expect(
                finalStatus,
                'Allegro canvas compilation failed — check liballegro*.a are in the sysroot',
            ).not.toMatch(/compilation failed/);
        }

        if (finalStatus && !/allegro done|done.*running/.test(finalStatus)) {
            dumpLogs(logs, 'UNEXPECTED STATUS');
        }

        expect(finalStatus).toMatch(/allegro done|done.*running/);
        console.log('Allegro 5 compilation succeeded!');

        // ── Terminal messages ────────────────────────────────────
        await Promise.race([expect(terminal(page)).toContainText('allegro detected', { timeout: 5_000 }), crash]);
        ...
        console.log('Allegro 5 canvas is visible and has non-zero dimensions!');
        ...
        console.log('Allegro 5 acquired a WebGL context.');

        dumpLogs(logs, 'allegro PASS');
    });
});
