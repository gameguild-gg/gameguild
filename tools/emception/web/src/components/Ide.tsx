...
import { boot } from '@/lib/orchestrator';
...
// Define the Orchestrator type based on boot return value
...
export default function Ide() {
  ...
  const orchestratorRef = useRef<BootResult | null>(null);
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      // Write code to the virtual filesystem via the VFSManager's overlay property.
      const tWrite = performance.now();
      await vfs.overlay.writeFile('/home/user/main.cpp', enc.encode(code));
      ...
      const result = await runner.run('emcc', [
      ...
      if (result.exitCode === 0) {
        ...
        // Check output file
        const wasmFile = await runner.getFile('/home/user/main.wasm');
