...
export interface TestCase {
  ...
  args?: unknown[]; // For inout tests: array of arguments to pass to the solution function
  expectedReturn?: unknown[]; // Expected return value(s) from the solution function
  predicate?: string;
}
...
export interface TestRunnerOptions {
  ...
  setTestResults: (
    results:
      ...
      | ((
          prev: Record<string, { passed: boolean; actual: string; expected: string }[]>,
        ) => Record<string, { passed: boolean; actual: string; expected: string }[]>),
  ) => void;
