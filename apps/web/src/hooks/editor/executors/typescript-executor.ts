      ...
      try {
        ...
        if (diagnostics.length > 0) {
          const errorMessages = diagnostics.map(
            (diag) => `Error ${diag.file?.fileName}(${diag.start}): ${diag.messageText}`,
          )
        ...
        const modifiedCode = transpiledCode
  ...
  handleCommand = (command: string, context: ExecutionContext): boolean => {
    ...
    if (command.includes(":") || command.includes("interface") || command.includes("class")) {
      try {
        ...
        addOutput(
          "Direct TypeScript evaluation in the terminal is not supported. Please create a file to execute TypeScript code.",
        )
        return true
      } catch (error) {
