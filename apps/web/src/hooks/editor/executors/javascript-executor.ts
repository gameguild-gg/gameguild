    ...
    const resolveFile = (fileName: string): string => {
      ...
      for (const match of activeImports) {
        ...
        if (resolvedPath) {
          ...
          const { named: availableExports, hasDefaultExport, defaultExportName } = this.extractExports(importedFileContent);
...
`;
          ...
          // Handle named imports
            ...
            if (unavailableImports.length > 0) {
              debugLog(`Error: The following exports are not available in ${resolvedPath}: ${unavailableImports.join(', ')}`);
...
${hasDefaultExport && defaultExportName ? `if (typeof ${defaultExportName} !== 'undefined') { window.${namespaceImport}.default = ${defaultExportName}; }` : ''}
})();
`;
          } else {
