...
describe('cross-domain auth', () => {
  ...
  it('allows local routes and the configured website and learning origins', () => {
    ...
    expect(resolveAllowedAuthRedirect('/dashboard', options)).toBe('/dashboard');
    expect(
      resolveAllowedAuthRedirect(
        'https://learning.gameguild.gg/courses/game-ai/lessons/intro?mode=focus',
        options,
      ),
    ).toBe('https://learning.gameguild.gg/courses/game-ai/lessons/intro?mode=focus');
    expect(resolveAllowedAuthRedirect('https://gameguild.gg/courses', options)).toBe(
  ...
  it('rejects protocol-relative, foreign, malformed, and credential-bearing redirects', () => {
    ...
    expect(resolveAllowedAuthRedirect('//evil.example/path', options)).toBe('/dashboard');
    expect(resolveAllowedAuthRedirect(['/dashboard', 'https://evil.example'], options)).toBe(
      '/dashboard',
    );
    expect(resolveAllowedAuthRedirect('https://evil.example/path', options)).toBe('/dashboard');
    expect(
      resolveAllowedAuthRedirect('https://admin:secret@learning.gameguild.gg/path', options),
    ).toBe('/dashboard');
