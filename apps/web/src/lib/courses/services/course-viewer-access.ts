...
export type CourseViewerAccessState =
  "signed-out" | "has-access" | "no-access" | "unavailable";
...
export async function getCourseViewerAccess(
  courseId: string,
): Promise<CourseViewerAccess> {
  ...
  try {
    ...
    if (
      progressResult.error?.status === 403 ||
      progressResult.error?.status === 404
    ) {
