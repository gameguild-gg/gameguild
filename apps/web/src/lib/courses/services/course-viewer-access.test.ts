...
describe('getCourseViewerAccess', () => {
  beforeEach(() => {
    ...
    mocks.getCoursesMeProgress.mockResolvedValue({
      ok: true,
      data: { completionPercentage: 75, lastAccessedAt: '2026-09-15T09:00:00Z' },
    });
  });

  it('does not call Learning APIs without both token and authenticated user', async () => {
    mocks.getToken.mockResolvedValueOnce(null);
    await expect(getCourseViewerAccess('course-1')).resolves.toEqual({ state: 'signed-out' });
  ...
  it('returns progress and initializes the generated client with normalized configuration', async () => {
    ...
    const getAccessToken = mocks.createServerClient.mock.calls[0]?.[0].auth.getAccessToken;
  ...
  it.each([401, 403, 404])('maps access HTTP status %s without exposing an infrastructure error', async (status) => {
  ...
  it.each([
    ...
    [{ status: 503, detail: 'Progress unavailable', message: 'Request failed' }, '[503] Progress unavailable'],
    [{ status: 'invalid', message: 'Gateway unavailable' }, '[unknown] Gateway unavailable'],
  ...
  it('normalizes thrown Error and unknown failures', async () => {
    mocks.getCoursesMeProgress.mockRejectedValueOnce(new Error('Network unavailable'));
    await expect(getCourseViewerAccess('course-1')).resolves.toEqual({
      state: 'unavailable',
