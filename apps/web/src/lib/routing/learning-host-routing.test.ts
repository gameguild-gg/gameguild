...
describe("learning host routing", () => {
  it("recognizes regular, secure, and chunked GameGuild session cookies", () => {
    expect(hasGameGuildSessionCookie(null)).toBe(false);
    expect(hasGameGuildSessionCookie("theme=dark")).toBe(false);
    expect(hasGameGuildSessionCookie("gameguild.session-token=value")).toBe(
      true,
    );
    expect(
      hasGameGuildSessionCookie("__Secure-gameguild.session-token.0=value"),
    ).toBe(true);
  });
  ...
  it("redirects legacy assignments URLs to activities", () => {
    const decision = resolveLearningHostRoute({
      ...
      hostname: "learning.gameguild.gg",
      url: new URL(
        "https://learning.gameguild.gg/courses/game-ai/assignments?state=open",
      ),
    });
  ...
  it("uses central sign-in and returns to the exact learner URL", () => {
    const decision = resolveLearningHostRoute({
      ...
      hostname: "learning.gameguild.gg",
      url: new URL(
        "http://0.0.0.0:3000/courses/game-ai/lessons/intro?mode=focus",
      ),
      requiresAuthentication: true,
    });
  ...
  it("redirects direct internal learner routes on the main host", () => {
    const decision = resolveLearningHostRoute({
      ...
      url: new URL(
        "https://gameguild.gg/pt-BR/learn/courses/game-ai/grades?view=groups",
      ),
    });
