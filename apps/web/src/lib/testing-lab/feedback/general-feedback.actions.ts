...
import { getMyTestingRequests, getAvailableTestingOpportunities } from '../requests/requests.actions';
import type { CreateSimpleTestingRequestDto } from '@/lib/api/generated/types.gen';
import type { TestingFeedback, TestingRequest } from '@/lib/api/generated/types.gen';
 ...
 */
export async function submitGeneralTestingFeedback(feedbackData: {
...
export async function getUserTestingDashboard(userId?: string) {
  ...
  try {
    const [myRequests, availableOpportunities, myFeedback, userActivity] = await Promise.all([
      ...
      getAvailableTestingOpportunities(),
