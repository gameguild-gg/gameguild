...
function $convertVegaLiteElement(domNode: HTMLElement): null | DOMConversionOutput {
...
export class VegaLiteLexicalNode extends DecoratorNode<React.JSX.Element> {
  ...
  static importDOM(): DOMConversionMap | null {
    return {
      div: (domNode) => {
        if (!(domNode as HTMLElement).hasAttribute("data-lexical-vega-lite")) return null
  ...
  getTextContent(): string { return this.__spec }

  // ── Getters / Setters ──
  getSpec(): string { return this.__spec }
  setSpec(v: string): void { this.getWritable().__spec = v }
  ...
  getTheme(): NonNullable<VegaLiteData["theme"]> { return this.__theme }
  setTheme(v: NonNullable<VegaLiteData["theme"]>): void { this.getWritable().__theme = v }
  getThemeMode(): NonNullable<VegaLiteData["themeMode"]> { return this.__themeMode }
  setThemeMode(v: NonNullable<VegaLiteData["themeMode"]>): void { this.getWritable().__themeMode = v }
  getLayout(): NonNullable<VegaLiteData["layout"]> { return this.__layout }
  setLayout(v: NonNullable<VegaLiteData["layout"]>): void { this.getWritable().__layout = v }
  getData(): Record<string, string> { return this.__data }
  setData(v: Record<string, string>): void { this.getWritable().__data = v }
...
export function $createVegaLiteLexicalNode(
  spec = "{}",
): VegaLiteLexicalNode {
  return $applyNodeReplacement(
    new VegaLiteLexicalNode(spec),
  )
}
