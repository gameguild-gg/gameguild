...
function $convertStickyElement(domNode: HTMLElement): null | DOMConversionOutput {
  ...
  const size = (domNode.getAttribute("data-lexical-sticky-size") || "wide") as StickySize
  const xOffset = parseFloat(domNode.getAttribute("data-lexical-sticky-x") || "0")
  const yOffset = parseFloat(domNode.getAttribute("data-lexical-sticky-y") || "0")
...
export class StickyNode extends DecoratorNode<React.JSX.Element> {
  ...
  static clone(node: StickyNode): StickyNode {
    return new StickyNode(
      node.__text, node.__color, node.__style,
      node.__size, node.__xOffset, node.__yOffset, node.__key,
    )
  }
  ...
  // ── Color ──
  getColor(): string { return this.__color }
  setColor(color: string): void { this.getWritable().__color = color }

  // ── Style ──
  getStyle(): StickyStyle { return this.__style }
  setStyle(style: StickyStyle): void { this.getWritable().__style = style }

  // ── Size ──
  getSize(): StickySize { return this.__size }
  setSize(size: StickySize): void { this.getWritable().__size = size }

  // ── Position offsets ──
  getXOffset(): number { return this.__xOffset }
  setXOffset(x: number): void { this.getWritable().__xOffset = x }
  getYOffset(): number { return this.__yOffset }
  setYOffset(y: number): void { this.getWritable().__yOffset = y }
...
export function $createStickyNode(
  ...
  return $applyNodeReplacement(new StickyNode(text, color, style, size, xOffset, yOffset))
}
