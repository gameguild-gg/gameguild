...
export function PagesPlugin({
  ...
  useEffect(() => {
    ...
    const clearFixedHeight = (node: PageNode) => fixedPageHeights.delete(node.getKey())
    const clearMeasurementFlag = (node: PageNode) =>
      pagesMarkedForMeasurement.delete(node.getKey())
    const getFixedHeight = (node: PageNode) => fixedPageHeights.get(node.getKey())
    ...
    const $getUnderflowingChildren = (node: PageNode) => {
      ...
      withNaturalPageHeight(pageElement, () => {
        ...
        while (currentPageHeight < pageHeight - REFLOW_HYSTERESIS_PX) {
          const nextChild = nextPageChildNodes[overflowAfterIndex]?.cloneNode(true) as
            | ChildNode
            | undefined
          if (!nextChild) break
    ...
    const $getOverflowingChildren = (node: PageNode): ReturnType<PageContentNode["getChildren"]> => {
      const rootElement = editor.getRootElement()
      if (!rootElement) return []
      const pageElement = node.getPageElement()
      if (!pageElement) return []
      ...
      if (
        ...
        if (nextFirst) {
          for (let i = underflowingChildren.length - 1; i >= 0; i--) {
            nextFirst.insertBefore(underflowingChildren[i])
          }
        } else {
    ...
    const updateZoom = () => {
      const rootElement = editor.getRootElement()
      if (!rootElement) return
      const pageWidth = parseInt(rootElement.style.getPropertyValue("--page-width"), 10)
      if (!pageWidth) return
      ...
      const nextZoom = Math.min(rootWidth / (pageWidth + rootPadding), 1).toFixed(6)
    ...
    const $enforcePageStructure = () => {
      ...
      const children = root.getChildren()
      const isInvalid =
        !children.some($isPageNode) || children.some((child) => !$isPageNode(child))
    ...
    const removeCommandListeners = mergeRegister(
      ...
      editor.registerCommand(
        ...
        () => {
          ...
          if (oldPreviousPageKey === null || oldPreviousPageKey === currentPageKey) {
            return false
          }
