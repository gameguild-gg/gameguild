...
import { Settings2, Check, Trash2, Paintbrush, ArrowDownUp, MoveHorizontal, ChevronDown } from "lucide-react"
import {
...
import {
  ...
  getStyleClasses,
  getPaletteColor,
} from "../../extras/divider/divider-styles"
import type {
  DividerStyle, DividerThickness, DividerSpacing, DividerColorPalette
} from "./divider-node"
...
export function DividerLexicalComponent({
  style, thickness, spacing, colorPalette, customColor, nodeKey,
  ...
  const customCSSStyle = colorPalette === "custom" && customColor ? {
  ...
  const renderDivider = () => {
    switch (style) {
      case "gradient":
        return (
          <div className={`${spacingClass} ${thicknessClass} ${colorClass}`} style={customCSSStyle} aria-hidden="true" />
        )
      case "double":
        ...
        return (
          <div className={spacingClass}>
            ...
            >
              <div className="absolute top-0 left-0 right-0" style={{ height: doubleThickness, backgroundColor: paletteColor }} />
              <div className="absolute bottom-0 left-0 right-0" style={{ height: doubleThickness, backgroundColor: paletteColor }} />
            </div>
          </div>
        )
      default:
        return <hr className={`${spacingClass} ${thicknessClass} ${colorClass} ${styleClass} w-full`} style={customCSSStyle} />
    }
  }
            ...
            <DropdownMenuContent align="end" className="w-56" onCloseAutoFocus={(e) => e.preventDefault()}>
              ...
              <DropdownMenuSub>
                <DropdownMenuSubTrigger>
                  ...
                  Thickness: {THICKNESS_LIST.find((t) => t.id === thickness)?.label}
                </DropdownMenuSubTrigger>
                <DropdownMenuSubContent>
              ...
              <DropdownMenuSub>
                ...
                <DropdownMenuSubContent className="w-56" onFocusOutside={(e) => {
                  ...
                  <DropdownMenuSub>
                    <DropdownMenuSubTrigger>
                      <span className="w-4 h-4 mr-2 rounded-full border" style={{ backgroundColor: customColor || "#3b82f6" }} />
                      Custom Color
                    </DropdownMenuSubTrigger>
                    <DropdownMenuSubContent className="p-3" onFocusOutside={(e) => {
                      const t = (e as any).detail?.originalEvent?.target;
                      if (t instanceof Element && t.closest('[contenteditable="true"]')) {
                        e.preventDefault();
                      }
                      ...
                      />
                      <DropdownMenuSeparator />
                      <DropdownMenuItem onSelect={(e) => { e.preventDefault(); updateField((n) => n.setCustomColor(null)); }}>
                        Clear custom color
                      </DropdownMenuItem>
                    </DropdownMenuSubContent>
                  </DropdownMenuSub>
              ...
              <DropdownMenuItem onSelect={handleDelete} className="text-red-600 dark:text-red-400 focus:text-red-700 dark:focus:text-red-300">
