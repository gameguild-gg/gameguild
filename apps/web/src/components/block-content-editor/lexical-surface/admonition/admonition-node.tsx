...
import type { AdmonitionType } from "../../extras/admonition"
import { AdmonitionLexicalComponent } from "./admonition-component"

export type AdmonitionDesign = "default" | "compact" | "bordered" | "vertical-bar"
...
function $convertAdmonitionElement(domNode: HTMLElement): null | DOMConversionOutput {
  const admonitionType = (domNode.getAttribute("data-lexical-admonition-type") || "note") as AdmonitionType
  ...
  const design = (domNode.getAttribute("data-lexical-admonition-design") || "default") as AdmonitionDesign
  const customBorderColor = domNode.getAttribute("data-lexical-admonition-border-color") || ""
  const customTextColor = domNode.getAttribute("data-lexical-admonition-text-color") || ""
  const node = $createAdmonitionLexicalNode(admonitionType, title, content, design, customBorderColor, customTextColor)
  return { node }
}
...
export class AdmonitionLexicalNode extends DecoratorNode<React.JSX.Element> {
  ...
  static clone(node: AdmonitionLexicalNode): AdmonitionLexicalNode {
    return new AdmonitionLexicalNode(
      node.__admonitionType, node.__title, node.__content,
      node.__design, node.__customBorderColor, node.__customTextColor, node.__key,
    )
  }
  ...
  static importJSON(serializedNode: SerializedAdmonitionLexicalNode): AdmonitionLexicalNode {
  ...
  exportDOM(): DOMExportOutput {
    ...
    if (this.__customBorderColor) el.setAttribute("data-lexical-admonition-border-color", this.__customBorderColor)
    if (this.__customTextColor) el.setAttribute("data-lexical-admonition-text-color", this.__customTextColor)
  ...
  static importDOM(): DOMConversionMap | null {
    return {
      div: (domNode) => {
        if (!(domNode as HTMLElement).hasAttribute("data-lexical-admonition-type")) return null
  ...
  // ── Getters / Setters ──
  getAdmonitionType(): AdmonitionType { return this.__admonitionType }
  setAdmonitionType(type: AdmonitionType): void { this.getWritable().__admonitionType = type }

  getTitle(): string { return this.__title }
  setTitle(title: string): void { this.getWritable().__title = title }

  getContent(): string { return this.__content }
  setContent(content: string): void { this.getWritable().__content = content }

  getDesign(): AdmonitionDesign { return this.__design }
  setDesign(design: AdmonitionDesign): void { this.getWritable().__design = design }

  getCustomBorderColor(): string { return this.__customBorderColor }
  setCustomBorderColor(color: string): void { this.getWritable().__customBorderColor = color }

  getCustomTextColor(): string { return this.__customTextColor }
  setCustomTextColor(color: string): void { this.getWritable().__customTextColor = color }
...
export function $createAdmonitionLexicalNode(
  ...
  return $applyNodeReplacement(
    new AdmonitionLexicalNode(admonitionType, title, content, design, customBorderColor, customTextColor),
  )
}
