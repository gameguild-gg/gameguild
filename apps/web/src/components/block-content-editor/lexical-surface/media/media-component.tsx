...
import type { BaseMediaData, MediaType } from "../../nodes/base/media-node-base"
...
import {
  ...
  detectVideoFileType,
  detectAudioFileType,
} from "../../extras/media/url-detection"
...
export function MediaLexicalComponent({
  ...
  const handleMediaSelected = (result: any) => {
    const urls = Array.isArray(result) ? result.map((r) => r.data) : [result.data]
            ...
            {galleryItems.length > 1 ? (
              ...
              <div className="space-y-4">
                <div className="flex justify-center w-full relative group">
                  <AssetImage src={src} alt={alt} className="max-h-[500px] w-auto rounded-lg object-contain" />
