...
export const ResetPasswordForm: FunctionComponent<ResetPasswordFormProps> = (): React.JSX.Element => {
  return (
    <AuthForm>
      <AuthFormHeader title="Reset Password" description="Enter your details to reset your password"></AuthFormHeader>
...
export const AuthFormHeader: FunctionComponent<AuthFormHeaderProps> = ({ title, description, children }): React.JSX.Element => {
  return (
    <CardHeader className="pb-0 pt-8">
      <CardTitle className="text-3xl font-bold text-center text-slate-800">{title}</CardTitle>
      <CardDescription className="text-center text-slate-500 mt-2 text-base">{description}</CardDescription>
      {children}
    </CardHeader>
  );
};
...
export const AuthFormContent: FunctionComponent<PropsWithChildren> = ({ children }): React.JSX.Element => {
  return <CardContent className="px-8 pt-8 pb-4">{children}</CardContent>;
};

export const AuthFormFooter: FunctionComponent<PropsWithChildren> = ({ children }): React.JSX.Element => {
  return <CardFooter className="flex flex-col space-y-6 px-8 pt-2 pb-8">{children}</CardFooter>;
};
