...
import type { ParseProgress, PresentationData, Slide, SlideTheme, TransitionEffect } from "./presentation/types"
...
function PresentationComponent({ data, nodeKey }: PresentationComponentProps) {
  ...
  const handleImportImages = async (results: MediaUploadResult | MediaUploadResult[]) => {
    ...
    try {
      const imageSlides: Slide[] = resultsArray.map((result, index) => ({
