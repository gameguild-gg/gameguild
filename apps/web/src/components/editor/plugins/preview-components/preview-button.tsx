...
export function PreviewButton({ node }: { node: SerializedButtonNode }) {
  ...
  const getButtonStyles = () => {
    ...
    return `${BASE_BUTTON_STYLES} cursor-pointer ${getSizeStyles(size, isVerticalIcon)} ${getVariantBaseStyles(variant, size)} ${getColorStyles()} ${getLayoutStyles(iconPosition || "right")}`
  }
