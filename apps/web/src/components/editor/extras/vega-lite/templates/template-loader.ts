...
export const TEMPLATE_CATEGORIES: TemplateCategory[] = [
  {
    ...
    subcategories: [
      { id: "bar-charts", label: "Bar Charts", path: "single-view-plots/bar-charts" },
      { id: "line-charts", label: "Line Charts", path: "single-view-plots/line-charts" },
      { id: "histograms-density-plots", label: "Histograms, Density Plots & Dot Plots", path: "single-view-plots/histograms-density-plots" },
      { id: "scatter-strip-plots", label: "Scatter & Strip Plots", path: "single-view-plots/scatter-strip-plots" },
      { id: "area-streamgraphs", label: "Area Charts & Streamgraphs", path: "single-view-plots/area-streamgraphs" },
      { id: "table-plots", label: "Table-based Plots", path: "single-view-plots/table-plots" },
      { id: "circular-plots", label: "Circular Plots", path: "single-view-plots/circular-plots" },
      { id: "advanced-calculations", label: "Advanced Calculations", path: "single-view-plots/advanced-calculations" },
    ]
  },
  {
    id: "composite-marks",
    ...
    subcategories: [
      { id: "error-bars-bands", label: "Error Bars & Error Bands", path: "composite-marks/error-bars-bands" },
      { id: "box-plots", label: "Box Plots", path: "composite-marks/box-plots" },
    ]
  },
  {
    id: "layered-plots",
    label: "Layered Plots",
    subcategories: [
      { id: "labeling-annotation", label: "Labeling & Annotation", path: "layered-plots/labeling-annotation" },
      { id: "other-layered", label: "Other Layered Plots", path: "layered-plots/other-layered" },
    ]
  },
  {
    id: "multi-view",
    ...
    subcategories: [
      { id: "faceting", label: "Faceting (Trellis Plot / Small Multiples)", path: "multi-view/faceting" },
      { id: "repeat-concat", label: "Repeat & Concatenation", path: "multi-view/repeat-concat" },
    ]
  },
  {
    id: "geographic",
    ...
    subcategories: [
      { id: "maps", label: "Maps", path: "geographic" },
    ]
  }
]
...
async function loadTemplatesFromFolder(): Promise<VegaLiteTemplate[]> {
  ...
  try {
    ...
    const groupedBar = await import("./single-view-plots/bar-charts/grouped-bar")
    ...
    const multiLine = await import("./single-view-plots/line-charts/multi-line")
    templates.push(multiLine.default)

    // Single-View Plots - Scatter & Strip Plots
    const scatterPlot = await import("./single-view-plots/scatter-strip-plots/scatter-plot")
    templates.push(scatterPlot.default)

    // Single-View Plots - Area Charts & Streamgraphs
    const areaChart = await import("./single-view-plots/area-streamgraphs/area-chart")
    templates.push(areaChart.default)

    const stackedArea = await import("./single-view-plots/area-streamgraphs/stacked-area")
    templates.push(stackedArea.default)

    // Single-View Plots - Histograms, Density Plots
    const histogram = await import("./single-view-plots/histograms-density-plots/histogram")
    templates.push(histogram.default)

    // Single-View Plots - Circular Plots
    const pieChart = await import("./single-view-plots/circular-plots/pie-chart")
    templates.push(pieChart.default)

    // Single-View Plots - Table-based Plots
    const textTable = await import("./single-view-plots/table-plots/text-table")
    templates.push(textTable.default)

    // Single-View Plots - Advanced Calculations
    const heatmap = await import("./single-view-plots/advanced-calculations/heatmap")
    ...
    const errorBars = await import("./composite-marks/error-bars-bands/error-bars")
    templates.push(errorBars.default)

    // Layered Plots - Labeling & Annotation
    const annotatedLine = await import("./layered-plots/labeling-annotation/annotated-line")
 ...
 */
export async function getTemplatesByCategory(categoryId: string): Promise<VegaLiteTemplate[]> {
 ...
 */
export async function getTemplatesBySubcategory(subcategoryId: string): Promise<VegaLiteTemplate[]> {
  const allTemplates = await getAllTemplates()
  return allTemplates.filter(template => template.subcategory === subcategoryId)
}

/**
 * Get a single template by ID
 */
export async function getTemplateById(templateId: string): Promise<VegaLiteTemplate | null> {
  const allTemplates = await getAllTemplates()
 ...
 */
export async function searchTemplates(keyword: string): Promise<VegaLiteTemplate[]> {
  const allTemplates = await getAllTemplates()
