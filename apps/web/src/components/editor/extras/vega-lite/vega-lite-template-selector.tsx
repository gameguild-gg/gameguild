...
export function VegaLiteTemplateSelector({ onSelect, onCancel }: VegaLiteTemplateSelectorProps) {
  ...
  const [filteredTemplates, setFilteredTemplates] = useState<VegaLiteTemplate[]>([])
            ...
            {filteredTemplates.map((template) => {
              ...
              if (template.previewImage) {
                try {
                  ...
                  previewImageSrc = require(`./templates/${template.category}/${template.subcategory}/${template.previewImage}`).default?.src || 
                                   require(`./templates/${template.category}/${template.subcategory}/${template.previewImage}`)
                } catch (e) {
                  console.warn(`Preview image not found for template ${template.id}:`, template.previewImage)
                }
              }
                ...
                >
                  ...
                  {previewImageSrc ? (
                    <div className="w-full aspect-square bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                      <img 
                        src={typeof previewImageSrc === 'string' ? previewImageSrc : previewImageSrc.src || previewImageSrc} 
                        alt={template.title}
