...
export function MatchingQuestion({ question, pairs, userMatches, onMatchChange, disabled = false }: MatchingQuestionProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-8">
        ...
        <div className="space-y-2">
          ...
          {rightItems.map((item) => (
            <Button key={item} variant="outline" onClick={() => handleRightClick(item)} disabled={disabled || !selectedLeft} className="w-full justify-start">
              {item}
            </Button>
          ))}
        </div>
