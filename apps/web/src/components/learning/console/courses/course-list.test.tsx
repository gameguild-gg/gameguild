...
vi.mock("@/lib/learning/course-route", () => ({
  buildDashboardCoursePath: (
    course: string | { id: string; routeParam?: string },
    path = "",
  ) => {
    const id =
      typeof course === "string" ? course : course.routeParam || course.id;
...
describe("CourseList", () => {
  it("renders grid cards and filters by search and status", async () => {
    ...
    await user.click(
      screen.getByRole("combobox", { name: /course status filter/i }),
    );
  ...
  it("switches to table view, sorts rows, and exposes row actions", async () => {
    ...
    expect(
      within(ascendingRows[0]).getByText("Cinematic Lighting"),
    ).toBeInTheDocument();

    await user.click(
      screen.getByRole("button", { name: /open boss ai actions/i }),
    );
    expect(
      await screen.findByRole("menuitem", { name: /^edit$/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("menuitem", { name: /preview/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("menuitem", { name: /manage lifecycle/i }),
    ).toBeInTheDocument();
  });
  ...
  it("covers table fallbacks, alternate sorts, and grid view toggle", () => {
    ...
    render(
      <CourseList
        courses={[...courses, courseWithUnknownStatus]}
        locale="en-US"
      />,
    );
    ...
    expect(screen.getByText("All Courses")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /unlisted prototype/i }),
    ).toHaveAttribute("href", "/workspace/learning/courses/course-4");
  ...
  it("renders the empty state for no courses and no filtered results", async () => {
    ...
    expect(
      screen.getByText("Create your first course to start teaching."),
    ).toBeInTheDocument();
