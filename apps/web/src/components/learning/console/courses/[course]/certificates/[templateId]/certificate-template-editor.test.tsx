...
describe("CertificateTemplateEditor", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.updateCertificateTemplate.mockResolvedValue({
      success: true,
      data: null,
    });
  });
  ...
  it("edits template fields and persists default and active state", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Template HTML"), {
      target: { value: "<main>{{recipientName}}</main>" },
    });
    fireEvent.change(screen.getByLabelText("Template styles"), {
      target: { value: "main { color: navy; }" },
    });
    await user.click(screen.getByRole("switch", { name: "Default template" }));
    await user.click(
      screen.getByRole("button", { name: "Save certificate template" }),
    );
    ...
    expect(screen.getByTitle("Certificate preview")).toHaveAttribute(
      "sandbox",
      "",
    );
  }, 15_000);

  it("shows the update error returned by the server", async () => {
    const user = userEvent.setup();
    mocks.updateCertificateTemplate.mockResolvedValue({
      success: false,
    ...
    render(
      <CertificateTemplateEditor
        template={{
          ...
          createdAt: "2026-07-01T00:00:00.000Z",
          updatedAt: "2026-07-01T00:00:00.000Z",
          previewUrl: "/api/certificates/templates/template-2",
          templateHtml: "<main>Draft</main>",
          templateStyles: "main { color: black; }",
        }}
      />,
    );
    ...
    expect(
      screen.getByRole("switch", { name: "Active template" }),
    ).not.toBeChecked();
  });
});
