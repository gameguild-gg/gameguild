...
vi.mock('@/lib/coding-assignment/code-payload', () => ({ filesToCodePayload: mocks.filesToPayload }));
vi.mock('@/lib/learner/activity-actions', () => ({ submitAssessment: mocks.submit }));
vi.mock('@game-guild/emception-ui/assessment/editor', () => ({ CodingAssessmentEditor: mocks.editor }));
...
vi.mock('@game-guild/emception-ui/assessment/storage', () => ({ workspaceStorageKey: mocks.storageKey }));
...
function renderActivity(overrides: Partial<ComponentProps<typeof CodingActivityClient>> = {}) {
  return render(
...
describe('CodingActivityClient', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.seedFiles.mockReturnValue([{ path: 'main.cpp', content: '// seed', encoding: 'text' }]);
    mocks.buildPlan.mockReturnValue({ plan: { cases: [] } });
    mocks.createWorkspace.mockImplementation((language, files) => ({ id: `workspace-${language}`, files }));
    mocks.storageKey.mockImplementation((token, workspaceId) => `${token}:${workspaceId}`);
  ...
  it('loads the browser editor and builds the default public workspace', async () => {
    ...
    expect(mocks.storageKey).toHaveBeenCalledWith('assessment-1', 'workspace-cpp');
    expect(props).toMatchObject({ mode: 'learner', maxScore: 100, passingScore: 60 });
  ...
  it('overlays prior submissions and namespaces local drafts by user', async () => {
    ...
    expect(mocks.storageKey).toHaveBeenCalledWith('user-1:assessment-1', 'workspace-rust');
  ...
  it('renders the public estimate after an editor run', async () => {
    ...
    expect(mocks.banner).toHaveBeenCalledWith(expect.objectContaining({
      report,
      plan: { cases: [] },
      maxScore: 100,
      passingScore: 60,
  ...
  it('submits a session delta once and navigates after success', async () => {
    ...
    renderActivity();
    const props = await editorProps();
    const delta = [{ path: 'main.cpp', content: 'int main(){}', encoding: 'text' }];
    ...
    expect(await screen.findByRole('status')).toHaveTextContent('Submission received');
    expect(session.getSubmissionDelta).toHaveBeenCalledOnce();
    expect(mocks.filesToPayload).toHaveBeenCalledWith(delta);
    const [initialState, formData] = mocks.submit.mock.calls[0] as [unknown, FormData];
    ...
    expect(mocks.push).toHaveBeenCalledWith('/learn/courses/game-ai/activities');
  });

  it('submits an empty delta when the session returns no value', async () => {
    mocks.submit.mockResolvedValue({ success: false, error: 'Submission rejected.' });
    renderActivity();
    const props = await editorProps();
  ...
  ])('shows unexpected submission failures without trapping the form', async (error, expected) => {
