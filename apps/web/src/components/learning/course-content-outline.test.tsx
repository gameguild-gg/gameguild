...
describe("CourseContentOutline", () => {
  it("uses native App Router paths for lessons and activities", () => {
    ...
    expect(
      screen.getByRole("link", { name: /Discuss game loops/ }),
    ).toHaveAttribute(
      "href",
  ...
  it("does not expose a navigation link for locked content", () => {
    ...
    expect(
      screen.queryByRole("link", { name: /Advanced agents/ }),
    ).not.toBeInTheDocument();
  });

  it("links completed non-participatory activities and shows their duration", () => {
    render(
    ...
    const activity = screen.getByRole("link", { name: /Ship a prototype/ });
    expect(activity).toHaveAttribute(
      "href",
      "/learn/courses/game-ai/activities",
    );
    expect(activity).toHaveTextContent("activity · 45 min");
  });
});
