        ...
        <section aria-labelledby="course-access-title" className="mx-auto flex min-h-[70vh] max-w-3xl items-center px-4 py-12">
            <Card className="w-full">
                ...
                <CardContent className="space-y-4">
                    ...
                    {access.kind === 'enrollment-closed' ? (
                        <p className="text-sm text-muted-foreground">This cohort is not accepting new learners. Check the catalog for the next offering.</p>
                    ) : null}
                    {access.kind === 'unavailable' ? (
                        <div className="flex gap-2 text-sm text-amber-600"><AlertCircle className="size-4 shrink-0" />{access.message}</div>
                    ) : null}
                    {message ? <p role="status" className={message === 'Enrollment confirmed' ? 'text-emerald-600' : 'text-destructive'}>{message}</p> : null}
