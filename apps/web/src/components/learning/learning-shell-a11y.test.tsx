...
describe("LearningShell accessibility and search", () => {
  ...
  it("provides a focusable content target and announces unread notifications", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Skip to learning content" }),
    ).toHaveAttribute("href", "#learning-content");
    expect(document.querySelector("#learning-content")).toHaveAttribute(
      "tabindex",
      "-1",
    );
  ...
  it("searches only the learner workspace and navigates to a returned lesson", async () => {
    ...
    await user.click(
      screen.getAllByRole("button", { name: /Search learning/ })[0],
    );
    await user.type(
      screen.getByPlaceholderText("Search your courses and lessons..."),
      "navigation",
    );
