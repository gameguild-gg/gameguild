...
describe('course editor actions', () => {
  ...
  it('saves editor changes through PUT /v1/courses/{id}', async () => {
    ...
    expect(mocks.revalidatePath).toHaveBeenCalledWith('/dashboard/platform/learning/courses/course-1');
  });
  ...
  it('publishes through the generated lifecycle API', async () => {
    ...
    expect(mocks.revalidatePath).toHaveBeenCalledWith('/dashboard/platform/learning/courses/course-1');
    expect(mocks.revalidatePath).toHaveBeenCalledWith('/dashboard/platform/learning/courses');
  });
});
