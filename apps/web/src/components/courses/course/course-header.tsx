...
} from "@/lib/courses/services/course.service";
import {
  getCourseShowcase,
  getProgramForCourse,
...
function getPrimaryCta(
  ...
  label: string;
  href?: string;
  kind: "link" | "checkout" | "enroll" | "disabled";
} {
  const signInHref = courseSlug
    ? `/sign-in?redirectTo=${encodeURIComponent(`/courses/${courseSlug}`)}`
    : "/sign-in";

  if (viewerAccess?.state === "has-access" && courseSlug) {
    return {
      label: "Continue learning",
      href: getLearnerCourseContentHref(courseSlug),
      kind: "link",
    };
  }
...
export function CourseHeader({
  ...
  const thumbnailSrc =
    typeof course.thumbnail === "string" && course.thumbnail.length > 0
      ? course.thumbnail
      : null;
  const courseTitle =
  ...
  const { name: levelName } = getCourseLevelConfig(
    course.difficulty as string | number | null | undefined,
  );
  const categoryName = getCourseCategoryName(
    course.category as string | number | null | undefined,
  );
        ...
        <div className="grid flex-1 gap-12 py-16 lg:grid-cols-[1fr_420px] lg:items-end">
          <div className="flex max-w-4xl flex-col gap-8">
            <div className="flex flex-wrap gap-2">
              <Badge
                variant="outline"
                className="border-white/20 bg-black/35 text-white backdrop-blur"
              >
                {categoryName}
              </Badge>
              <Badge
            ...
            <div className="flex flex-col gap-6">
              ...
              <p className="max-w-3xl text-lg leading-8 text-slate-300 md:text-xl">
                {courseDescription || showcase?.headline}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-5">
              <div>
                ...
                <p className="mt-3 text-sm leading-6 text-slate-300">
                  {showcase?.studioPrompt ||
                    "A practical GameGuild course with public catalog metadata, classroom content, and a focused project outcome."}
                </p>
              </div>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  ...
                  <p className="text-sm font-semibold">
                    {course.programContents?.length ?? 0}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
