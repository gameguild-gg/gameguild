...
import type { Program, ProgramDifficulty } from "@/lib/api/generated/types.gen";
...
export function CourseCard({ course, viewMode = "grid" }: { course: Program; viewMode?: "grid" | "list" }) {
  ...
  const status = course.status === 'ACTIVE' ? 'published' as const : 
                course.status === 'INACTIVE' ? 'archived' as const : 'draft' as const;
