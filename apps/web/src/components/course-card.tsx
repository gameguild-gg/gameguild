...
export function CourseCard({ course, viewMode = 'grid' }: { course: Course; viewMode?: 'grid' | 'list' }) {
  ...
  if (viewMode === 'list') {
    return (
      <Link href={`/dashboard/courses/${course.slug}`} className="absolute inset-0" aria-label={`Open ${course.title}`}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-6">
              <div className="relative w-32 h-20 bg-muted/20 rounded-lg overflow-hidden flex-shrink-0">
                <Image
                  src={course.thumbnailUrl || '/placeholder.svg?height=80&width=128&query=course%20cover'}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6 text-xs text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    <BookOpen className="w-3 h-3" />0 modules
                  </span>
  ...
  return (
    <Card className="group relative hover:shadow-xl transition-all duration-200 cursor-pointer">
      <CardHeader className="p-0">
        <div className="relative w-full h-48 bg-muted/20 rounded-t-lg overflow-hidden">
          <Image
            src={course.thumbnailUrl || '/placeholder.svg?height=192&width=384&query=course%20cover'}
            alt={`${course.title} cover`}
            fill
