...
// Update a project
export async function updateProject(id: string, data: Partial<{ title: string; description: string; shortDescription: string; websiteUrl: string; repositoryUrl: string; status: 'not-started' | 'in-progress' | 'active' | 'on-hold'; visibility: 'Private' | 'Public' | 'Restricted' }>) {
  try {
    ...
    if (data.repositoryUrl !== undefined) payload.RepositoryUrl = data.repositoryUrl;
    if (data.status !== undefined) payload.Status = mapDisplayToCmsStatus(data.status);
    if (data.visibility !== undefined) payload.Visibility = mapDisplayToAccessLevel(data.visibility);

    const response = await makeApiCall(`${CMS_API_URL}/api/projects/${id}`, {
      method: 'PUT',
...
export async function deleteProject(id: string) {
  try {
    const response = await makeApiCall(`${CMS_API_URL}/api/projects/${id}?softDelete=true`, { method: 'DELETE' });
...
export async function getProjectVersions(projectId: string): Promise<ProjectVersion[]> {
  const response = await makeApiCall(`${CMS_API_URL}/api/projects/${projectId}/versions`, { method: 'GET', cache: 'no-store' });
  if (!response.ok) throw new Error('Failed to load versions');
  return response.json();
}

export async function createProjectVersion(projectId: string, data: { versionNumber: string; releaseNotes?: string; status?: string }) {
  const response = await makeApiCall(`${CMS_API_URL}/api/projects/${projectId}/versions`, { method: 'POST', body: JSON.stringify({ VersionNumber: data.versionNumber, ReleaseNotes: data.releaseNotes, Status: data.status }) });
  if (!response.ok) throw new Error('Failed to create version');
  revalidatePath(`/dashboard/projects/${projectId}/versions`);
  return response.json();
}

export async function updateProjectVersion(projectId: string, versionId: string, data: { versionNumber?: string; releaseNotes?: string; status?: string }) {
  const body: Record<string, any> = {};
  if (data.versionNumber !== undefined) body.VersionNumber = data.versionNumber;
  if (data.releaseNotes !== undefined) body.ReleaseNotes = data.releaseNotes;
  if (data.status !== undefined) body.Status = data.status;
  const response = await makeApiCall(`${CMS_API_URL}/api/projects/${projectId}/versions/${versionId}`, { method: 'PUT', body: JSON.stringify(body) });
  if (!response.ok) throw new Error('Failed to update version');
  revalidatePath(`/dashboard/projects/${projectId}/versions`);
  return response.json();
}

export async function deleteProjectVersion(projectId: string, versionId: string) {
  const response = await makeApiCall(`${CMS_API_URL}/api/projects/${projectId}/versions/${versionId}?softDelete=true`, { method: 'DELETE' });
