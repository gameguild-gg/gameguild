...
export default function Header() {
  ...
  const toRoute = (href: string) => href as Route;
  ...
  return (
    <header className="bg-neutral-900 h-[70px] flex items-center justify-between px-4 text-white">
      <div className="flex items-center">
        <Link href={toRoute('/')} className="mr-4">
          <img src="/assets/images/logo-text.png" className="h-[40px] my-auto mx-[10px]" />
        </Link>
        <nav className="hidden md:flex space-x-2 h-[50px] p-0">
          {menuItems.map((item) => (
            <button key={item} onClick={() => router.push(toRoute(`/${item.toLowerCase()}`))} className="px-2 my-0 hover:text-gray-400 transition-colors h-full">
              {item}
            </button>
          ))}
          <DropdownMenu open={isMoreOpen} onOpenChange={setIsMoreOpen}>
            ...
            <DropdownMenuContent className="bg-neutral-900 text-white border-0">
              {moreItems.map((item) => (
                <Link key={item} href={toRoute(`/${item.toLowerCase().replace(/\s/g, '')}`)}>
            ...
            <DropdownMenuContent className="bg-neutral-900 text-white border-0">
              {complexItems.map((item) => (
                <DropdownMenuItem key={item.name}>
                  <Link href={toRoute(item.href)} className=" hover:text-gray-400 transition-colors">
                    {item.name}
                  </Link>
                </DropdownMenuItem>
              ))}
        ...
        {user ? (
          <div className="flex items-center space-x-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => router.push(toRoute('/profile/edit'))}>Profile</DropdownMenuItem>
                <DropdownMenuItem>Settings</DropdownMenuItem>
                <DropdownMenuItem
                  onClick={
                    // redirect to /disconnect
                    async () => {
                      router.push(toRoute('/disconnect'));
                    }
                  }
                >
                  Logout
          ...
          <Button onClick={() => router.push(toRoute('/connect'))} variant="outline" className="bg-white text-black font-semibold text-base rounded-md hover:bg-gray-200">
            Connect
          </Button>
        )}
      </div>
        ...
        <div className="absolute top-[70px] left-0 right-0 bg-neutral-900 p-4 md:hidden z-30">
          <div className="flex flex-col space-y-4">
            ...
            {user ? (
              ...
              <Button onClick={() => router.push(toRoute('/connect'))} variant="outline" className="bg-white text-black font-semibold rounded-md hover:bg-gray-200 w-full">
                Connect
              </Button>
            )}
            {menuItems.map((item) => (
              <Link key={item} href={toRoute(`/${item.toLowerCase()}`)} className="hover:text-gray-400 transition-colors">
                {item}
              </Link>
            ))}
            <DropdownMenu>
