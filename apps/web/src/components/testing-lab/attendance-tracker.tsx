...
export function AttendanceTracker() {
  ...
  const fetchAttendanceData = async () => {
    ...
    const exportAttendanceReport = () => {
      ...
      const csvContent = [
        [ 'Student Name', 'Student ID', 'Email', 'Sessions Attended', 'Sessions Scheduled', 'Attendance Rate', 'Games Tested', 'Feedback Submitted', 'On Track' ].join(','),
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Monitor student attendance and testing progress
            </p>
          </div>
                ...
                <TableBody>
                  { filteredRecords.map((record) => (
                    <TableRow key={ record.id } className="border-slate-700">
                      ...
                      <TableCell>
                        <Badge className={ getAttendanceStatusColor(record.attendanceStatus) }>
                      ...
                      <TableCell className="text-slate-300">
                        { record.checkInTime && record.checkOutTime ? (
                          <div className="text-sm">
                            <div>In: { new Date(record.checkInTime).toLocaleTimeString('en-US', {
                              hour: '2-digit',
                              minute: '2-digit',
                            }) }</div>
                            <div>Out: { new Date(record.checkOutTime).toLocaleTimeString('en-US', {
                              hour: '2-digit',
                              minute: '2-digit',
                            }) }</div>
                          </div>
          ...
          <Card className="bg-slate-800/50 border-slate-700/50 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                Track individual student progress and completion requirements
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                    ...
                    <TableRow key={ progress.user.id } className="border-slate-700">
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          ...
                          <div className="text-sm">
                            <Badge
                              className={ progress.attendanceRate >= 75 ? 'bg-green-600/20 text-green-400' : 'bg-red-600/20 text-red-400' }>
                      ...
                      <TableCell className="text-white text-center">
                        ...
                        <div className="text-xs text-slate-400">
                          { progress.totalFeedbackSubmitted >= progress.totalGamesTermd ? '✓' : 'Incomplete' }
                        </div>
                      </TableCell>
