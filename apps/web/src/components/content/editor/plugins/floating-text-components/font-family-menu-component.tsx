...
export function FontFamilyMenuComponent({ editor, currentFontFamily }: FontFamilyMenuComponentProps) {
  ...
  useEffect(() => {
    ...
    const matchedFont = ALL_FONT_FAMILIES_FLAT.find((font) => currentFontFamily.toLowerCase().includes(font.name.toLowerCase()));
    if (matchedFont) {
  ...
  const getCurrentFontDisplay = () => {
    ...
    const matchedFont = ALL_FONT_FAMILIES_FLAT.find((font) => displayedFontFamily.toLowerCase().includes(font.name.toLowerCase()));

    let displayString = matchedFont ? matchedFont.name : 'Custom';

    if (matchedFont) {
      const matchedVariation = matchedFont.variations.find((v) => v.weight === displayedFontWeight && v.style === displayedFontStyle);
