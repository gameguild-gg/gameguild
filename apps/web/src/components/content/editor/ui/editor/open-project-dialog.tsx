        ...
        <div className="space-y-4 py-3">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-gray-500 mt-2 mb-3">
              Você está prestes a excluir <span className="font-semibold">{projectToDelete?.name}</span>. Esta ação não pode ser desfeita.
            </p>
          </div>
            ...
            <div className="space-y-3 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <div className="flex gap-3">
                <div className="flex-1">
                  <Input placeholder="Search projects by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full" />
                </div>
                          ...
                          <div key={project.id} className="flex items-center justify-between p-3 border dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                            <div className="flex flex-col flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {project.tags && project.tags.length > 0 && (
                                  <div className="flex gap-1">
                                    ...
                                    {project.tags.length > 3 && <span className="text-xs text-gray-500 dark:text-gray-400">+{project.tags.length - 3}</span>}
                                  </div>
                                )}
                              </div>
                              <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                                ...
                                <span>•</span>
                                <span className="text-gray-400 dark:text-gray-500 font-mono text-xs">ID: {project.id.slice(0, 8)}...</span>
                              </div>
                            </div>
              ...
              <div className="flex justify-between items-center pt-4 border-t dark:border-gray-700">
                {isStorageNearLimit() ? (
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-red-600 dark:text-red-400 mt-1">Armazenamento quase cheio ({getStorageUsagePercentage().toFixed(1)}%)</span>
                  </div>
                ) : (
                  <Button
                    variant="ghost"
                ...
                <Button variant="outline" onClick={() => onOpenChange(false)} disabled={!currentProjectName} className="bg-transparent">
                  Fechar
                </Button>
              </div>
            </div>
