    ...
    <div className={`flex flex-col gap-2 ${className}`}>
      <div className="flex items-center justify-between gap-2">
        {sizePresets.map((preset) => (
          <Button key={preset.name} variant={size === preset.size ? 'secondary' : 'ghost'} size="sm" className="flex-1" onClick={() => onChange(preset.size)}>
            {preset.name}
          </Button>
        ))}
      </div>
      <div className="flex items-center gap-2">
        ...
        <Slider value={[size]} min={10} max={100} step={1} className="flex-1" onValueChange={(values) => onChange(values[0])} />
