...
export async function proxy(request: NextRequest): Promise<NextResponse> {
  ...
  const hostDecision =
    process.env.UNIFIED_LEARNING_ENABLED === "true" && isLearningPath
      ? resolveLearningHostRoute({
          ...
          requiresAuthentication: !hasGameGuildSessionCookie(
  ...
  if (hostDecision.action === "redirect") {
    ...
    const resolvedRewrite = new URL(hostDecision.url);
    const rewriteUrl = request.nextUrl.clone();
    rewriteUrl.pathname = resolvedRewrite.pathname;
    rewriteUrl.search = resolvedRewrite.search;
    ...
    requestHeaders.set("x-gameguild-visible-url", visibleUrl.toString());
