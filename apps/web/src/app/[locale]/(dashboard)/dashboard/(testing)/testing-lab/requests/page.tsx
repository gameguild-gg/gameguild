...
function pageHref(page: number, q: string, status: string) {
  ...
  return `/dashboard/testing-lab/requests?${params}`;
}

export default async function TestingLabRequestsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <main className="space-y-6 p-4 lg:p-6">
      <TestingLabPageHeader
        ...
        description="Manage project build intake, testing objectives, participant capacity, feedback briefs, and request lifecycle."
      ...
      {rows.length === 0 ? (
        <TestingLabEmptyState
          title={directory.requests.length === 0 ? 'No testing requests yet' : 'No requests match these filters'}
          description={
            directory.requests.length === 0
              ? 'Submit a real project build to create the first testing request.'
                  ...
                  return (
                    <tr key={request.id} className="hover:bg-muted/25">
                      ...
                      <td className="max-w-80 px-4 py-3">
                        <Link href={`/dashboard/testing-lab/requests/${request.id}`} className="font-medium hover:underline">
                          {request.title}
                        </Link>
          ...
          <div className="flex items-center justify-between border-t px-4 py-3 text-sm">
            ...
            <div className="flex gap-2">
              <Button asChild size="sm" variant="outline" disabled={page <= 1}>
                <Link href={pageHref(page - 1, q, status)}>Previous</Link>
              </Button>
