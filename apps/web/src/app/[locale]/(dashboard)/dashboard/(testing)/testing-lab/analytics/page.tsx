    ...
    <div className="overflow-hidden rounded-md border">
      ...
      <div className="divide-y">
        {events.map((event) => (
          <Link
            key={event.eventId}
            href={`/dashboard/testing-lab/events/${event.eventId}/overview`}
    ...
    <div className="space-y-6 p-4 lg:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex gap-1" aria-label="Preset periods">
            {[7, 30, 90].map((days) => (
              ...
              >
                <Link href={`/dashboard/testing-lab/analytics?range=${days}`}>
                  {days} days
                </Link>
              </Button>
            ))}
      ...
      {!hasActivity && analytics.accessIssues.length === 0 ? (
        <TestingLabEmptyState
          ...
          action={
            <Button asChild>
              <Link href="/dashboard/testing-lab/events">Manage events</Link>
            </Button>
          }
        />
      ) : (
