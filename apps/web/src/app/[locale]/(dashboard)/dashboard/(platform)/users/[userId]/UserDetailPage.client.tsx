...
export default function UserDetailPageClient({ userId }: { userId: string }) {
    const router = useRouter();

    const { user: apiUser, loading, error } = useUserDetail(UserIdentifierType.ID, userId);
    const [user, setUser] = useState(apiUser as any);
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                {editingBasicInfo ? (
                                    <Input id="name" value={basicForm.name} onChange={(e) => setBasicForm({ ...basicForm, name: e.target.value })} />
                                ) : (
                            ...
                            <div>
                                ...
                                {editingBasicInfo ? (
                                    <Input id="username" value={basicForm.username} onChange={(e) => setBasicForm({ ...basicForm, username: e.target.value })} />
                                ) : (
                            ...
                            <div className="md:col-span-2">
                                ...
                                {editingBasicInfo ? (
                                    <Input id="email" type="email" value={basicForm.email} onChange={(e) => setBasicForm({ ...basicForm, email: e.target.value })} />
                                ) : (
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <Label>Role</Label>
                                <div className="mt-1 text-sm">{user.role || 'No role assigned'}</div>
                            </div>

                            <div>
                                <Label>Version</Label>
                                <div className="mt-1 text-sm font-mono">v{user.version || 0}</div>
                            </div>
                        </div>
                        ...
                        {user.deletedAt && (
                            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                                ...
                                <div className="text-sm text-red-700">This account was marked for deletion on {formatDate(user.deletedAt)}</div>
                            </div>
                        )}
                    </CardContent>
                </Card>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="space-y-2">
                                <div>
                                    <Label>Type</Label>
                                    <div className="mt-1 text-sm">{user.subscriptionType || 'Not specified'}</div>
                                </div>
                                <div className="text-sm text-gray-600">Additional subscription details would be available here based on the UserSubscriptionSummaryDto structure.</div>
                            </div>
                        </CardContent>
                    </Card>
                )}
