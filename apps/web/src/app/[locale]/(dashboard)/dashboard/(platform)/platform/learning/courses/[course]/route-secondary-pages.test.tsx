...
describe("course-management secondary route pages", () => {
  ...
  it("redirects index routes to their canonical course-management tabs", async () => {
    await expect(
      ...
      "redirect:/en-US/dashboard/platform/learning/courses/advanced-ai-by-gameguild/overview",
    );
    await expect(
      SupportRedirectPage({ params: params() } as never),
    ).rejects.toThrow(
      "redirect:/en-US/dashboard/platform/learning/courses/advanced-ai-by-gameguild/support/tickets",
    );
    await expect(
      SettingsRedirectPage({ params: params() } as never),
    ).rejects.toThrow(
      "redirect:/en-US/dashboard/platform/learning/courses/advanced-ai-by-gameguild/settings/danger",
    );
    await expect(
      GeneralSettingsPage({ params: params() } as never),
    ).rejects.toThrow(
      "redirect:/en-US/dashboard/platform/learning/courses/course-1/listing/info",
    );
  });
  ...
  it("renders support queues, ticket details, discussions, and discussion details", async () => {
    ...
    expect(
      ...
      "href",
      "/dashboard/platform/learning/courses/course-1/support/tickets/ticket-1",
    );
