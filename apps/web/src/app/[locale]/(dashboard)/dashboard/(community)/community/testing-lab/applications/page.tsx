...
function pageHref(page: number, q: string, status?: TestingLabTestingApplicationStatus) {
  ...
  return `/dashboard/community/testing-lab/applications?${params}`;
}
    ...
    <div className="space-y-6 p-4 lg:p-6">
      ...
      <nav aria-label="Application status" className="flex flex-wrap gap-2">
        {statuses.map((candidate) => (
          ...
          >
            <Link href={candidate.value ? `/dashboard/community/testing-lab/applications?status=${candidate.value}` : '/dashboard/community/testing-lab/applications'}>
              {candidate.label}
            </Link>
          </Button>
        ))}
      ...
      {entries.length === 0 ? (
        ...
        <section className="divide-y rounded-md border" aria-label="Project application directory">
          {entries.map(({ event, application }) => {
            ...
            const reviewHref = `/dashboard/community/testing-lab/events/${event.id}/applications${
