...
export default async function TestingLabSessionsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  function href(nextPage: number) {
    ...
    return `/dashboard/community/testing-lab/sessions?${next}`;
  }
                ...
                {rows.map((session) => (
                  <tr key={session.id} className="hover:bg-muted/25">
                    <td className="px-4 py-3">
                      <Link href={`/dashboard/community/testing-lab/sessions/${session.id}`} className="font-medium hover:underline">
                        {session.sessionName}
                      </Link>
                    </td>
