    ...
    <div className="space-y-6 p-4 lg:p-6">
      ...
      <nav aria-label="Filter testing events" className="flex flex-wrap gap-2">
        {statuses.map((status) => {
          ...
          return (
            <Button key={status.label} asChild size="sm" variant={active ? 'default' : 'outline'}>
              <Link href={status.value ? `/dashboard/community/testing-lab/events?status=${status.value}` : '/dashboard/community/testing-lab/events'}>
                {status.label}
              </Link>
            </Button>
          );
        ...
        <Button asChild size="sm" variant={archived ? 'default' : 'outline'}>
          <Link href="/dashboard/community/testing-lab/events?archived=true">Archived</Link>
        </Button>
      </nav>
              ...
              {archived ? (
                <RestoreTestingEventDialog event={event} />
                  ...
                  <Button asChild variant="outline">
                    <Link href={`/dashboard/community/testing-lab/events/${event.id}`}>
      ...
      {pageCount > 1 ? (
        <nav aria-label="Testing event pages" className="flex items-center justify-end gap-2">
          <Button asChild size="sm" variant="outline" disabled={page <= 1}>
            <Link href={`/dashboard/community/testing-lab/events?page=${Math.max(1, page - 1)}${querySuffix}`}>Previous</Link>
          </Button>
          ...
          <Button asChild size="sm" variant="outline" disabled={page >= pageCount}>
            <Link href={`/dashboard/community/testing-lab/events?page=${Math.min(pageCount, page + 1)}${querySuffix}`}>Next</Link>
          </Button>
        </nav>
      ) : null}
    </div>
