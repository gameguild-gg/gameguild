      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button asChild variant="outline">
            <Link href={`/dashboard/learning/courses/${courseId}/classes/calendar`}>
      ...
      {visible.length === 0 ? (
        ...
        <>
                ...
                {visible.map((cohort) => (
                  <TableRow key={cohort.id}>
                    <TableCell className="max-w-72 whitespace-normal">
                      <Link href={`/dashboard/learning/courses/${courseId}/classes/${cohort.id}/schedule`} className="font-medium hover:underline">
                        {cohort.name}
                      </Link>
                    </TableCell>
          ...
          <div className="grid gap-3 md:hidden">
            {visible.map((cohort) => (
              <Link key={cohort.id} href={`/dashboard/learning/courses/${courseId}/classes/${cohort.id}/schedule`} className="rounded-lg border p-4 transition-colors hover:bg-muted/40">
