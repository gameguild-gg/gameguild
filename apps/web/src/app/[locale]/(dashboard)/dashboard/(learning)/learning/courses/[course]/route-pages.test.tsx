...
vi.mock('@/lib/learning/course-route', () => ({
  buildDashboardCoursePath: (course: string, path = '') => `/dashboard/learning/courses/${course}${path ? `/${path}` : ''}`,
...
import CourseRouteLayout from './layout';
import CourseRouteRedirectPage from './page';
...
describe('course-management dashboard route pages', () => {
  ...
  it('redirects the course root to the canonical overview slug route', async () => {
    await expect(CourseRouteRedirectPage({ params: params('course-1') } as never)).rejects.toThrow(
      'redirect:/en-US/dashboard/learning/courses/advanced-game-ai-by-gameguild/overview',
    );
  });
