...
export function CourseCard({ course, locale }: CourseCardProps) {
  return (
    <Card className="flex h-full flex-col overflow-hidden transition-shadow hover:shadow-lg">
      <Link href={`/${locale}/dashboard/learning/courses/${course.id}`} className="relative aspect-video bg-muted">
      ...
      <CardHeader className="pb-2">
        ...
        <CardTitle className="line-clamp-2 text-lg">
          <Link href={`/${locale}/dashboard/learning/courses/${course.id}`}>{course.title}</Link>
        </CardTitle>
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem asChild>
          <Link href={`/${locale}/dashboard/learning/courses/${courseId}`}>
            <Edit className="mr-2 size-4" />
            Edit
          </Link>
        </DropdownMenuItem>
