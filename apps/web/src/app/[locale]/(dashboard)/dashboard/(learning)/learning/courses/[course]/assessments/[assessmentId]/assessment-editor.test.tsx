...
describe('AssessmentEditor', () => {
  ...
  it('validates the title before updating assessment settings', async () => {
    ...
    render(<AssessmentEditor courseId="course-1" assessment={assessment} assessmentGroups={groups} />);
  ...
  it('saves details, scoring, availability, and weighted group assignment', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/description/i), { target: { value: 'Updated instructions.' } });
    fireEvent.change(screen.getByLabelText(/max score/i), { target: { value: '20' } });
    fireEvent.change(screen.getByLabelText(/passing score/i), { target: { value: '14' } });
    fireEvent.change(screen.getByLabelText(/available from/i), { target: { value: '2026-08-01T10:00' } });
  ...
  it('shows API errors and deletes after explicit confirmation', async () => {
    const user = userEvent.setup();
    vi.mocked(updateAssessment).mockResolvedValueOnce({ success: false, error: 'Bad Request' });
