        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {events.length === 0 ? <p className="text-slate-400">No public Launch Pad event is available.</p> : events.map((event) => (
            <Card key={event.id} className="border-white/10 bg-white/[0.04] text-white">
              ...
              <CardContent className="space-y-4"><p className="text-sm text-slate-400">{event.description || 'Community launch event'}</p><p className="flex items-center gap-2 text-sm"><CalendarDays className="size-4" />{new Date(event.startsAt).toLocaleString()}</p><Button nativeButton={false} render={<Link href={projectId ? `/launch-pad/events/${event.id}?projectId=${encodeURIComponent(projectId)}` : `/launch-pad/events/${event.id}`} />}>View event</Button></CardContent>
            </Card>
          ))}
        </section>
      </div>
