...
export default async function SupportTicketsPage({
  params,
}: PageProps<'/[locale]/workspace/learning/courses/[course]/support/tickets'>): Promise<React.JSX.Element> {
        ...
        <CardContent className="space-y-3">
          {tickets.tickets.length === 0 ? (
            ...
            tickets.tickets.map((ticket) => (
              <Link key={ticket.id} href={`/workspace/learning/courses/${courseId}/support/tickets/${ticket.id}`} className="flex items-center justify-between rounded-lg border p-4 transition-colors hover:bg-muted/50">
                <div>
