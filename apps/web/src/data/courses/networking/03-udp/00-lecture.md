...
#### IPv4 vs IPv6: Why is the Checksum Optional/Mandatory?
...
IPv6 removed the IP header checksum for performance (routers had to recalculate it at every hop because TTL changes). Without an IP-layer checksum, the transport layer **must** provide integrity checking—so UDP checksum became mandatory. An all-zeros checksum in IPv6 causes the packet to be dropped.
