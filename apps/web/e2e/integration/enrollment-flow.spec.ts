...
test.describe('Course Enrollment Flow - Frontend to API Integration', () => {
  ...
  test('should handle course access levels correctly', async () => {
    ...
    for (let i = 0; i < Math.min(3, courseCount); i++) {
      ...
      if (await sidebar.isVisible()) {
        ...
        const hasAccessInfo =
          accessInfo?.toLowerCase().includes('access') ||
    ...
    if (slugMatch) {
      ...
      if (await sidebar.isVisible()) {
        ...
        const hasProductInfo =
          sidebarContent?.includes('$') || sidebarContent?.toLowerCase().includes('access') || sidebarContent?.toLowerCase().includes('enroll');
  ...
  test('should handle user authentication flow for enrollment', async () => {
    ...
    if (await enrollButton.isVisible()) {
      ...
      const currentUrl = page.url();
      const isAuthPage = currentUrl.includes('sign-in') || currentUrl.includes('login') || currentUrl.includes('auth');
  ...
  test('should handle enrollment errors gracefully', async () => {
    ...
    if (await enrollButton.isVisible()) {
      ...
      const hasErrorHandling =
        pageContent?.toLowerCase().includes('error') || pageContent?.toLowerCase().includes('failed') || pageContent?.toLowerCase().includes('try again');
