...
const webBaseUrl = (process.env.PUBLIC_E2E_BASE_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3011').replace(/\/$/, '');
const learningBaseUrl = (process.env.LEARNING_E2E_BASE_URL ?? process.env.NEXT_PUBLIC_LEARNING_APP_URL ?? 'http://localhost:3002').replace(/\/$/, '');
...
async function seedPaidPublishedCourse() {
  ...
  const course = unwrap(
    await programs.postCourses({
      title,
      description: 'A paid course seeded by Playwright to validate prospect registration, checkout, and classroom access.',
      slug,
  ...
  unwrap(
    await content.postCoursesContent(courseId, {
      ...
      description: 'Start with a practical loop that is easy to inspect, test, and iterate.',
      type: 'Lesson',
  ...
  unwrap(await lifecycle.postCoursesSubmit(courseId), 'Submit browser E2E course');
  unwrap(await lifecycle.postCoursesApprove(courseId), 'Approve browser E2E course');
  unwrap(await lifecycle.postCoursesPublish(courseId), 'Publish browser E2E course');

  const productId = unwrap(
    await programs.postCoursesCreateProduct(courseId, {
      name: 'Paid course access',
      description: 'Grants immediate classroom access for the browser E2E course.',
      basePrice: 49,
      currency: 'USD',
    }),
  ...
  return { courseId, slug, title, productId };
}
...
async function waitForClientHydration(page) {
  ...
  await page.locator('script[src*="/_next/static"]').first().waitFor({ timeout: 30_000 }).catch(() => undefined);
...
async function waitForCompletionEvidence(page) {
  ...
  try {
    await Promise.any(completionSignals.map((locator) => locator.waitFor({ timeout: 20_000 })));
  } catch {
    const body = await page.locator('body').innerText({ timeout: 10_000 }).catch(() => '');
    throw new Error(`Lesson completion did not refresh in the classroom at ${page.url()}:\n${body.slice(0, 1_500)}`);
  }
}
...
async function runBrowserJourney(course) {
  ...
  try {
    await page.goto(`${webBaseUrl}/courses/${course.slug}`, { waitUntil: 'domcontentloaded' });
    await assertNoErrorSurface(page, 'Signed-out course page');
    await page.getByRole('heading', { name: course.title, exact: true }).first().waitFor();
    await page.getByRole('link', { name: 'Sign in to enroll' }).first().click();

    await page.waitForURL((url) => url.pathname.endsWith('/sign-in') && url.searchParams.get('redirectTo') === `/courses/${course.slug}`);
    await assertNoErrorSurface(page, 'Course sign-in page');
    await waitForClientHydration(page);
    await page.getByRole('link', { name: 'Sign up' }).click();

    await page.waitForURL((url) => url.pathname.endsWith('/sign-up') && url.searchParams.get('redirectTo') === `/courses/${course.slug}`);
    ...
    await page.getByLabel('Full Name').fill('Learning Checkout Learner');
    ...
    await page.getByRole('button', { name: 'Confirm and enter classroom' }).click();

    await page.waitForURL((url) => url.href.includes(`/courses/${course.slug}/content`), { timeout: 45_000 });
    await page.waitForLoadState('domcontentloaded');

    if (page.url().startsWith(webBaseUrl)) {
      await page.waitForURL((url) => url.origin === learningBaseUrl && url.pathname === `/courses/${course.slug}/content`, { timeout: 45_000 });
    }

    if (page.url().includes('/sign-in')) {
      throw new Error(`Learning app redirected back to sign-in instead of classroom: ${page.url()}`);
    }

    await assertNoErrorSurface(page, 'Learning app classroom');
    await page.getByRole('heading', { name: course.title, exact: true }).first().waitFor();
    await page.getByText('Course Navigation').waitFor();
    await page.getByRole('button', { name: /Lesson 1: Build a readable game loop/ }).click();

    const startButton = page.getByRole('button', { name: 'Start this item' });
    if (await startButton.count()) {
      await startButton.click();
      await page.waitForLoadState('networkidle').catch(() => undefined);
    }

    const completeButton = page.getByRole('button', { name: 'Mark completed' });
    await completeButton.waitFor();
    await completeButton.click();
    await page.waitForLoadState('networkidle').catch(() => undefined);
    await waitForCompletionEvidence(page);

    const relevantErrors = errors.filter((message) => !/favicon/i.test(message));
...
async function main() {
  ...
  console.log(`Learning checkout browser E2E passed for ${webBaseUrl}/courses/${course.slug} using product ${course.productId}`);
}
