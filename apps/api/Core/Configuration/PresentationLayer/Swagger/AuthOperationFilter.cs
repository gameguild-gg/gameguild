...
//     var hasPublicAttribute = context.MethodInfo.GetCustomAttributes<Public>(true).Any(attr => attr.IsPublic);
// 
//     // Also check if the controller has the [Public] attribute
//     var controllerHasPublicAttribute = context.MethodInfo.DeclaringType?.GetCustomAttributes<Public>(true).Any(attr => attr.IsPublic) ?? false;
// 
