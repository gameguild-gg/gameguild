...
public class RateLimitingService : IRateLimitingService
{
    ...
    public RateLimitingService(IOptions<RateLimitingOptions> options, IMemoryCache memoryCache, IDistributedCache? distributedCache, ILogger<RateLimitingService> logger)
    {
    ...
    public async Task<RateLimitCheckResult> CheckRateLimitAsync(HttpContext context, string endpoint)
    {
        ...
        _logger.LogDebug("Checking rate limit for endpoint: {Endpoint}, User: {UserId}, IP: {IpAddress}", endpoint, userId ?? "Anonymous", ipAddress);

        // Check if path is exempt
        if (IsExemptPath(context.Request.Path)) { return RateLimitCheckResult.Allow(); }

        // Get endpoint-specific configuration
        var endpointConfig = GetEndpointConfig(endpoint);

        // Check multiple rate limit scopes
        var checks = new List<Task<RateLimitCheckResult>> { CheckGlobalRateLimit(ipAddress), CheckIpRateLimit(ipAddress, endpointConfig), };

        if (!string.IsNullOrEmpty(userId)) { checks.Add(CheckUserRateLimit(userId, endpoint, endpointConfig)); }
        ...
        var mostRestrictive = results.OrderBy(r => r.IsAllowed ? 1 : 0).ThenBy(r => r.RetryAfter?.TotalSeconds ?? 0).First();

        if (!mostRestrictive.IsAllowed)
        {
            _logger.LogWarning("Rate limit exceeded for endpoint: {Endpoint}, User: {UserId}, IP: {IpAddress}, Reason: {Reason}", endpoint, userId ?? "Anonymous", ipAddress, mostRestrictive.Reason);
        }
    ...
    public async Task RecordRequestAsync(HttpContext context, string endpoint)
    {
        ...
        if (IsExemptPath(context.Request.Path)) { return; }

        var endpointConfig = GetEndpointConfig(endpoint);

        var tasks = new List<Task> { RecordGlobalRequest(ipAddress), RecordIpRequest(ipAddress, endpointConfig), RecordEndpointRequest(endpoint, endpointConfig) };

        if (!string.IsNullOrEmpty(userId)) { tasks.Add(RecordUserRequest(userId, endpoint, endpointConfig)); }

        await Task.WhenAll(tasks);

        _logger.LogDebug("Recorded request for endpoint: {Endpoint}, User: {UserId}, IP: {IpAddress}", endpoint, userId ?? "Anonymous", ipAddress);
    }
    ...
    public async Task<Dictionary<string, object>> GetRateLimitStatsAsync(string? userId = null, string? ipAddress = null)
    {
        ...
        if (!string.IsNullOrEmpty(userId)) { stats["user"] = await GetUserStatsAsync(userId); }

        if (!string.IsNullOrEmpty(ipAddress)) { stats["ip"] = await GetIpStatsAsync(ipAddress); }
    ...
    private string? GetUserId(HttpContext context) { return context.User?.FindFirst(ClaimTypes.NameIdentifier)?.Value; }

    private string GetClientIpAddress(HttpContext context)
    {
        // Check for Cloudflare's real IP header first
        if (context.Request.Headers.TryGetValue("CF-Connecting-IP", out var cfIp) && !string.IsNullOrEmpty(cfIp)) { return cfIp.ToString(); }
        ...
        if (context.Request.Headers.TryGetValue("X-Real-IP", out var realIp) && !string.IsNullOrEmpty(realIp)) { return realIp.ToString(); }

        return context.Connection.RemoteIpAddress?.ToString() ?? "unknown";
    }

    private bool IsExemptPath(string path) { return _options.ExemptPaths.Any(exemptPath => path.StartsWith(exemptPath, StringComparison.OrdinalIgnoreCase)); }

    private EndpointRateLimitConfig GetEndpointConfig(string endpoint)
    {
        if (_options.EndpointSpecificLimits.TryGetValue(endpoint, out var config)) { return config; }
    ...
    {
        if (_options.UseDistributedRateLimiting && _distributedCache != null) { return await CheckDistributedRateLimit(key, requestsPerMinute, burstSize, reason); }
    ...
    {
        var window = _memoryCache.GetOrCreate(
            $"{key}:window",
            entry =>
            {
    ...
    private Task<RateLimitCheckResult> CheckDistributedRateLimit(string key, int requestsPerMinute, int burstSize, string reason)
    {
        ...
        return Task.FromResult(CheckMemoryRateLimit(key, requestsPerMinute, burstSize, reason));
    }

    private async Task RecordGlobalRequest(string ipAddress) { await RecordRequest($"global_ip:{ipAddress}", _options.RequestsPerMinutePerIp, _options.BurstSizePerIp); }

    private async Task RecordIpRequest(string ipAddress, EndpointRateLimitConfig config)
    {
        if (config.ApplyToIp) { await RecordRequest($"ip:{ipAddress}", config.RequestsPerMinute, config.BurstSize); }
    }

    private async Task RecordUserRequest(string userId, string endpoint, EndpointRateLimitConfig config)
    {
        if (config.ApplyToUser) { await RecordRequest($"user:{userId}:{endpoint}", config.RequestsPerMinute, config.BurstSize); }
    }

    private async Task RecordEndpointRequest(string endpoint, EndpointRateLimitConfig config) { await RecordRequest($"endpoint:{endpoint}", config.RequestsPerMinute * 10, config.BurstSize * 10); }
    ...
    private Task<object> GetUserStatsAsync(string userId)
    {
        // Return user-specific rate limit statistics
        return Task.FromResult<object>(new { userId, message = "User stats not implemented yet" });
    }

    private Task<object> GetIpStatsAsync(string ipAddress)
    {
        // Return IP-specific rate limit statistics
        return Task.FromResult<object>(new { ipAddress, message = "IP stats not implemented yet" });
    }

    private Task<object> GetGlobalStatsAsync()
    {
        // Return global rate limit statistics
        return Task.FromResult<object>(new { message = "Global stats not implemented yet" });
    }
}
