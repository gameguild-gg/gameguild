...
namespace GameGuild.Tests.Helpers
{
    ...
    public class MockTenantContextService : ITenantContextService
    {
        ...
        {
            if (permission.UserId.HasValue && permission.TenantId.HasValue)
            {
                _permissions[(permission.UserId.Value, permission.TenantId.Value)] = permission;
            }
        }
        ...
        {
            if (_permissions.TryGetValue((userId, tenantId), out TenantPermission? permission) && permission.IsValid)
            {
