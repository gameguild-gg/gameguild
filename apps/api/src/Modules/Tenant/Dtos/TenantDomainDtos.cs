...
public class UpdateTenantDomainDto
{
    ...
    public void UpdateTenantDomain(TenantDomain domain)
    {
        if (!string.IsNullOrEmpty(TopLevelDomain))
            domain.TopLevelDomain = TopLevelDomain;
        
        if (Subdomain != null)
            domain.Subdomain = Subdomain;
        
        if (IsMainDomain.HasValue)
            domain.IsMainDomain = IsMainDomain.Value;
        
        if (IsSecondaryDomain.HasValue)
            domain.IsSecondaryDomain = IsSecondaryDomain.Value;
        
        if (UserGroupId.HasValue)
            domain.UserGroupId = UserGroupId.Value;
    }
}
...
public class UpdateTenantUserGroupDto
{
    ...
    public void UpdateTenantUserGroup(TenantUserGroup userGroup)
    {
        ...
        if (ParentGroupId.HasValue)
            userGroup.ParentGroupId = ParentGroupId.Value;
        
        if (IsActive.HasValue)
            userGroup.IsActive = IsActive.Value;
    }
}
