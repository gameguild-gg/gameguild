...
public class TestingRequestResolvers
{
    ...
    {
        return await context.Set<ProjectVersion>()
            .FirstOrDefaultAsync(pv => pv.Id == request.ProjectVersionId);
    }
        ...
        [Service] ApplicationDbContext context)
    {
        return await context.TestingParticipants
            .Where(p => p.TestingRequestId == request.Id)
            .ToListAsync();
    }
        ...
        [Service] ApplicationDbContext context)
    {
        return await context.TestingSessions
            .Where(s => s.TestingRequestId == request.Id)
            .ToListAsync();
    }
}
