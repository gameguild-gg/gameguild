...
public class ProgramContentService : IProgramContentService
{
    ...
    public ProgramContentService(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    {
        var existingContent = await _context.ProgramContents
            .FirstOrDefaultAsync(pc => pc.Id == content.Id && !pc.IsDeleted);
    ...
    public async Task<bool> DeleteContentAsync(Guid id)
    {
        ...
        foreach (var child in content.Children.Where(c => !c.IsDeleted))
        {
    ...
    {
        return await _context.ProgramContents
            .Where(pc => pc.ProgramId == programId && pc.IsRequired && !pc.IsDeleted)
            .OrderBy(pc => pc.SortOrder)
            .ToListAsync();
    }
    ...
    {
        return await _context.ProgramContents
            .Where(pc => pc.ProgramId == programId && pc.Type == type && !pc.IsDeleted)
            .OrderBy(pc => pc.SortOrder)
            .ToListAsync();
    }
    ...
    {
        var content = await _context.ProgramContents
            .FirstOrDefaultAsync(pc => pc.Id == contentId && !pc.IsDeleted);
    ...
    public async Task<int> GetContentCountAsync(Guid programId)
    {
        return await _context.ProgramContents
            .CountAsync(pc => pc.ProgramId == programId && !pc.IsDeleted);
    }

    public async Task<int> GetRequiredContentCountAsync(Guid programId)
    {
        return await _context.ProgramContents
            .CountAsync(pc => pc.ProgramId == programId && pc.IsRequired && !pc.IsDeleted);
    }
