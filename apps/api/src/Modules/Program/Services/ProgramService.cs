...
public class ProgramService : IProgramService
{
    ...
    public async Task<ProgramEntity?> GetProgramByIdAsync(Guid id)
    {
    ...
    public async Task<bool> ProgramExistsAsync(Guid id)
    {
        return await _context.Programs
            .Where(p => !p.IsDeleted)
            .AnyAsync(p => p.Id == id);
    ...
    public async Task<ProgramContent> AddContentAsync(Guid programId, ProgramContent content)
    {
        ...
        if (content.SortOrder == 0)
        {
            var maxOrder = await _context.ProgramContents
                .Where(pc => !pc.IsDeleted && pc.ProgramId == programId)
                .MaxAsync(pc => (int?)pc.SortOrder) ?? 0;
    ...
    public async Task<IEnumerable<ContentInteraction>> GetUserInteractionsAsync(Guid programId, Guid userId)
    {
        ...
        if (programUser == null)
            return Enumerable.Empty<ContentInteraction>();
    ...
    public async Task<IEnumerable<ProgramEntity>> SearchProgramsAsync(string searchTerm, int skip = 0, int take = 50)
    {
        if (string.IsNullOrWhiteSpace(searchTerm))
            return await GetProgramsAsync(skip, take);
    ...
    public async Task<decimal> GetAverageCompletionRateAsync(Guid programId)
    {
        var averageCompletion = await _context.ProgramUsers
            .Where(pu => !pu.IsDeleted && pu.ProgramId == programId && pu.IsActive)
            .AverageAsync(pu => (decimal?)pu.CompletionPercentage) ?? 0;
    ...
    public async Task<Dictionary<string, object>> GetProgramStatisticsAsync(Guid programId)
    {
        ...
        return new Dictionary<string, object>
        {
            ...
            ["completionRate"] = userCount > 0 ? (decimal)completedCount / userCount * 100 : 0
        };
    }
    ...
    private async Task RecalculateUserProgressAsync(Guid programUserId)
    {
        ...
        var completedContent = await _context.ContentInteractions
            .Where(ci => !ci.IsDeleted && 
                        ci.ProgramUserId == programUserId && 
                        ci.Status == ProgressStatus.Completed)
            .CountAsync();

        programUser.CompletionPercentage = (decimal)completedContent / totalContent * 100;
        
        if (programUser.CompletionPercentage >= 100 && programUser.CompletedAt == null)
        {
            programUser.CompletedAt = DateTime.UtcNow;
        }
    ...
    {
        return await _context.Programs
            .Where(p => !p.IsDeleted && p.Difficulty == difficulty)
            .OrderByDescending(p => p.CreatedAt)
            .Skip(skip)
            .Take(take)
            .ToListAsync();
    ...
    public async Task<ProgramContent?> UpdateContentAsync(Guid programId, Guid contentId, UpdateContentDto contentDto)
    {
        var content = await _context.ProgramContents
            .FirstOrDefaultAsync(c => c.Id == contentId && c.ProgramId == programId && !c.IsDeleted);
    ...
    public async Task<UserProgressDto?> AddUserToProgramAsync(Guid programId, Guid userId)
    {
        ...
        var existingUser = await _context.ProgramUsers
            .FirstOrDefaultAsync(pu => pu.ProgramId == programId && pu.UserId == userId && !pu.IsDeleted);
    ...
    {
        var programUsers = await _context.ProgramUsers
            .Where(pu => pu.ProgramId == programId && !pu.IsDeleted)
            .Skip(skip)
            .Take(take)
            .ToListAsync();
    ...
    public async Task<ProgramAnalyticsDto?> GetProgramAnalyticsAsync(Guid id)
    {
        ...
        var userCount = await _context.ProgramUsers
            .CountAsync(pu => pu.ProgramId == id && !pu.IsDeleted);
    ...
    public async Task<RevenueAnalyticsDto?> GetRevenueAnalyticsAsync(Guid id)
    {
        ...
        return new RevenueAnalyticsDto(
            id,
            0, 0, // Total and monthly revenue
            0, 0, // Total and monthly purchases
            0, 0, // ARPU and conversion rate
