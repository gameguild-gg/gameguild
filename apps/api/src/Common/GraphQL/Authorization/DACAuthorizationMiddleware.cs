...
public class DACAuthorizationMiddleware
{
    ...
    public DACAuthorizationMiddleware(FieldDelegate next)
    {
        _next = next;
    }
    ...
    public async ValueTask InvokeAsync(IMiddlewareContext context)
    {
        ...
        if (dacAttribute != null)
        {
            ...
            if (!hasPermission)
            {
                throw new UnauthorizedAccessException($"Insufficient permissions for {dacAttribute.GetType().Name}");
            }
        }
    ...
    private DACAuthorizationAttribute? GetDACAttribute(IMiddlewareContext context)
    {
        ...
        {
            return resolverMember.GetCustomAttributes(typeof(DACAuthorizationAttribute), true)
                .FirstOrDefault() as DACAuthorizationAttribute;
        }
    ...
    private async ValueTask<bool> CheckPermissionAsync(
        ...
        DACAuthorizationAttribute dacAttribute,
        IMiddlewareContext context)
    {
        return dacAttribute switch
        {
            RequireTenantPermissionAttribute tenantAttr => 
                await CheckTenantPermissionAsync(permissionService, userContext, tenantAttr),
            
            _ when IsContentTypePermissionAttribute(dacAttribute) => 
                await CheckContentTypePermissionDynamicAsync(permissionService, userContext, dacAttribute),
            
            _ when IsResourcePermissionAttribute(dacAttribute) => 
                await CheckResourcePermissionDynamicAsync(permissionService, userContext, dacAttribute, context),
    ...
    private async ValueTask<bool> CheckTenantPermissionAsync(
        IPermissionService permissionService,
        UserContext userContext,
        RequireTenantPermissionAttribute attribute)
    {
    ...
    private async ValueTask<bool> CheckContentTypePermissionDynamicAsync(
        IPermissionService permissionService,
        UserContext userContext,
        DACAuthorizationAttribute attribute)
    {
    ...
    private Guid? GetResourceIdFromContext(IMiddlewareContext context, string parameterName)
    {
        ...
        if (argumentValue is Guid guidValue)
        {
            return guidValue;
        }

        if (argumentValue is string stringValue && Guid.TryParse(stringValue, out var parsedGuid))
        {
            return parsedGuid;
        }
