...
public sealed class AssessmentContractMappingTests
{
    ...
    {
        var group = AssessmentGroup.Create(Guid.NewGuid(), " Projects ", Percent(40), 3, " Weighted work ");

        AssessmentGroupDto.FromEntity(group).Should().Be(
            new AssessmentGroupDto(group.Id, group.CourseId, "Projects", "Weighted work", Percent(40), 3));
    }

    [Fact]
    public void AssessmentDto_MapsLoadedAssessmentGroupMetadata()
    {
        var group = AssessmentGroup.Create(Guid.NewGuid(), "Projects", Percent(35), 7);
        var assessment = Assessment.Create(
            group.CourseId, "Build", AssessmentType.Project, Score(100), assessmentGroupId: group.Id);
        ...
        dto.AssessmentGroupWeightPercent.Should().Be(Percent(35));
    ...
    public void AssessmentGroupRequests_PreserveProvidedValues()
    {
        ...
        var update = new UpdateAssessmentGroupRequest("Name", "Description", Percent(25), 2);
        var assign = new AssignAssessmentGroupRequest(groupId, true);

        update.Should().Be(new UpdateAssessmentGroupRequest("Name", "Description", Percent(25), 2));
