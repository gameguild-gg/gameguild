...
public class GetTenantsPageQueryHandlerTests
{
    ...
    public async Task Handle_Should_Return_Paged_Result()
    {
        ...
        repo.Setup(r => r.GetPagedAsync(1, 10, true, It.IsAny<CancellationToken>()))
            .ReturnsAsync((tenants, 1));

        var handler = new GetTenantsPageQueryHandler(repo.Object);
        var result = await handler.Handle(new GetTenantsPageQuery(Page: 1, PageSize: 10, IncludeInactive: false), CancellationToken.None);
