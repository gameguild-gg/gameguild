...
using GameGuild.Authentication.Presentation;
...
/// <summary>
/// Unit tests for the AccessReviewMiddleware
/// </summary>
public class AccessReviewMiddlewareTests
{
    private readonly Mock<RequestDelegate> _mockNext;
    private readonly Mock<ILogger<AccessReviewMiddleware>> _mockLogger;
    private readonly AccessReviewMiddleware _middleware;
    private readonly DefaultHttpContext _httpContext;

    public AccessReviewMiddlewareTests()
    {
        _mockNext = new Mock<RequestDelegate>();
        _mockLogger = new Mock<ILogger<AccessReviewMiddleware>>();
        _middleware = new AccessReviewMiddleware(_mockNext.Object, _mockLogger.Object);
        _httpContext = new DefaultHttpContext();
    }

    [Fact]
    public async Task InvokeAsync_Should_Add_Access_Review_Header()
    {
        // Arrange
        ...
        // Assert
        _httpContext.Response.Headers.Should().ContainKey("X-Access-Review");
        _httpContext.Response.Headers["X-Access-Review"].ToString().Should().Be("enabled");
    }
    ...
    public async Task InvokeAsync_Should_Add_Header_Before_Calling_Next()
    {
        ...
        _mockNext.Setup(x => x(It.IsAny<HttpContext>()))
            ...
            {
                headerAddedBeforeNext = ctx.Response.Headers.ContainsKey("X-Access-Review");
            })
            .Returns(Task.CompletedTask);
    ...
    public async Task InvokeAsync_Should_Complete_Successfully_On_Valid_Request()
    {
        ...
        // Assert
        _httpContext.Response.Headers["X-Access-Review"].Should().Be("enabled");
        _mockNext.Verify(x => x(_httpContext), Times.Once);
    }
    ...
    public async Task InvokeAsync_Should_Work_With_Multiple_Requests()
    {
        ...
        // Assert
        context1.Response.Headers["X-Access-Review"].Should().Be("enabled");
        context2.Response.Headers["X-Access-Review"].Should().Be("enabled");
    }
    ...
    {
        // Act
        var middleware = new AccessReviewMiddleware(_mockNext.Object, _mockLogger.Object);
