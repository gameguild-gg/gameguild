...
public sealed class EconomyFifoTransferMigrationTests
{
    ...
    {
        await using var container = await EconomyPostgreSqlTestDatabase.CreateAsync("economy_fifo_transfer");
        ...
        var receipt = await PostAsync(connection, capability, riskDecision, posting, sourceWallet, destinationWallet);
        await ExecuteAsync(connection, "RESET ROLE;");
        receipt.Duplicate.Should().BeFalse();
        receipt.PostingId.Should().Be(posting);
        receipt.Sequence.Should().BePositive();
        ...
        var replay = await PostAsync(connection, capability, riskDecision, posting, sourceWallet, destinationWallet);
        replay.Duplicate.Should().BeTrue();
        replay.Sequence.Should().Be(receipt.Sequence);
        await ExecuteAsync(connection, "RESET ROLE;");
        (await ScalarAsync<long>(connection, "SELECT count(*) FROM public.economy_outbox_messages;"))
            .Should().Be(1);
        await ExecuteAsync(connection, "SET ROLE gameguild_economy_writer;");
