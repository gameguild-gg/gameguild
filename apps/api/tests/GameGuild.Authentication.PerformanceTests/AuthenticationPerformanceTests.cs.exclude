...
public class AuthenticationPerformanceTests : IDisposable
{
    ...
    private void SetupMocks()
    {
        ...
        _mockAuthService.Setup(x => x.LocalSignInAsync(It.IsAny<GameGuild.Authentication.Models.Requests.LocalSignInRequest>(), It.IsAny<CancellationToken>()))
                       .ReturnsAsync(new GameGuild.Authentication.Models.Responses.SignInResponse
                       {
                           User = new GameGuild.Authentication.Models.UserIdentity { Id = Guid.NewGuid(), Email = "test@example.com" },
