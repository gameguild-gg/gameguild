...
public sealed class SubscriptionsCoverageCompletionTests
{
    ...
    public async Task RemainingPrivateHelpersAndHandlers_ShouldCoverSwitchesAndGuardBranches()
    {
        ...
        {
            InvokePrivateStatic<Money>(typeof(SubscriptionBillingService), "GetPriceForCycle", planWithAnnual, cycle).Amount.Should().BeGreaterThan(0);
        ...
        InvokePrivateStatic<Money>(typeof(SubscriptionBillingService), "GetPriceForCycle", planWithoutAnnual, BillingCycle.Annually).Amount.Should().Be(1200m);
    ...
    public void SubscriptionDomainBranches_ShouldCoverRemainingEconomicPaths()
    {
        ...
        subscription.ProcessRenewal(new Money(300m, "USD"), "renewal-key").Success.Should().BeTrue();
        ...
        nullTenantLifecycleSubscription.ProcessRenewal(new Money(100m, "USD"), "null-tenant-renewal").Success.Should().BeTrue();
        ...
        foreach (var cycle in new[] { BillingCycle.Weekly, BillingCycle.Monthly, BillingCycle.Quarterly, BillingCycle.SemiAnnually, BillingCycle.Annually, BillingCycle.Biannually })
        {
            ...
            paymentSubscription.RecordPayment(10m, "USD", DateTime.UtcNow, $"payment-{cycle}", 1).IsSuccess.Should().BeTrue();
        }

        var sameCycleSubscription = CreateSubscription(Guid.NewGuid());
        sameCycleSubscription.Activate();
        var paymentDate = DateTime.UtcNow;
        sameCycleSubscription.RecordPayment(10m, "USD", paymentDate, "first", 1);
        sameCycleSubscription.RecordPayment(10m, "USD", paymentDate.AddDays(-1), "second", 1).IsAlreadyProcessed.Should().BeTrue();

        var sameCycleNewerPaymentSubscription = CreateSubscription(Guid.NewGuid());
        sameCycleNewerPaymentSubscription.Activate();
        sameCycleNewerPaymentSubscription.RecordPayment(10m, "USD", paymentDate, "initial", 1);
        sameCycleNewerPaymentSubscription.RecordPayment(10m, "USD", paymentDate.AddDays(1), "retry", 1).IsAlreadyProcessed.Should().BeTrue();
        ...
        sameCycleWithoutPreviousPaymentDate.RecordPayment(10m, "USD", paymentDate, "same-cycle-no-date", 1)
        ...
        currentCycleSubscription.RecordPayment(10m, "USD", paymentDate, "current-cycle").IsSuccess.Should().BeTrue();
        ...
        nullTenantPaymentSubscription.RecordPayment(10m, "USD", paymentDate, "null-tenant-payment").IsSuccess.Should().BeTrue();
        ...
        FluentActions.Invoking(() => invalidCycleSubscription.RecordPayment(10m, "USD", paymentDate, "invalid-cycle", 1))
            .Should().Throw<System.Runtime.CompilerServices.SwitchExpressionException>();
