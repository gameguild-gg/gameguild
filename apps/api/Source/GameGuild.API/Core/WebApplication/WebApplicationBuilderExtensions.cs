...
public static class WebApplicationBuilderExtensions
{
    ...
    public static WebApplicationBuilder AddApplicationLayer(this WebApplicationBuilder builder)
    {
        ...
        // Ensure all GameGuild module assemblies are loaded before scanning for CQRS handlers
        // In some hosting scenarios, referenced assemblies are not yet loaded in the AppDomain here.
        // We proactively load all GameGuild.*.dll from the application base directory.
        var baseDirectory = AppContext.BaseDirectory;
        var moduleDlls = Directory.GetFiles(baseDirectory, "GameGuild.*.dll").Where(f => !f.EndsWith("GameGuild.API.dll", StringComparison.OrdinalIgnoreCase)).ToArray();
        ...
        foreach (var dllPath in moduleDlls)
        {
            try
            {
                ...
                // Load into the default load context if not already loaded
                if (!AppDomain.CurrentDomain.GetAssemblies().Any(a => AssemblyName.ReferenceMatchesDefinition(a.GetName(), assemblyName))) { Assembly.Load(assemblyName); }
            }
            catch
            {
                // Swallow and continue; non-loadable files shouldn't block startup
            }
        }

        // Now collect all loaded GameGuild assemblies for CQRS registration
