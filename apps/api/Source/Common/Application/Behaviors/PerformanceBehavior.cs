    ...
    : IPipelineBehavior<TRequest, TResponse> where TRequest : IRequest<TResponse>
{
        ...
        bool hasError = false)
    {
        ...
        {
            logger.LogWarning("High memory usage detected: {RequestName} used {MemoryKB:F2}KB",
                requestName, memoryUsedKb);
        }

        if (elapsedMilliseconds > CriticalRequestThresholdMs)
        {
            logger.LogCritical("Critical performance issue: {RequestName} took {ElapsedMs}ms",
