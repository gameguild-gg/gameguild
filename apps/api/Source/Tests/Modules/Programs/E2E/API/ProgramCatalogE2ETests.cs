...
public class ProgramCatalogE2ETests : IClassFixture<TestWebApplicationFactory>, IDisposable {
  ...
  public async Task CourseFiltering_ByCategory_MaintainsSlugConsistency() {
    ...
    Assert.Contains(allCourses, c => c.Slug == programmingCourse.Slug && c.Category == "programming");
    Assert.Contains(allCourses, c => c.Slug == artCourse.Slug && c.Category == "art");
    Assert.Contains(allCourses, c => c.Slug == designCourse.Slug && c.Category == "design");
  }
