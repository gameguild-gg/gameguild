...
/// <summary> Service interface for Post business logic operations Provides comprehensive post management capabilities similar to ProjectService </summary>
public interface IPostService {
  #region Basic CRUD Operations

  /// <summary> Gets a post by ID with basic includes </summary>
  Task<Post?> GetPostByIdAsync(Guid id);

  /// <summary> Gets a post by ID with full details and relationships </summary>
  ...
  /// <summary> Gets all posts (use with caution) </summary>
  ...
  /// <summary> Gets posts by author ID </summary>
  Task<IEnumerable<Post>> GetPostsByAuthorAsync(Guid authorId);

  /// <summary> Gets posts by post type </summary>
  Task<IEnumerable<Post>> GetPostsByTypeAsync(string postType);

  /// <summary> Gets posts by content status </summary>
  ...
  /// <summary> Searches posts by title and description </summary>
  Task<IEnumerable<Post>> SearchPostsAsync(string searchTerm, int skip = 0, int take = 50);

  /// <summary> Gets posts with specific tags </summary>
  Task<IEnumerable<Post>> GetPostsByTagsAsync(string[ ] tags);

  /// <summary> Gets trending posts (by likes, comments, shares) </summary>
  Task<IEnumerable<Post>> GetTrendingPostsAsync(int skip = 0, int take = 50);

  /// <summary> Gets recent posts from followed users (for feed functionality) </summary>
  ...
  /// <summary> Validates if user can perform action on post </summary>
  Task<bool> CanUserPerformActionAsync(Guid postId, Guid userId, string action);

  /// <summary> Generates unique slug for post </summary>
