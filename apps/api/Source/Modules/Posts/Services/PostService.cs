...
public class PostService(ApplicationDbContext context, ILogger<PostService> logger) : IPostService {
  ...
  public async Task<Post> CreatePostAsync(Post post) {
    ...
    if (string.IsNullOrEmpty(post.Slug)) { post.Slug = await GenerateUniqueSlugAsync(post.Title, post.Tenant?.Id); }
    ...
    var statistics = new PostStatistics { PostId = post.Id, LastCalculatedAt = DateTime.UtcNow };
  ...
  public async Task<Post> UpdatePostAsync(Post post) {
    ...
    if (!string.IsNullOrEmpty(post.Title) && existingPost.Slug != Post.GenerateSlug(post.Title)) { existingPost.Slug = await GenerateUniqueSlugAsync(post.Title, existingPost.Tenant?.Id); }
  ...
  public async Task<bool> TogglePostLikeAsync(Guid postId, Guid userId, string reactionType = "like") {
    ...
    if (existingLike != null) {
      ...
      // Add like
      var like = new PostLike { PostId = postId, UserId = userId, ReactionType = reactionType, CreatedAt = DateTime.UtcNow };
