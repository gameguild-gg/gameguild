...
public class PostAnnouncementService : IPostAnnouncementService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (tenant != null)
            {
                announcementPost.Tenant = tenant;
            }
        ...
        catch (Exception ex)
        {
            ...
            return GameGuild.Common.Result.Failure<Post>(new GameGuild.Common.Error(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (tenant != null)
            {
                celebrationPost.Tenant = tenant;
            }

            _context.Posts.Add(celebrationPost);
            await _context.SaveChangesAsync(cancellationToken);

            _logger.LogInformation("Successfully created milestone celebration {PostId} for user {UserId}", 
                celebrationPost.Id, authorId);
        ...
        {
            _logger.LogError(ex, "Failed to create milestone celebration for user {UserId}: {Milestone}", 
                authorId, milestoneName);
            return GameGuild.Common.Result.Failure<Post>(new GameGuild.Common.Error(
