...
) : IQueryHandler<GetPostsQuery, Common.Result<PostsPageDto>>
{
    public async Task<Common.Result<PostsPageDto>> Handle(GetPostsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("Getting posts for tenant {TenantId}, page {PageNumber}", 
                request.TenantId, request.PageNumber);
            ...
            if (request.UserId.HasValue)
                query = query.Where(p => p.AuthorId == request.UserId.Value);

            if (request.IsPinned.HasValue)
                query = query.Where(p => p.IsPinned == request.IsPinned.Value);
            ...
            if (!request.IsPinned.HasValue)
            {
                query = query.OrderByDescending(p => p.IsPinned).ThenByDescending(p => p.CreatedAt);
            }
            ...
            logger.LogInformation("Retrieved {PostCount} posts out of {TotalCount} for tenant {TenantId}", 
                posts.Count, totalCount, request.TenantId);
        ...
        catch (Exception ex)
        {
            ...
            return Common.Result.Failure<PostsPageDto>(new Common.Error(
                "GetPosts.Failed", 
