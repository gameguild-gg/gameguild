...
) : ICommandHandler<CreatePostCommand, Common.Result<Post>>
{
    public async Task<Common.Result<Post>> Handle(CreatePostCommand request, CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("Creating post for user {AuthorId} in tenant {TenantId}", 
                request.AuthorId, request.TenantId);
            ...
            if (request.ContentReferences?.Any() == true)
            {
                ...
                {
                    var reference = new PostContentReference
                    {
                        PostId = post.Id,
                        ReferencedResourceId = refId,
                        ReferenceType = "related_content"
                    };
            ...
            logger.LogInformation("Successfully created post {PostId} with slug '{Slug}' for user {AuthorId}", 
                post.Id, post.Slug, request.AuthorId);

            // Publish domain event
            await eventPublisher.PublishAsync(new PostCreatedEvent(
                post.Id,
                post.AuthorId,
                post.Title,
                post.PostType,
    ...
    private async Task<string> EnsureUniqueSlug(string baseSlug, Guid? tenantId, CancellationToken cancellationToken)
    {
        ...
        {
            query = query.Where(p => p.Tenant!.Id == tenantId.Value);
        }
        else
        {
            query = query.Where(p => p.Tenant == null);
        }
