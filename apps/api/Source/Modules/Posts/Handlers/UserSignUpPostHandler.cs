...
/// </summary>
public class UserSignUpPostHandler(IMediator mediator, ILogger<UserSignUpPostHandler> logger) : INotificationHandler<UserSignedUpNotification> {
  public async Task Handle(UserSignedUpNotification notification, CancellationToken cancellationToken) {
    try {
      logger.LogInformation("Creating welcome post for new user {UserId} ({Email})", notification.UserId, notification.Email);
