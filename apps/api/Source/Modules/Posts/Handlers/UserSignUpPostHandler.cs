...
) : INotificationHandler<UserSignedUpNotification>
{
    public async Task Handle(UserSignedUpNotification notification, CancellationToken cancellationToken)
    {
        try
        {
            logger.LogInformation("Creating welcome post for new user {UserId} ({Email})", 
            ...
            if (result.IsSuccess)
            {
                logger.LogInformation("Successfully created welcome post {PostId} for user {UserId}", 
                    result.Value?.Id, notification.UserId);
            }
            else
            {
                logger.LogWarning("Failed to create welcome post for user {UserId}: {Error}", 
                    notification.UserId, result.Error?.Description);
            }
        }
        catch (Exception ex)
        {
    ...
    private static (string Title, string Description) GenerateWelcomeContent(UserSignedUpNotification notification)
    {
        ...
        var welcomeTitles = new[]
        {
            $"🎉 Welcome {displayName} to Game Guild!",
            $"👋 {displayName} joined the community!",
            $"� New member alert: {displayName}!",
    ...
    private static string GenerateRichWelcomeContent(UserSignedUpNotification notification)
    {
        ...
        var richContent = new
        {
            ...
            welcome = new
            {
                ...
                tips = new[]
                {
                    "Explore our community programs and projects",
                    "Connect with other creators and learners",
                    "Share your own projects and achievements",
            ...
            {
                auto_generated = true,
                post_type = "user_signup",
                template_version = "1.0"
            }
        };

        return System.Text.Json.JsonSerializer.Serialize(richContent, new System.Text.Json.JsonSerializerOptions 
        { 
    ...
    private static string DetermineSignUpMethod(UserSignedUpNotification notification)
    {
        ...
        if (notification.Username == notification.Email)
            return "Google"; // Google OAuth typically sets username to email
