...
#pragma warning restore IDE0060 // Remove unused parameter
{
    public async Task<SignInResponse> LocalSignInAsync(LocalSignInRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (userExists && user != null)
            {
                ...
                var hashPreview = user.PasswordHash != null ? user.PasswordHash.Substring(0, Math.Min(20, user.PasswordHash.Length)) : "NULL";
                logger.LogInformation("DEBUG: Verifying password for user {Email}. Password length: {PasswordLength}, Hash: {Hash}", user.Email, request.Password?.Length ?? 0, hashPreview);
    ...
    public async Task<SignInResponse> GitHubSignInAsync(OAuthSignInRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var expiresInMinutes = int.Parse(configuration["Jwt:ExpirationMinutes"] ?? configuration["Jwt:ExpiryInMinutes"] ?? "60", CultureInfo.InvariantCulture);
    ...
    public async Task<SignInResponse> GoogleIdTokenSignInAsync(GoogleIdTokenRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var refreshTokenExpiresAt = DateTime.UtcNow.AddDays(refreshTokenExpiryDays);
            var accessTokenExpiryMinutes = int.Parse(configuration["Jwt:ExpiryInMinutes"] ?? "60", CultureInfo.InvariantCulture);
    ...
    public async Task<EmailOperationResponse> ForgotPasswordAsync(PasswordResetRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return new EmailOperationResponse { Success = true, Message = "Password reset email sent successfully" };
    }
    ...
    public async Task<EmailOperationResponse> ChangePasswordAsync(ChangePasswordRequest request, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        // TEMPORARY: Return success placeholder
