...
[ApiController]
[ApiVersion("1.0")]
[Route("v{version:apiVersion}/programs")]
public class ProgramController(IProgramService programService) : ControllerBase {
  // ===== CONTENT-TYPE LEVEL OPERATIONS =====

  /// <summary> Get all programs with optional filtering (content-type level read permission) </summary>
  /// <remarks>
  /// Supports filtering via query parameters:
  /// - status=published: Filter to published programs only
  /// - category={category}: Filter by program category
  /// - difficulty={difficulty}: Filter by difficulty level
  /// - creatorId={guid}: Filter by creator
  /// - q={searchTerm}: Search programs
