...
/// <summary> Handler for getting payment by ID </summary>
public class GetPaymentByIdQueryHandler : IRequestHandler<GetPaymentByIdQuery, Payment?> {
  ...
  public async Task<Payment?> Handle(GetPaymentByIdQuery request, CancellationToken cancellationToken) {
    var query = _context.Payments.Include(p => p.Refunds).Where(p => p.Id == request.PaymentId);
...
/// <summary> Handler for getting user payments </summary>
public class GetUserPaymentsQueryHandler : IRequestHandler<GetUserPaymentsQuery, IEnumerable<Payment>> {
  ...
  public async Task<IEnumerable<Payment>> Handle(GetUserPaymentsQuery request, CancellationToken cancellationToken) {
    ...
    return await query.OrderByDescending(p => p.CreatedAt).Skip(request.Skip).Take(request.Take).ToListAsync(cancellationToken);
  }
}

/// <summary> Handler for getting product payments </summary>
...
/// <summary> Handler for getting payment statistics </summary>
public class GetPaymentStatsQueryHandler : IRequestHandler<GetPaymentStatsQuery, PaymentStats> {
  ...
  public GetPaymentStatsQueryHandler(ApplicationDbContext context, IUserContext userContext, ITenantContext tenantContext) {
    _context = context;
    _userContext = userContext;
  ...
  public async Task<PaymentStats> Handle(GetPaymentStatsQuery request, CancellationToken cancellationToken) {
    ...
    var totalRefunded = await _context.PaymentRefunds.Where(r => payments.Select(p => p.Id).Contains(r.PaymentId) && r.Status == RefundStatus.Succeeded).SumAsync(r => r.RefundAmount, cancellationToken);
...
/// <summary> Handler for getting revenue reports </summary>
public class GetRevenueReportQueryHandler : IRequestHandler<GetRevenueReportQuery, RevenueReport> {
  ...
  public GetRevenueReportQueryHandler(ApplicationDbContext context, IUserContext userContext, ITenantContext tenantContext) {
    _context = context;
    _userContext = userContext;
    _tenantContext = tenantContext;
  }
  ...
  public async Task<RevenueReport> Handle(GetRevenueReportQuery request, CancellationToken cancellationToken) {
    ...
    if (!_userContext.IsInRole("Admin")) { return new RevenueReport { FromDate = request.FromDate, ToDate = request.ToDate, TotalRevenue = 0, NetRevenue = 0, TotalTransactions = 0 }; }

    var query = _context.Payments.Where(p => p.Status == PaymentStatus.Completed).Where(p => p.CreatedAt >= request.FromDate && p.CreatedAt <= request.ToDate);
    ...
    var refunds = await _context.PaymentRefunds.Where(r => payments.Select(p => p.Id).Contains(r.PaymentId) && r.Status == RefundStatus.Succeeded).ToListAsync(cancellationToken);
    ...
    return new RevenueReport { FromDate = request.FromDate, ToDate = request.ToDate, TotalRevenue = totalRevenue, NetRevenue = totalRevenue - totalRefunded, TotalTransactions = payments.Count, DataPoints = dataPoints };
  }

  private IEnumerable<RevenueDataPoint> GroupPaymentsByPeriod(List<Payment> payments, List<PaymentRefund> refunds, string groupBy, DateTime fromDate, DateTime toDate) {
