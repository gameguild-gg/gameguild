...
namespace GameGuild.Modules.Auth.Services {
  ...
  public class OAuthService : IOAuthService {
    ...
    public async Task<string> ExchangeGitHubCodeAsync(string code, string redirectUri) {
      ...
      var content = new StringContent(JsonSerializer.Serialize(tokenRequest), System.Text.Encoding.UTF8, "application/json");
    ...
    public async Task<GitHubUserDto> GetGitHubUserAsync(string accessToken) {
      ...
      GitHubUserDto user = JsonSerializer.Deserialize<GitHubUserDto>(content, new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.SnakeCaseLower }) ?? throw new InvalidOperationException("Failed to parse GitHub user");
    ...
    public async Task<GoogleUserDto> ValidateGoogleIdTokenAsync(string idToken) {
      try {
        ...
        var googleUser = new GoogleUserDto {
          ...
          EmailVerified = tokenInfo.TryGetProperty("email_verified", out JsonElement verifiedElement) && (verifiedElement.GetString() == "true" || verifiedElement.ValueKind == JsonValueKind.True)
        };
