...
public class UserPermission : EntityBase {
  ...
  [Required] [MaxLength(50)] public string Action { get; set; } = string.Empty; // "read", "create", "edit", "delete"

  [Required] [MaxLength(100)] public string ResourceType { get; set; } = string.Empty; // "TestingSession", "Project", etc.

  public Guid? ResourceId { get; set; } // Specific resource ID, null for type-level permissions

  [MaxLength(100)] public string? GrantedByRole { get; set; } // Which role template granted this permission
  ...
  public List<PermissionConstraint> Constraints {
    get => string.IsNullOrEmpty(ConstraintsJson) ? new List<PermissionConstraint>() : JsonSerializer.Deserialize<List<PermissionConstraint>>(ConstraintsJson) ?? new List<PermissionConstraint>();
...
public class SimpleUserRoleAssignment : EntityBase {
  ...
  [Required] [MaxLength(100)] public string RoleTemplateName { get; set; } = string.Empty; // "TestingLabAdmin", "ProjectManager", etc.
...
public class RoleTemplate : EntityBase {
  ...
  public List<PermissionTemplate> PermissionTemplates {
    get => string.IsNullOrEmpty(PermissionTemplatesJson) ? new List<PermissionTemplate>() : JsonSerializer.Deserialize<List<PermissionTemplate>>(PermissionTemplatesJson) ?? new List<PermissionTemplate>();
