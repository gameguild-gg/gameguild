...
/// </summary>
public class ProcessBillingWebhookHandler : CQRS.IRequestHandler<ProcessBillingWebhookCommand, WebhookProcessingResult>
{
    private readonly ILogger<ProcessBillingWebhookHandler> _logger;
    private readonly IBillingWebhookService _webhookService;

    public ProcessBillingWebhookHandler(
        ILogger<ProcessBillingWebhookHandler> logger,
    ...
    public async Task<WebhookProcessingResult> Handle(ProcessBillingWebhookCommand command, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (result.IsSuccess)
            {
                _logger.LogInformation("Successfully processed {Provider} webhook", command.Provider);
            }
            else
            {
                _logger.LogWarning("Failed to process {Provider} webhook: {Error}", command.Provider, result.ErrorMessage);
            }
...
/// </summary>
public class ProcessStripeWebhookHandler : CQRS.IRequestHandler<ProcessStripeWebhookCommand, WebhookProcessingResult>
{
    private readonly ILogger<ProcessStripeWebhookHandler> _logger;
    private readonly IBillingWebhookService _webhookService;

    public ProcessStripeWebhookHandler(
    ...
    public async Task<WebhookProcessingResult> Handle(ProcessStripeWebhookCommand command, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await _webhookService.ProcessStripeWebhookAsync(
                command.Payload,
...
/// </summary>
public class ProcessPayPalWebhookHandler : CQRS.IRequestHandler<ProcessPayPalWebhookCommand, WebhookProcessingResult>
{
    private readonly ILogger<ProcessPayPalWebhookHandler> _logger;
    private readonly IBillingWebhookService _webhookService;

    public ProcessPayPalWebhookHandler(
        ILogger<ProcessPayPalWebhookHandler> logger,
        IBillingWebhookService webhookService)
    {
        _logger = logger;
        _webhookService = webhookService;
