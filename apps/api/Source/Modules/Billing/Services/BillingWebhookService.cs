...
public class BillingWebhookService : IBillingWebhookService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                return WebhookProcessingResult.Failure("Could not extract event ID or type from payload");
            }
            ...
            {
                _context.Set<BillingWebhookEvent>().Add(webhookEvent);
            }
            ...
            var result = provider.ToLowerInvariant() switch
            {
                ...
                _ => WebhookProcessingResult.Failure($"Unsupported provider: {provider}")
            };
            ...
            {
                webhookEvent.MarkAsFailed(result.ErrorMessage ?? "Unknown error");
            }
    ...
    public async Task<BillingWebhookEvent?> GetWebhookEventAsync(
        string provider,
        string externalEventId,
        CancellationToken cancellationToken = default)
    {
        return await _context.Set<BillingWebhookEvent>()
            .FirstOrDefaultAsync(e => e.Provider == provider && e.ExternalEventId == externalEventId, cancellationToken);
    }

    public async Task<IEnumerable<BillingWebhookEvent>> GetWebhookEventsAsync(
        Guid? tenantId = null,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (subscriptionId.HasValue)
            query = query.Where(e => e.SubscriptionId == subscriptionId.Value);
    ...
    public async Task<WebhookProcessingResult> RetryWebhookProcessingAsync(
        Guid webhookEventId,
        CancellationToken cancellationToken = default)
    {
        var webhookEvent = await _context.Set<BillingWebhookEvent>()
            .FirstOrDefaultAsync(e => e.Id == webhookEventId, cancellationToken);

        if (webhookEvent == null)
        {
            return WebhookProcessingResult.Failure("Webhook event not found");
        }

        if (webhookEvent.IsProcessed)
        {
            return WebhookProcessingResult.Failure("Webhook event already processed");
        }

        var headers = string.IsNullOrEmpty(webhookEvent.Headers)
            ? new Dictionary<string, string>()
            : JsonSerializer.Deserialize<Dictionary<string, string>>(webhookEvent.Headers) ?? new Dictionary<string, string>();
    ...
    private string? ExtractEventId(string payload, string provider)
    {
        try
        {
            ...
            {
                "stripe" => json.TryGetProperty("id", out var stripeId) ? stripeId.GetString() : null,
                "paypal" => json.TryGetProperty("id", out var paypalId) ? paypalId.GetString() : null,
                _ => null
            };
        }
        catch
    ...
    private string? ExtractEventType(string payload, string provider)
    {
        try
        {
            ...
            return provider.ToLowerInvariant() switch
            {
                "stripe" => json.TryGetProperty("type", out var stripeType) ? stripeType.GetString() : null,
                "paypal" => json.TryGetProperty("event_type", out var paypalType) ? paypalType.GetString() : null,
                _ => null
            };
        }
        catch
