...
public sealed class BillingWebhooksController : ControllerBase
{
    ...
    public async Task<IActionResult> HandleWebhook(string provider, CancellationToken ct)
    {
        try
        {
            ...
            var result = await _mediator.Send(
                new ProcessBillingWebhookCommand(provider, payload, headers),
                ct);
    ...
    public async Task<IActionResult> HandleStripeWebhook(CancellationToken ct)
    {
        try
        {
            ...
            if (result.IsSuccess)
            {
                return Ok(new { success = true, message = "Stripe webhook processed successfully" });
            }
    ...
    public async Task<IActionResult> HandlePayPalWebhook(CancellationToken ct)
    {
        try
        {
            ...
            var headers = new Dictionary<string, string>();
            foreach (var header in Request.Headers.Where(h => h.Key.StartsWith("PAYPAL-", StringComparison.OrdinalIgnoreCase)))
            {
                headers[header.Key] = header.Value.ToString();
            }
            ...
            var result = await _mediator.Send(
                new ProcessPayPalWebhookCommand(payload, headers),
                ct);

            if (result.IsSuccess)
            {
                return Ok(new { success = true, message = "PayPal webhook processed successfully" });
            }
    ...
    public async Task<IActionResult> RetryWebhook(Guid webhookEventId, CancellationToken ct)
    {
        try
        {
            ...
            if (result.IsSuccess)
            {
                return Ok(new { success = true, message = "Webhook retry successful" });
            }
