...
public sealed class CreateTestingSessionCommandHandler : ITestingLabCommandHandler<CreateTestingSessionCommand, TestingSession> {
  ...
  public async Task<TestingSession> Handle(CreateTestingSessionCommand request, CancellationToken cancellationToken) {
    ...
    var testingRequest = await _requestService.GetByIdAsync(request.TestingRequestId).ConfigureAwait(false);
    ...
    if (request.LocationId.HasValue) {
      var locationExists = await _locationRepository.ExistsAsync(request.LocationId.Value, cancellationToken).ConfigureAwait(false);
    ...
    var createdSession = await _sessionService.CreateAsync(testingSession).ConfigureAwait(false);
