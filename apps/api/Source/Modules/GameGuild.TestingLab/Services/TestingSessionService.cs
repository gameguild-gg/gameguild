...
public class TestingSessionService : ITestingSessionService {
  ...
  public async Task<IEnumerable<TestingSession>> GetAllAsync() {
    return await _context.TestingSessions.Where(ts => ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderByDescending(ts => ts.CreatedAt).ToListAsync();
  }

  public async Task<IEnumerable<TestingSession>> GetWithPaginationAsync(int skip = 0, int take = 50) {
    return await _context.TestingSessions.Where(ts => ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderByDescending(ts => ts.CreatedAt).Skip(skip).Take(take).ToListAsync();
  }

  public async Task<TestingSession?> GetByIdAsync(Guid id) { return await _context.TestingSessions.Where(ts => ts.Id == id && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).FirstOrDefaultAsync(); }

  public async Task<TestingSession?> GetByIdWithDetailsAsync(Guid id) {
    return await _context.TestingSessions.Where(ts => ts.Id == id && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).Include(ts => ts.Registrations).FirstOrDefaultAsync();
  }
  ...
  public async Task<TestingSession> UpdateAsync(TestingSession testingSession) {
    var existingSession = await _context.TestingSessions.FindAsync(testingSession.Id).ConfigureAwait(false);
    ...
    return await GetByIdAsync(existingSession.Id) ?? existingSession.ConfigureAwait(false);
  }

  public async Task<bool> DeleteAsync(Guid id) {
    var session = await _context.TestingSessions.FindAsync(id).ConfigureAwait(false);
  ...
  public async Task<IEnumerable<TestingSession>> GetByTestingRequestAsync(Guid testingRequestId) {
    return await _context.TestingSessions.Where(ts => ts.TestingRequestId == testingRequestId && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderBy(ts => ts.SessionDate).ToListAsync();
  }

  public async Task<IEnumerable<TestingSession>> GetByStatusAsync(SessionStatus status) {
    return await _context.TestingSessions.Where(ts => ts.Status == status && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderBy(ts => ts.SessionDate).ToListAsync();
  }
  ...
  public async Task<IEnumerable<TestingSession>> GetByLocationAsync(Guid locationId) {
    return await _context.TestingSessions.Where(ts => ts.LocationId == locationId && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderBy(ts => ts.SessionDate).ToListAsync();
  }

  public async Task<IEnumerable<TestingSession>> GetByDateRangeAsync(DateTime startDate, DateTime endDate) {
    return await _context.TestingSessions.Where(ts => ts.SessionDate >= startDate && ts.SessionDate <= endDate && ts.DeletedAt == null).Include(ts => ts.TestingRequest).Include(ts => ts.Location).OrderBy(ts => ts.SessionDate).ToListAsync();
  }
  ...
  public async Task<TestingSession> JoinSessionAsync(Guid userId, Guid testingSessionId) {
    ...
    var existingRegistration = await _context.SessionRegistrations.FirstOrDefaultAsync(sr => sr.SessionId == testingSessionId && sr.UserId == userId);
  ...
  public async Task<TestingSession> LeaveSessionAsync(Guid userId, Guid testingSessionId) {
    var registration = await _context.SessionRegistrations.FirstOrDefaultAsync(sr => sr.SessionId == testingSessionId && sr.UserId == userId);
