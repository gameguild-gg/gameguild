...
public sealed class CreateTestingRequestCommandValidator : AbstractValidator<CreateTestingRequestCommand> {
  public CreateTestingRequestCommandValidator() {
    RuleFor(x => x.ProjectVersionId).NotEmpty().WithMessage("Project Version ID is required.");

    RuleFor(x => x.Title).NotEmpty().MaximumLength(255).WithMessage("Title is required and must be less than 255 characters.");

    RuleFor(x => x.Description).MaximumLength(2000).When(x => !string.IsNullOrEmpty(x.Description)).WithMessage("Description must be less than 2000 characters.");

    RuleFor(x => x.DownloadUrl).Must(BeValidUrl).When(x => !string.IsNullOrEmpty(x.DownloadUrl)).WithMessage("Download URL must be a valid URL.");
