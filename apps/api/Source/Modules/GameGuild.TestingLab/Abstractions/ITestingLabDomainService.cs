...
public interface ITestingLabDomainService {
  ...
  Task<TestingSession> CreateTestingSessionWithParticipantsAsync(CreateTestingSessionCommand command, IEnumerable<Guid> participantIds, CancellationToken cancellationToken = default);

  /// <summary> Processes bulk feedback submission for a testing request </summary>
  Task<IEnumerable<TestingFeedback>> ProcessBulkFeedbackAsync(Guid testingRequestId, IEnumerable<SubmitFeedbackCommand> feedbackCommands, CancellationToken cancellationToken = default);

  /// <summary> Automatically closes expired testing requests and creates summary reports </summary>
  Task<IEnumerable<TestingRequest>> ProcessExpiredTestingRequestsAsync(CancellationToken cancellationToken = default);
