...
/// <summary> Service implementation for Project business logic Provides operations for managing projects, collaborators, releases, and access control </summary>
public class ProjectService(IApplicationDbContext context) : IProjectService {
  #region Deleted Projects

  public async Task<IEnumerable<Project>> GetDeletedProjectsAsync() { return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DeletedAt != null).OrderByDescending(p => p.DeletedAt).ToListAsync(); }

  #endregion

  #region Basic CRUD Operations

  public async Task<Project?> GetProjectByIdAsync(Guid id) { return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DeletedAt == null).FirstOrDefaultAsync(p => p.Id == id); }

  public async Task<Project?> GetProjectByIdWithDetailsAsync(Guid id) {
    return await context.Set<Project>().Include(p => p.CreatedBy!)
  ...
  public async Task<Project?> GetProjectBySlugAsync(string slug) { return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DeletedAt == null).FirstOrDefaultAsync(p => p.Slug == slug); }

  public async Task<IEnumerable<Project>> GetProjectsAsync(int skip = 0, int take = 50) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).Skip(skip).Take(take).ToListAsync();
  }

  /// <summary> Gets projects with pagination without loading related entities for performance-critical scenarios </summary>
  public async Task<IEnumerable<Project>> GetProjectsOptimizedAsync(int skip = 0, int take = 50) { return await context.Set<Project>().Where(p => p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).Skip(skip).Take(take).ToListAsync(); }

  public async Task<IEnumerable<Project>> GetAllProjectsAsync() { return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync(); }
  ...
  public async Task<IEnumerable<Project>> GetProjectsByCategoryAsync(Guid categoryId) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.CategoryId == categoryId && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync();
  }

  public async Task<IEnumerable<Project>> GetProjectsByCreatorAsync(Guid creatorId) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.CreatedById == creatorId && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync();
  }

  public async Task<IEnumerable<Project>> GetProjectsByStatusAsync(ContentStatus status) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.Status == status && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync();
  }

  public async Task<IEnumerable<Project>> GetProjectsByTypeAsync(ProjectType type) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.Type == type && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync();
  }

  public async Task<IEnumerable<Project>> GetProjectsByDevelopmentStatusAsync(DevelopmentStatus status) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.DevelopmentStatus == status && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).ToListAsync();
  }
  ...
  public async Task<IEnumerable<Project>> GetPublicProjectsAsync(int skip = 0, int take = 50) {
    ...
    return await context.Set<Project>().IgnoreQueryFilters() // Bypass global tenant and other filters
  ...
  public async Task<Project> CreateProjectAsync(Project project) {
    ...
    // Set default values
    if (project.Status == default(ContentStatus)) project.Status = ContentStatus.Draft;

    if (project.Visibility == default(AccessLevel)) project.Visibility = AccessLevel.Private;
  ...
  public async Task<bool> RestoreProjectAsync(Guid id) {
    var project = await context.Set<Project>().FirstOrDefaultAsync(p => p.Id == id && p.DeletedAt != null);
  ...
  /*
      private async Task<string> GenerateSlugAsync(string title)
      {
          ...
          while (await context.Set<Project>().AnyAsync(p => p.Slug == slug))
          {
  ...
  */
  ...
  public async Task<ProjectTeam> AddTeamToProjectAsync(Guid projectId, Guid teamId, string role, string? permissions = null) {
    var projectTeam = new ProjectTeam { ProjectId = projectId, TeamId = teamId, Role = role, Permissions = permissions, AssignedAt = DateTime.UtcNow, IsActive = true };
  ...
  public async Task<bool> RemoveTeamFromProjectAsync(Guid projectId, Guid teamId) {
    var projectTeam = await context.Set<ProjectTeam>().FirstOrDefaultAsync(pt => pt.ProjectId == projectId && pt.TeamId == teamId);
  ...
  public async Task<ProjectFollower> FollowProjectAsync(Guid projectId, Guid userId, bool emailNotifications = true, bool pushNotifications = true) {
    ...
    var existing = await context.Set<ProjectFollower>().FirstOrDefaultAsync(pf => pf.ProjectId == projectId && pf.UserId == userId);

    if (existing != null) return existing;

    var follower = new ProjectFollower { ProjectId = projectId, UserId = userId, FollowedAt = DateTime.UtcNow, EmailNotifications = emailNotifications, PushNotifications = pushNotifications };
  ...
  public async Task<bool> UnfollowProjectAsync(Guid projectId, Guid userId) {
    var follower = await context.Set<ProjectFollower>().FirstOrDefaultAsync(pf => pf.ProjectId == projectId && pf.UserId == userId);
  ...
  public async Task<bool> IsUserFollowingProjectAsync(Guid projectId, Guid userId) { return await context.Set<ProjectFollower>().AnyAsync(pf => pf.ProjectId == projectId && pf.UserId == userId); }

  public async Task<IEnumerable<ProjectFollower>> GetProjectFollowersAsync(Guid projectId) { return await context.Set<ProjectFollower>().Include(pf => pf.User).Where(pf => pf.ProjectId == projectId).OrderBy(pf => pf.FollowedAt).ToListAsync(); }

  public async Task<IEnumerable<Project>> GetProjectsFollowedByUserAsync(Guid userId) {
    return await context.Set<ProjectFollower>().Include(pf => pf.Project)
                        .ThenInclude(p => p.CreatedBy)
  ...
  public async Task<ProjectFeedback> AddProjectFeedbackAsync(Guid projectId, Guid userId, int rating, string title, string? content = null) {
    ...
    var existing = await context.Set<ProjectFeedback>().FirstOrDefaultAsync(pf => pf.ProjectId == projectId && pf.UserId == userId);
    ...
    var feedback = new ProjectFeedback { ProjectId = projectId, UserId = userId, Rating = rating, Title = title, Content = content, Status = ContentStatus.Published };
  ...
  public async Task<ProjectFeedback> UpdateProjectFeedbackAsync(Guid feedbackId, int rating, string title, string? content = null) {
    var feedback = await context.Set<ProjectFeedback>().FindAsync(feedbackId);
  ...
  public async Task<IEnumerable<ProjectFeedback>> GetProjectFeedbackAsync(Guid projectId, int skip = 0, int take = 50) {
    return await context.Set<ProjectFeedback>().Include(pf => pf.User).Where(pf => pf.ProjectId == projectId && pf.Status == ContentStatus.Published).OrderByDescending(pf => pf.CreatedAt).Skip(skip).Take(take).ToListAsync();
  }

  public async Task<ProjectFeedback?> GetUserFeedbackForProjectAsync(Guid projectId, Guid userId) { return await context.Set<ProjectFeedback>().Include(pf => pf.User).FirstOrDefaultAsync(pf => pf.ProjectId == projectId && pf.UserId == userId); }
  ...
  public async Task<ProjectJamSubmission> SubmitProjectToJamAsync(Guid projectId, Guid jamId, string? submissionNotes = null) {
    ...
    var existing = await context.Set<ProjectJamSubmission>().FirstOrDefaultAsync(pjs => pjs.ProjectId == projectId && pjs.JamId == jamId);

    if (existing != null) return existing;

    var submission = new ProjectJamSubmission { ProjectId = projectId, JamId = jamId, SubmittedAt = DateTime.UtcNow, SubmissionNotes = submissionNotes, IsEligible = true };
  ...
  public async Task<bool> RemoveProjectFromJamAsync(Guid projectId, Guid jamId) {
    var submission = await context.Set<ProjectJamSubmission>().FirstOrDefaultAsync(pjs => pjs.ProjectId == projectId && pjs.JamId == jamId);
  ...
  public async Task<IEnumerable<ProjectJamSubmission>> GetProjectJamSubmissionsAsync(Guid projectId) {
    return await context.Set<ProjectJamSubmission>().Include(pjs => pjs.Jam).Include(pjs => pjs.Scores).Where(pjs => pjs.ProjectId == projectId).ToListAsync();
  }

  public async Task<IEnumerable<Project>> GetProjectsByJamAsync(Guid jamId) {
    return await context.Set<ProjectJamSubmission>().Include(pjs => pjs.Project)
                        .ThenInclude(p => p.CreatedBy)
  ...
  public async Task<IEnumerable<Project>> GetProjectsByTenantAsync(Guid tenantId, int skip = 0, int take = 50) {
    return await context.Set<Project>().Include(p => p.CreatedBy).Include(p => p.Category).Where(p => p.TenantId == tenantId && p.DeletedAt == null).OrderByDescending(p => p.CreatedAt).Skip(skip).Take(take).ToListAsync();
  }
  ...
  public async Task<ProjectStatistics> GetProjectStatisticsAsync(Guid projectId) {
    var project = await context.Set<Project>().Include(p => p.Followers)
  ...
  public async Task<IEnumerable<Project>> GetTrendingProjectsAsync(int take = 10, TimeSpan? timeWindow = null) {
    ...
    var projects = await context.Set<Project>().Include(p => p.CreatedBy)
                                .Include(p => p.Category)
                                .Include(p => p.Followers)
                                .Include(p => p.Feedbacks)
                                .Include(p => p.Releases)
