...
using GameGuild.Projects.Queries;
using GameGuild.Projects.Services;
...
// [ExtendObjectType<Query>] // TODO: Configure GraphQL Query type
public class ProjectQueries {
  /// <summary> Gets all projects accessible to the current user using CQRS pattern </summary>
  ...
  /// <summary> Gets a project by its unique identifier using CQRS pattern </summary>
  ...
  /// <summary> Gets a project by its slug using CQRS pattern </summary>
  ...
  /// <summary> Gets projects by category using CQRS pattern </summary>
  public async Task<IEnumerable<Project>> GetProjectsByCategory(Guid categoryId, [Service] IMediator mediator, [Service] IProjectService projectService) {
    var query = new GetProjectsByCategoryQuery { CategoryId = categoryId };

    return await mediator.Send(query);
  }

  /// <summary> Gets projects by creator using CQRS pattern </summary>
  public async Task<IEnumerable<Project>> GetProjectsByCreator(Guid creatorId, [Service] IMediator mediator, [Service] IProjectService projectService) {
    var query = new GetProjectsByCreatorQuery { CreatorId = creatorId };

    return await mediator.Send(query);
  }

  /// <summary> Gets projects by status using CQRS pattern </summary>
  public async Task<IEnumerable<Project>> GetProjectsByStatus(ContentStatus status, [Service] IMediator mediator, [Service] IProjectService projectService) {
    var query = new GetProjectsByStatusQuery { Status = status };

    return await mediator.Send(query);
  }

  /// <summary> Gets deleted projects (admin only) using CQRS pattern </summary>
  ...
  /// <summary> Search projects using CQRS pattern </summary>
  public async Task<IEnumerable<Project>> SearchProjects(string searchTerm, [Service] IMediator mediator, ProjectType? type = null, Guid? categoryId = null, ContentStatus? status = null, int skip = 0, int take = 50) {
  ...
  /// <summary> Gets popular projects using CQRS pattern </summary>
  ...
  /// <summary> Gets recent projects using CQRS pattern </summary>
  ...
  /// <summary> Gets featured projects using CQRS pattern </summary>
