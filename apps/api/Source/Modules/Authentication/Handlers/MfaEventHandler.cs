...
public class MfaEventHandler(ILogger<MfaEventHandler> logger) :
    INotificationHandler<MfaEnabledEvent>,
    ...
    INotificationHandler<MfaVerificationFailedEvent>
{
    public async Task Handle(MfaEnabledEvent notification, CancellationToken cancellationToken)
    {
        logger.LogInformation(
            "MFA enabled for user {UserId} using method {Method} at {EnabledAt}",
            notification.UserId,
            notification.Method,
            notification.EnabledAt
        );
    ...
    {
        logger.LogInformation(
            "MFA disabled for user {UserId} (method: {Method}) at {DisabledAt}",
            notification.UserId,
            notification.Method,
    ...
    {
        logger.LogInformation(
            "MFA verification succeeded for user {UserId} using method {Method} at {VerifiedAt}",
            notification.UserId,
            notification.Method,
    ...
    {
        logger.LogWarning(
            "MFA verification failed for user {UserId} using method {Method}. Reason: {Reason} at {AttemptedAt}",
            notification.UserId,
            notification.Method,
            notification.Reason,
