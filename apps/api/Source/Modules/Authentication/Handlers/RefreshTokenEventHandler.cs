...
public class RefreshTokenEventHandler(ILogger<RefreshTokenEventHandler> logger) :
    INotificationHandler<RefreshTokenGeneratedEvent>,
    ...
    INotificationHandler<RefreshTokenRevokedEvent>
{
    public async Task Handle(RefreshTokenGeneratedEvent notification, CancellationToken cancellationToken)
    {
        logger.LogInformation(
            "Refresh token {TokenId} generated for user {UserId}, expires at {ExpiresAt}",
    ...
    {
        logger.LogInformation(
            "Refresh token {TokenId} used by user {UserId} at {UsedAt}",
            notification.TokenId,
            notification.UserId,
            notification.UsedAt
        );
    ...
    {
        logger.LogInformation(
            "Refresh token {TokenId} revoked for user {UserId}. Reason: {Reason} at {RevokedAt}",
            notification.TokenId,
            notification.UserId,
            notification.Reason,
