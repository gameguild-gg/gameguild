...
public class TrustedDeviceRepository(ApplicationDbContext context) : ITrustedDeviceRepository
{
    ...
    public async Task<TrustedDevice?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) { return await _context.TrustedDevices.FirstOrDefaultAsync(d => d.Id == id, cancellationToken); }
    ...
    {
        return await _context.TrustedDevices.FirstOrDefaultAsync(d => d.UserId == userId && d.DeviceFingerprint == deviceFingerprint, cancellationToken);
    }
    ...
    {
        IQueryable<TrustedDevice> query = _context.TrustedDevices.Where(d => d.UserId == userId);

        if (activeOnly) query = query.Where(d => d.IsValid);

        return await query.OrderByDescending(d => d.LastUsedAt).ToListAsync(cancellationToken);
    }
    ...
    {
        return await _context.TrustedDevices.AnyAsync(d => d.UserId == userId && d.DeviceFingerprint == deviceFingerprint && d.IsValid, cancellationToken);
    }
    ...
    {
        List<TrustedDevice> userDevices = await _context.TrustedDevices.Where(d => d.UserId == userId && d.IsActive).ToListAsync(cancellationToken);

        if (userDevices.Count == 0) return 0;
    ...
    {
        DateTime now = DateTime.UtcNow;
        List<TrustedDevice> expiredDevices = await _context.TrustedDevices.Where(d => (d.ExpiresAt.HasValue && d.ExpiresAt.Value < now) || (!d.IsActive && d.UpdatedAt < now.AddDays(-90))).ToListAsync(cancellationToken);

        if (expiredDevices.Count == 0) return 0;
