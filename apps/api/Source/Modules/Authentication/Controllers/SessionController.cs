...
[Authorize]
public class SessionController(ISessionManagementService sessionService, ILogger<SessionController> logger) : BaseController
{
    /// <summary>
    ...
    public async Task<ActionResult<List<SessionResponse>>> GetSessions()
    {
        ...
        {
            logger.LogError(ex, "Failed to get sessions for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult<SessionSecurityAnalysis>> GetSecurityAnalysis()
    {
        try
        {
            ...
            var analysis = await sessionService.AnalyzeSessionSecurityAsync(userId, ipAddress, userAgent);

            return Ok(analysis);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to analyze session security for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult> TerminateSession(Guid sessionId)
    {
        try
        {
            ...
            if (session == null || session.UserId != userId) { return NotFound("Session not found"); }

            var success = await sessionService.TerminateSessionAsync(sessionId, SessionTerminationReason.UserLogout);

            if (!success) { return BadRequest("Failed to terminate session"); }

            logger.LogInformation("User {UserId} terminated session {SessionId}", userId, sessionId);

            return Ok(new { message = "Session terminated successfully" });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to terminate session {SessionId} for user {UserId}", sessionId, GetCurrentUserId());
    ...
    public async Task<ActionResult> TerminateOtherSessions()
    {
        try
        {
            ...
            logger.LogInformation("User {UserId} terminated {Count} other sessions", userId, terminatedCount);

            return Ok(new { message = "Other sessions terminated successfully", terminatedCount });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to terminate other sessions for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult> TerminateAllSessions()
    {
        try
        {
            ...
            logger.LogInformation("User {UserId} terminated all {Count} sessions", userId, terminatedCount);

            return Ok(new { message = "All sessions terminated successfully", terminatedCount });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to terminate all sessions for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult<List<TrustedDeviceResponse>>> GetTrustedDevices()
    {
        try
        {
            ...
            var response = devices.Select(d =>
                    new TrustedDeviceResponse { Id = d.Id, DeviceName = d.DeviceName, DeviceInfo = ParseDeviceInfo(d.DeviceInfo), TrustedAt = d.TrustedAt, LastUsedAt = d.LastUsedAt, ExpiresAt = d.ExpiresAt }
                )
                .ToList();
        ...
        {
            logger.LogError(ex, "Failed to get trusted devices for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult> TrustCurrentDevice([FromBody] TrustDeviceRequest request)
    {
        try
        {
            ...
            var success = await sessionService.TrustDeviceAsync(userId, deviceFingerprint, request.DeviceName);

            if (!success) { return BadRequest("Failed to trust device"); }

            logger.LogInformation("User {UserId} trusted device {DeviceName}", userId, request.DeviceName);

            return Ok(new { message = "Device trusted successfully" });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to trust device for user {UserId}", GetCurrentUserId());
    ...
    public async Task<ActionResult> RevokeTrustedDevice(Guid deviceId)
    {
        try
        {
            ...
            if (!success) { return NotFound("Trusted device not found"); }

            logger.LogInformation("User {UserId} revoked trusted device {DeviceId}", userId, deviceId);

            return Ok(new { message = "Device trust revoked successfully" });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to revoke trusted device {DeviceId} for user {UserId}", deviceId, GetCurrentUserId());
    ...
    public async Task<ActionResult> RefreshSession()
    {
        try
        {
            ...
            if (currentSessionId == null) { return BadRequest("No active session found"); }

            var success = await sessionService.RefreshSessionAsync(currentSessionId.Value);

            if (!success) { return BadRequest("Failed to refresh session"); }

            return Ok(new { message = "Session refreshed successfully" });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to refresh session for user {UserId}", GetCurrentUserId());
    ...
    private Guid? GetCurrentSessionId()
    {
        ...
        if (string.IsNullOrEmpty(sessionIdClaim) || !Guid.TryParse(sessionIdClaim, out var sessionId)) { return null; }
    ...
    private DeviceInfo? ParseDeviceInfo(string? deviceInfoJson)
    {
        ...
        try { return System.Text.Json.JsonSerializer.Deserialize<DeviceInfo>(deviceInfoJson); }
        catch { return null; }
    }
    ...
    private LocationInfo? ParseLocation(string? locationJson)
    {
        ...
        try { return System.Text.Json.JsonSerializer.Deserialize<LocationInfo>(locationJson); }
        catch { return null; }
    }
