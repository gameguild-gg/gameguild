...
public class AuthController(IMediator mediator, ILogger<AuthController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<SignInResponse>> LocalSignIn([FromBody] LocalSignInRequest request)
    {
        try
        {
            ...
            {
                ErrorType.Validation => BadRequest(new ProblemDetails { Title = "Validation Error", Detail = result.Error.Message, Status = StatusCodes.Status400BadRequest }),
                ErrorType.Problem => Unauthorized(new ProblemDetails { Title = "Authentication Failed", Detail = result.Error.Message, Status = StatusCodes.Status401Unauthorized }),
                ErrorType.NotFound => NotFound(new ProblemDetails { Title = "Not Found", Detail = result.Error.Message, Status = StatusCodes.Status404NotFound }),
                _ => StatusCode(StatusCodes.Status500InternalServerError, new ProblemDetails { Title = "Internal Server Error", Detail = result.Error.Message, Status = StatusCodes.Status500InternalServerError })
            };
        }
        catch (ValidationException ex)
        {
