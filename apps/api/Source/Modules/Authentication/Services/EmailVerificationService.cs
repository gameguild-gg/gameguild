...
public class EmailVerificationService(ApplicationDbContext context, ILogger<EmailVerificationService> logger, IConfiguration configuration) : IEmailVerificationService
{
    ...
    public async Task<EmailOperationResponse> SendEmailVerificationAsync(string email)
    {
        try
        {
        ...
        catch (Exception ex)
        {
            ...
            return new EmailOperationResponse { Success = false, Message = "Failed to send verification email" };
        }
    }

    public async Task<EmailOperationResponse> VerifyEmailAsync(string token)
    {
        try
        {
            if (!_tokens.TryGetValue(token, out TokenInfo? tokenInfo)) return new EmailOperationResponse { Success = false, Message = "Invalid or expired token" };

            if (tokenInfo.ExpiresAt < DateTime.UtcNow)
            {
                _tokens.Remove(token);

                return new EmailOperationResponse { Success = false, Message = "Token has expired" };
            }

            if (tokenInfo.Type != "email_verification") return new EmailOperationResponse { Success = false, Message = "Invalid token type" };
            ...
            return new EmailOperationResponse { Success = true, Message = "Email verified successfully" };
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error verifying email with token {Token}", token);

            return new EmailOperationResponse { Success = false, Message = "Email verification failed" };
        }
    }

    public async Task<EmailOperationResponse> SendPasswordResetAsync(string email)
    {
        try
        {
            var normalizedEmail = email.ToLowerInvariant();
            ...
            if (user == null)
                ...
                return new EmailOperationResponse { Success = true, Message = "If an account with that email exists, a password reset link has been sent" };
        ...
        catch (Exception ex)
        {
            ...
            return new EmailOperationResponse { Success = false, Message = "Failed to send password reset email" };
        }
    }

    public async Task<EmailOperationResponse> ResetPasswordAsync(string token, string newPassword)
    {
        try
        {
            if (!_tokens.TryGetValue(token, out var tokenInfo)) return new EmailOperationResponse { Success = false, Message = "Invalid or expired token" };
            ...
            if (tokenInfo.Type != "password_reset") return new EmailOperationResponse { Success = false, Message = "Invalid token type" };

            var user = await context.Users.Include(u => u.Credentials).FirstOrDefaultAsync(u => u.Id == tokenInfo.UserId);
            ...
            else
            {
                ...
                passwordCredential = new Credential { Id = Guid.NewGuid(), UserId = user.Id, Type = "password", Value = HashPassword(newPassword), CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
        ...
        catch (Exception ex)
        {
            ...
            return new EmailOperationResponse { Success = false, Message = "Password reset failed" };
        }
    }
