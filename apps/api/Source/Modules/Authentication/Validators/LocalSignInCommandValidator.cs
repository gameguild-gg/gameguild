...
public class LocalSignInCommandValidator : AbstractValidator<LocalSignInCommand> {
  public LocalSignInCommandValidator() {
    ...
    RuleFor(x => x.Password).NotEmpty().WithMessage("Password is required").NotNull().WithMessage("Password cannot be null").MaximumLength(128).WithMessage("Password is too long");
