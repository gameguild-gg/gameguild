...
public class AuthQueries
{
    ...
    public async Task<User?> GetUserByEmail(
        [GraphQLDescription("The email address to search for")] string email, 
    ...
    {
        var userId = contextAccessor.HttpContext?.User?.FindFirst("sub")?.Value 
            ?? contextAccessor.HttpContext?.User?.FindFirst("nameid")?.Value;
    ...
    public async Task<bool> IsEmailAvailable(
        [GraphQLDescription("The email address to check")] string email,
        [Service] IMediator mediator)
    {
