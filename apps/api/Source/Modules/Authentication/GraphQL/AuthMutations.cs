...
/// <summary> GraphQL mutations for Authentication module using CQRS pattern. Provides clean separation between GraphQL layer and business logic with comprehensive error handling. </summary>
[ExtendObjectType<Mutation>]
public class AuthMutations {
  /// <summary> Local sign-up mutation using CQRS pattern. </summary>
  ...
  public async Task<SignInResponseDto> LocalSignUp([GraphQLDescription("User registration details")] LocalSignUpRequestDto input, [Service] IMediator mediator) {
  ...
  /// <summary> Local sign-in mutation using CQRS pattern. </summary>
  ...
  [Error<UnauthorizedAccessException>]
  [Error<ValidationException>]
  public async Task<bool> RevokeToken([GraphQLDescription("The refresh token to revoke")] string refreshToken, [Service] IMediator mediator, [Service] IHttpContextAccessor contextAccessor) {
  ...
  /// <summary> Refresh access token using CQRS pattern. </summary>
  ...
  [Error<UnauthorizedAccessException>]
  [Error<ValidationException>]
  public async Task<SignInResponseDto> RefreshToken([GraphQLDescription("Refresh token to use")] string refreshToken, [GraphQLDescription("Optional tenant ID")] Guid? tenantId, [Service] IMediator mediator) {
    var command = new RefreshTokenCommand { RefreshToken = refreshToken, TenantId = tenantId };

    return await mediator.Send(command);
  }

  /// <summary> Generate Web3 challenge using CQRS pattern. </summary>
  ...
  /// <summary> Verify Web3 signature using CQRS pattern. </summary>
