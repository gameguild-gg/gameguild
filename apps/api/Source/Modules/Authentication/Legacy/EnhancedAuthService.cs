...
public class EnhancedAuthService : IAuthService {
  ...
  private readonly IAuthenticationAnomalyDetectionService _anomalyDetectionService;
  ...
  public async Task<SignInResponse> LocalSignInAsync(LocalSignInRequest request) {
    ...
    try {
      ...
      var throttleDecision = await _anomalyDetectionService.ShouldThrottleAsync(ipAddress, request.Email);
      if (throttleDecision.ShouldThrottle) {
        await RecordFailedAttempt(request.Email, null, ipAddress, userAgent,
            AuthenticationFailureReasons.RateLimited, stopwatch.Elapsed, correlationId, request.TenantId);
      ...
      if (user != null) {
        var userAnalysis = await _anomalyDetectionService.AnalyzeUserLoginPatternsAsync(user.Id, ipAddress, userAgent);
    ...
    for (var attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (existing == null) {
          ...
          await _anomalyDetectionService.RecordLoginAttemptAsync(new CreateAuthenticationAttemptRequest {
            Email = "unknown",
            UserId = null,
            IpAddress = ipAddress,
  ...
  private async Task RecordSuccessfulAttempt(string email, Guid userId, string ipAddress, string? userAgent,
    ...
    var deviceFingerprint = _anomalyDetectionService.GenerateDeviceFingerprint(userAgent);
  ...
  public Task<SignInResponse> GitHubSignInAsync(OAuthSignInRequest request) {
    throw new NotImplementedException("GitHubSignInAsync not yet implemented");
  }

  public Task<SignInResponse> GoogleSignInAsync(OAuthSignInRequest request) {
    throw new NotImplementedException("GoogleSignInAsync not yet implemented");
  }

  public Task<SignInResponse> GoogleIdTokenSignInAsync(GoogleIdTokenRequestDto request) {
  ...
  public Task<SignInResponse> VerifyWeb3SignatureAsync(Web3AuthenticationVerificationRequest request) {
    throw new NotImplementedException("VerifyWeb3SignatureAsync not yet implemented");
  }

  public Task<EmailOperationResponse> SendEmailVerificationAsync(SendEmailVerificationRequest request) {
    throw new NotImplementedException("SendEmailVerificationAsync not yet implemented");
  }

  public Task<EmailOperationResponse> ForgotPasswordAsync(ForgotPasswordRequestDto request) {
    throw new NotImplementedException("ForgotPasswordAsync not yet implemented");
  }

  public Task<EmailOperationResponse> ChangePasswordAsync(ChangePasswordRequest request, Guid userId) {
