...
/// <summary> Handler for user signed up notifications - logs analytics event </summary>
public class LogAnalyticsEventHandler(ILogger<LogAnalyticsEventHandler> logger) : INotificationHandler<UserSignedUpNotification> {
  public async Task Handle(UserSignedUpNotification notification, CancellationToken cancellationToken) {
    ...
    logger.LogInformation("Analytics: User sign-up event - UserId: {UserId}, Email: {Email}, Time: {SignUpTime}", notification.UserId, notification.Email, notification.SignUpTime);
