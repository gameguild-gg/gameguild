...
public class TagRelationshipConfiguration : IEntityTypeConfiguration<TagRelationship> {
  public void Configure(EntityTypeBuilder<TagRelationship> builder) {
    ...
    builder.HasOne(tr => tr.Source).WithMany(t => t.SourceRelationships).HasForeignKey(tr => tr.SourceId).OnDelete(DeleteBehavior.Cascade);

    // Configure relationship with Target (can't be done with annotations)
    builder.HasOne(tr => tr.Target).WithMany(t => t.TargetRelationships).HasForeignKey(tr => tr.TargetId).OnDelete(DeleteBehavior.Cascade);
