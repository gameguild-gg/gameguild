...
public class NotificationService : INotificationService
{
  ...
  public async Task<NotificationResponseDto> GetNotificationsAsync(Guid userId, NotificationQueryDto query, CancellationToken cancellationToken = default)
  {
    try
    {
      ...
      {
        queryable = queryable.Where(n => n.IsStarred == query.IsStarred.Value);
      }
      ...
      {
        notificationDtos.Add(await MapToDto(notification, cancellationToken));
      }
  ...
  public async Task<int> BulkActionAsync(Guid userId, BulkNotificationActionDto dto, CancellationToken cancellationToken = default)
  {
    try
    {
      ...
      _logger.LogInformation("Performed bulk action {Action} on {Count} notifications for user {UserId}",
          dto.Action, notifications.Count, userId);
  ...
  public async Task<NotificationPreferencesDto> UpdatePreferencesAsync(Guid userId, NotificationPreferencesDto dto, CancellationToken cancellationToken = default)
  {
    try
    {
      ...
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Comment), out var comment))
        preferences.CommentNotifications = comment;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Follow), out var follow))
        preferences.FollowNotifications = follow;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Invite), out var invite))
        preferences.InviteNotifications = invite;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Reminder), out var reminder))
        preferences.ReminderNotifications = reminder;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Task), out var task))
        preferences.TaskNotifications = task;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Mention), out var mention))
        preferences.MentionNotifications = mention;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.System), out var system))
        preferences.SystemNotifications = system;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Course), out var course))
        preferences.CourseNotifications = course;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Achievement), out var achievement))
        preferences.AchievementNotifications = achievement;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Social), out var social))
        preferences.SocialNotifications = social;
      if (dto.TypePreferences.TryGetValue(nameof(NotificationType.Promotion), out var promotion))
        preferences.PromotionNotifications = promotion;
  ...
  public async Task<int> CleanupOldNotificationsAsync(DateTime olderThan, CancellationToken cancellationToken = default)
  {
    try
    {
      var oldNotifications = await _context.Notifications
          .Where(n => n.Status == NotificationStatus.Archived && n.CreatedAt < olderThan)
          .ToListAsync(cancellationToken);
