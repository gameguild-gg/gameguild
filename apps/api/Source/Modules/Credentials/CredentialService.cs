...
public class CredentialService(ApplicationDbContext context) : ICredentialService {
  ...
  /// <returns> Credential or null if not found </returns>
  public async Task<Credential?> GetCredentialByUserIdAndTypeAsync(Guid userId, string type) { return await context.Credentials.Include(c => c.User).FirstOrDefaultAsync(c => c.UserId == userId && c.Type == type); }
  ...
  public async Task<bool> RestoreCredentialAsync(Guid id) {
    ...
    var credential = await context.Credentials.IgnoreQueryFilters().FirstOrDefaultAsync(c => c.Id == id && c.DeletedAt != null);
  ...
  public async Task<IEnumerable<Credential>> GetDeletedCredentialsAsync() { return await context.Credentials.IgnoreQueryFilters().Where(c => c.DeletedAt != null).Include(c => c.User).ToListAsync(); }
}
