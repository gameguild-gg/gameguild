...
public sealed class GetContentInteractionEventsQueryHandler(IApplicationDbContext context)
    : IQueryHandler<GetContentInteractionEventsQuery, IReadOnlyList<ContentInteractionEventDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        var interactionBelongsToCourse = await context.Set<ContentInteraction>()
            .AnyAsync(
                item => item.Id == request.InteractionId && item.Content.ProgramId == request.ProgramId,
                cancellationToken)
