...
public class ProgramRatingConfiguration : IEntityTypeConfiguration<ProgramRating> {
  public void Configure(EntityTypeBuilder<ProgramRating> builder) {
    ...
    builder.HasIndex(pr => new { pr.ProgramId, pr.UserId }).IsUnique().HasDatabaseName("IX_ProgramRatings_ProgramId_UserId_Unique");
    ...
    builder.Property(pr => pr.Review).HasMaxLength(2000);

    builder.Property(pr => pr.UserId).HasMaxLength(450).IsRequired();

    builder.Property(pr => pr.IsVerified).HasDefaultValue(false);

    builder.Property(pr => pr.IsFeatured).HasDefaultValue(false);

    builder.Property(pr => pr.HelpfulVotes).HasDefaultValue(0);

    builder.Property(pr => pr.UnhelpfulVotes).HasDefaultValue(0);

    // Indexes for performance
    builder.HasIndex(pr => pr.Rating).HasDatabaseName("IX_ProgramRatings_Rating");

    builder.HasIndex(pr => pr.CreatedAt).HasDatabaseName("IX_ProgramRatings_CreatedAt");
