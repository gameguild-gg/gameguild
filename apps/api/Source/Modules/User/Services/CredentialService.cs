...
public class CredentialService : ICredentialService {
  ...
  public CredentialService(ApplicationDbContext context) { _context = context; }
  ...
  public async Task<IEnumerable<Credential>> GetCredentialsByUserIdAsync(Guid userId) { return await _context.Credentials.Where(c => c.UserId == userId).ToListAsync(); }
  ...
  public async Task<Credential?> GetCredentialByIdAsync(Guid id) { return await _context.Credentials.Include(c => c.User).FirstOrDefaultAsync(c => c.Id == id); }
  ...
  public async Task<Credential> CreateCredentialAsync(Credential credential) {
    ...
    await _context.Entry(credential).Reference(c => c.User).LoadAsync();
  ...
  public async Task<Credential> UpdateCredentialAsync(Credential credential) {
    Credential? existingCredential = await _context.Credentials.FirstOrDefaultAsync(c => c.Id == credential.Id);
    ...
    // Load the related User for the response
    await _context.Entry(existingCredential).Reference(c => c.User).LoadAsync();
  ...
  public async Task<bool> SoftDeleteCredentialAsync(Guid id) {
    Credential? credential = await _context.Credentials.FirstOrDefaultAsync(c => c.Id == id);
  ...
  public async Task<bool> HardDeleteCredentialAsync(Guid id) {
    ...
    Credential? credential = await _context.Credentials.IgnoreQueryFilters().FirstOrDefaultAsync(c => c.Id == id);
  ...
  public async Task<IEnumerable<Credential>> GetAllCredentialsAsync() { return await _context.Credentials.IgnoreQueryFilters().Include(c => c.User).ToListAsync(); }
