...
public class GetResourceUsageByTypeQueryValidator : AbstractValidator<GetResourceUsageByTypeQuery>
{
    public GetResourceUsageByTypeQueryValidator()
    {
        RuleFor(x => x.ResourceUsageType).IsInEnum().WithMessage("Invalid usage type");

        RuleFor(x => x.StartDate).NotEmpty().WithMessage("Start date is required").LessThanOrEqualTo(DateTime.UtcNow).WithMessage("Start date cannot be in the future");
