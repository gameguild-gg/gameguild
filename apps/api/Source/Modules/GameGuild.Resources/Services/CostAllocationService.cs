    ...
    public async Task<CostAllocationReport> GenerateReportAsync(Guid tenantId, DateTime periodStart, DateTime periodEnd, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var group in groupedByType)
        {
            ...
            if (quota?.Metadata != null)
            {
                try
                {
                    var quotaTags = JsonSerializer.Deserialize<Dictionary<string, string>>(quota.Metadata);

                    if (quotaTags != null)
                    {
                        foreach (var tag in quotaTags) { allocationTags.TryAdd(tag.Key, tag.Value); }
                    }
                }
                catch (JsonException ex) { logger.LogWarning(ex, "Failed to parse metadata for quota {QuotaId}", quota.Id); }
            }
        }
