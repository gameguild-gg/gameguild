...
public class ProductService : IProductService {
  ...
  public async Task<ProductEntity?> GetProductByIdAsync(Guid id) { return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null).FirstOrDefaultAsync(p => p.Id == id); }

  public async Task<ProductEntity?> GetProductByIdWithDetailsAsync(Guid id) {
    return await _context.Products.Include(p => p.Creator)
  ...
  public async Task<IEnumerable<ProductEntity>> GetProductsAsync(int skip = 0, int take = 50) {
    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null).OrderBy(p => p.Name).Skip(skip).Take(take).ToListAsync();
  }

  public async Task<IEnumerable<ProductEntity>> GetProductsByTypeAsync(ProductType type, int skip = 0, int take = 50) {
    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null && p.Type == type).OrderBy(p => p.Name).Skip(skip).Take(take).ToListAsync();
  }
  ...
  public async Task<bool> ProductExistsAsync(Guid id) { return await _context.Products.Where(p => p.DeletedAt == null).AnyAsync(p => p.Id == id); }
  ...
  public async Task<IEnumerable<ProductEntity>> GetBundleItemsAsync(Guid bundleId) {
    ...
    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null && bundleItemIds.Contains(p.Id)).OrderBy(p => p.Name).ToListAsync();
  }
  ...
  public async Task<ProductPricing?> GetCurrentPricingAsync(Guid productId) {
    return await _context.ProductPricings.Where(pp => pp.DeletedAt == null && pp.ProductId == productId && pp.IsDefault).OrderByDescending(pp => pp.CreatedAt).FirstOrDefaultAsync();
  }

  public async Task<IEnumerable<ProductPricing>> GetPricingHistoryAsync(Guid productId) {
    return await _context.ProductPricings.Where(pp => pp.DeletedAt == null && pp.ProductId == productId).OrderByDescending(pp => pp.CreatedAt).ToListAsync();
  }
  ...
  public async Task<ProductPricing> UpdatePricingAsync(Guid pricingId, decimal basePrice) {
    ProductPricing? pricing = await _context.ProductPricings.Where(pp => pp.DeletedAt == null).FirstOrDefaultAsync(pp => pp.Id == pricingId);
  ...
  public async Task<IEnumerable<ProductSubscriptionPlan>> GetSubscriptionPlansAsync(Guid productId) {
    return await _context.ProductSubscriptionPlans.Where(psp => !psp.IsDeleted && psp.ProductId == productId).OrderBy(psp => psp.Name).ToListAsync();
  }

  public async Task<ProductSubscriptionPlan?> GetSubscriptionPlanAsync(Guid planId) { return await _context.ProductSubscriptionPlans.Where(psp => !psp.IsDeleted).FirstOrDefaultAsync(psp => psp.Id == planId); }
  ...
  public async Task<bool> HasUserAccessAsync(Guid userId, Guid productId) {
    return await _context.UserProducts.Where(up => !up.IsDeleted && up.UserId == userId && up.ProductId == productId && up.AccessStatus == ProductAccessStatus.Active && (up.AccessEndDate == null || up.AccessEndDate > DateTime.UtcNow))
                         .AnyAsync();
  }

  public async Task<UserProduct?> GetUserProductAsync(Guid userId, Guid productId) {
    return await _context.UserProducts.Include(up => up.User).Include(up => up.Product).Where(up => !up.IsDeleted && up.UserId == userId && up.ProductId == productId).FirstOrDefaultAsync();
  }

  public async Task<IEnumerable<UserProduct>> GetUserProductsAsync(Guid userId) {
    return await _context.UserProducts.Include(up => up.Product).Where(up => !up.IsDeleted && up.UserId == userId).OrderByDescending(up => up.CreatedAt).ToListAsync();
  }
  ...
  public async Task<PromoCode?> GetPromoCodeAsync(string code) { return await _context.PromoCodes.Include(pc => pc.Product).Where(pc => !pc.IsDeleted && pc.Code == code).FirstOrDefaultAsync(); }
  ...
  public async Task<bool> IsPromoCodeValidAsync(string code, Guid? productId = null) {
    ...
    if (promoCode.MaxUses.HasValue) {
      int currentUsageCount = await _context.PromoCodeUses.Where(pcu => !pcu.IsDeleted && pcu.PromoCodeId == promoCode.Id).CountAsync();
  ...
  public async Task<int> GetUserCountForProductAsync(Guid productId) { return await _context.UserProducts.Where(up => !up.IsDeleted && up.ProductId == productId && up.AccessStatus == ProductAccessStatus.Active).CountAsync(); }

  public async Task<decimal> GetTotalRevenueForProductAsync(Guid productId) {
    return await _context.UserProducts.Where(up => !up.IsDeleted && up.ProductId == productId && up.AccessStatus == ProductAccessStatus.Active).SumAsync(up => up.PricePaid);
  }
  ...
  public async Task<IEnumerable<ProductEntity>> GetRecentProductsAsync(int count = 10) {
    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null && p.Status == ContentStatus.Published).OrderByDescending(p => p.CreatedAt).Take(count).ToListAsync();
  }
  ...
  public async Task<IEnumerable<ProductEntity>> GetProductsByCreatorAsync(Guid creatorId, int skip = 0, int take = 50) {
    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null && p.CreatorId == creatorId).OrderByDescending(p => p.CreatedAt).Skip(skip).Take(take).ToListAsync();
  }
  ...
  public async Task<IEnumerable<ProductEntity>> GetProductsInPriceRangeAsync(decimal minPrice, decimal maxPrice, string currency = "USD", int skip = 0, int take = 50) {
    var productIds = await _context.ProductPricings.Where(pp => pp.DeletedAt == null && pp.IsDefault && pp.Currency == currency && pp.BasePrice >= minPrice && pp.BasePrice <= maxPrice).Select(pp => pp.ProductId).ToListAsync();

    return await _context.Products.Include(p => p.Creator).Include(p => p.ProductPricings).Where(p => p.DeletedAt == null && productIds.Contains(p.Id)).OrderBy(p => p.Name).Skip(skip).Take(take).ToListAsync();
  }
}
