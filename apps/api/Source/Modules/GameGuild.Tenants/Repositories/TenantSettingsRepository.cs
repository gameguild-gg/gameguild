...
public class TenantSettingsRepository(IApplicationDbContext context) : ITenantSettingsRepository
{
    ...
    {
        return await context.Set<TenantSettings>().Include(ts => ts.Tenant).FirstOrDefaultAsync(ts => ts.TenantId == tenantId && !ts.IsDeleted, cancellationToken).ConfigureAwait(false);
    }
