    ...
    IRequestHandler<CheckProgramInWishlistQuery, bool>
{
    ...
    public async Task<IEnumerable<GameGuild.Program>> Handle(GetAllProgramsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.EnrollmentStatus.HasValue)
            query = query.Where(p => p.EnrollmentStatus == request.EnrollmentStatus.Value);
    ...
    public async Task<GameGuild.Program?> Handle(GetProgramByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.IncludeEnrollments)
            query = query.Include(p => p.ProgramUsers.Where(pu => !pu.IsDeleted));
    ...
    public async Task<IEnumerable<GameGuild.Program>> Handle(GetProgramsByCategoryQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            query = query.Where(p => p.Status == ContentStatus.Published && p.Visibility == AccessLevel.Public);
        }
    ...
    public async Task<GlobalProgramStatistics> Handle(GetGlobalProgramStatisticsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.FromDate.HasValue)
            enrollmentQuery = enrollmentQuery.Where(pu => pu.EnrollmentDate >= request.FromDate.Value);
        if (request.ToDate.HasValue)
            enrollmentQuery = enrollmentQuery.Where(pu => pu.EnrollmentDate <= request.ToDate.Value);
