...
/// <summary> GraphQL mutations for ContentInteraction operations Following permission inheritance: ContentInteraction permissions come from parent Program Implements immutable submission pattern: once submitted, interactions cannot be changed </summary>
...
public class ContentInteractionMutations {
  ...
  public async Task<ContentInteractionResult> StartContentInteraction(Guid programId, StartContentInput input, [Service] IContentInteractionService contentInteractionService, [Service] IProgramContentService programContentService) {
    try {
      ...
      var interaction = await contentInteractionService.StartContentAsync(input.ProgramUserId, input.ContentId);
