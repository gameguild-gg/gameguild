...
/// <summary> GraphQL queries for ContentInteraction operations using CQRS pattern Following permission inheritance: ContentInteraction permissions come from parent Program </summary>
...
public class ContentInteractionQueries {
  ...
  public Task<ContentInteractionStats> GetContentInteractionStats(Guid programId, [Service] IContentInteractionService contentInteractionService) {
    ...
    return Task.FromResult(
      new ContentInteractionStats { ProgramId = programId, TotalInteractions = 0, CompletedInteractions = 0, SubmittedInteractions = 0, InProgressInteractions = 0, AverageCompletionPercentage = 0, AverageTimeSpentMinutes = 0 }
    );
  }
}
