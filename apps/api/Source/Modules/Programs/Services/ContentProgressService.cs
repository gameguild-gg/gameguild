...
public class ContentProgressService : IContentProgressService {
  ...
  public async Task<ContentProgress> TrackContentAccessAsync(Guid userId, Guid contentId, Guid programEnrollmentId) {
    var progress = await _context.ContentProgress.FirstOrDefaultAsync(cp => cp.UserId == userId && cp.ContentId == contentId);
  ...
  public async Task<ContentProgress?> GetContentProgressAsync(Guid userId, Guid contentId) { return await _context.ContentProgress.Include(cp => cp.Content).FirstOrDefaultAsync(cp => cp.UserId == userId && cp.ContentId == contentId); }
  ...
  public async Task<IEnumerable<ContentProgress>> GetProgramProgressAsync(Guid userId, Guid programId) {
    return await _context.ContentProgress.Include(cp => cp.Content)
                         .Join(_context.ProgramContents, cp => cp.ContentId, pc => pc.Id, (cp, pc) => new { cp, pc })
  ...
  public async Task<decimal> CalculateProgramProgressAsync(Guid userId, Guid programId) {
    ...
    var totalContent = await _context.ProgramContents.Where(pc => pc.ProgramId == programId && pc.IsRequired).CountAsync();

    if (totalContent == 0) return 0;

    // Get completed content
    var completedContent = await _context.ContentProgress.Join(_context.ProgramContents, cp => cp.ContentId, pc => pc.Id, (cp, pc) => new { cp, pc })
  ...
  public async Task<Guid?> GetNextContentAsync(Guid userId, Guid programId) {
    ...
    var programContents = await _context.ProgramContents.Where(pc => pc.ProgramId == programId).OrderBy(pc => pc.SortOrder).Select(pc => pc.Id).ToListAsync();
  ...
  public async Task<bool> CanAccessContentAsync(Guid userId, Guid contentId) {
    ...
    var content = await _context.ProgramContents.FirstOrDefaultAsync(pc => pc.Id == contentId);
  ...
  public async Task<ContentCompletionStats> GetCompletionStatsAsync(Guid programId) {
    var allProgress = await _context.ContentProgress.Include(cp => cp.Content)
                                    .Join(_context.ProgramContents, cp => cp.ContentId, pc => pc.Id, (cp, pc) => new { cp, pc })
                                    .Where(x => x.pc.ProgramId == programId)
                                    .Select(x => x.cp)
                                    .ToListAsync();

    var totalContent = await _context.ProgramContents.Where(pc => pc.ProgramId == programId).CountAsync();
  ...
  public async Task<bool> ResetProgramProgressAsync(Guid userId, Guid programId) {
    var progressRecords = await _context.ContentProgress.Join(_context.ProgramContents, cp => cp.ContentId, pc => pc.Id, (cp, pc) => new { cp, pc })
                                        .Where(x => x.cp.UserId == userId && x.pc.ProgramId == programId)
                                        .Select(x => x.cp)
                                        .ToListAsync();
    ...
    var enrollment = await _context.ProgramEnrollments.FirstOrDefaultAsync(pe => pe.UserId == userId && pe.ProgramId == programId);
  ...
  private async Task UpdateProgramProgressAsync(Guid userId, Guid programEnrollmentId) {
    var enrollment = await _context.ProgramEnrollments.FirstOrDefaultAsync(pe => pe.Id == programEnrollmentId);
