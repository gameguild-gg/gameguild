...
public class TenantContext(IHttpContextAccessor httpContextAccessor) : ITenantContext
{
    ...
    public Guid? TenantId
    {
        get
        {
            ...
            if (Guid.TryParse(tenantIdClaim, out var tenantIdFromClaim)) { return tenantIdFromClaim; }

            // Try header
            if (httpContext.Request.Headers.TryGetValue(TenantHeaderKey, out var tenantIdHeader) && Guid.TryParse(tenantIdHeader, out var tenantIdFromHeader)) { return tenantIdFromHeader; }

            // Try query string
            if (httpContext.Request.Query.TryGetValue(TenantQueryKey, out var tenantIdQuery) && Guid.TryParse(tenantIdQuery, out var tenantIdFromQuery)) { return tenantIdFromQuery; }

            // Try route value
            if (httpContext.Request.RouteValues.TryGetValue(TenantQueryKey, out var tenantIdRoute) && Guid.TryParse(tenantIdRoute?.ToString(), out var tenantIdFromRoute)) { return tenantIdFromRoute; }

            return null;
        }
    }

    public string? TenantName { get => httpContextAccessor.HttpContext?.User?.FindFirst(TenantNameClaimType)?.Value; }
    ...
    public string? SubscriptionPlan { get => httpContextAccessor.HttpContext?.User?.FindFirst("subscription_plan")?.Value; }
