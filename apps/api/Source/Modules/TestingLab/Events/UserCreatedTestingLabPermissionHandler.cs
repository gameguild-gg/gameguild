...
/// <summary>
/// Handles UserCreatedEvent to grant basic TestingLab permissions to new users
/// </summary>
internal class UserCreatedTestingLabPermissionHandler : IDomainEventHandler<UserCreatedEvent> {
  ...
  public async Task Handle(UserCreatedEvent domainEvent, CancellationToken cancellationToken) {
    ...
    try {
      ...
      foreach (var tenantId in userTenants.Where(t => t.HasValue).Select(t => t!.Value)) {
        await GrantBasicTestingLabPermissions(domainEvent.UserId, tenantId, cancellationToken);
      }
