...
public class GetTestingRequestsQueryHandler : ITestingLabQueryHandler<GetTestingRequestsQuery, IEnumerable<TestingRequest>>
{
    ...
    public GetTestingRequestsQueryHandler(ITestingRequestService service)
    {
        _service = service;
    }
    ...
    public async Task<IEnumerable<TestingRequest>> Handle(GetTestingRequestsQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            requests = requests.Where(r => r.ProjectVersionId == request.ProjectVersionId.Value);
        }

        if (request.Status.HasValue)
        {
            requests = requests.Where(r => r.Status == request.Status.Value);
        }

        if (request.IsActive.HasValue)
        {
            requests = requests.Where(r => r.IsActive == request.IsActive.Value);
        }
