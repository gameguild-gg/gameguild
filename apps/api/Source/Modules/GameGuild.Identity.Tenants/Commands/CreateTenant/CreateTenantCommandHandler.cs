...
public class CreateTenantCommandHandler(ITenantRepository tenantRepository) : ICommandHandler<CreateTenantCommand, Guid>
{
    public async Task<Guid> Handle(CreateTenantCommand request, CancellationToken cancellationToken)
    {
        ...
        var tenant = new Tenant { Name = request.Name, Slug = request.Slug, AdminEmail = request.AdminEmail, Description = request.Description, IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
