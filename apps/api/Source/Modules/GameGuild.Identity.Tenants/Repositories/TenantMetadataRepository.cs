...
public class TenantMetadataRepository(IApplicationDbContext context) : ITenantMetadataRepository
{
    ...
    {
        return await context.Set<TenantMetadata>().FirstOrDefaultAsync(tm => tm.TenantId == tenantId && tm.DeletedAt == null, cancellationToken).ConfigureAwait(false);
    }

    public async Task<TenantMetadata?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        return await context.Set<TenantMetadata>().FirstOrDefaultAsync(tm => tm.Id == id && tm.DeletedAt == null, cancellationToken).ConfigureAwait(false);
    }

    public async Task<List<TenantMetadata>> GetByIndustryAsync(string industry, CancellationToken cancellationToken = default)
    {
        return await context.Set<TenantMetadata>().Where(tm => tm.Industry == industry && tm.DeletedAt == null).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    public async Task<List<TenantMetadata>> GetBySizeAsync(TenantSize size, CancellationToken cancellationToken = default)
    {
        return await context.Set<TenantMetadata>().Where(tm => tm.Size == size && tm.DeletedAt == null).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    public async Task<List<TenantMetadata>> GetByTypeAsync(string type, CancellationToken cancellationToken = default)
    {
        return await context.Set<TenantMetadata>().Where(tm => tm.Type == type && tm.DeletedAt == null).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
