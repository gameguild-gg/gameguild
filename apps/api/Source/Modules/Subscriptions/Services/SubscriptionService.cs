...
/// <summary> Service implementation for managing user subscriptions </summary>
public class SubscriptionService(ApplicationDbContext context) : ISubscriptionService {
  public async Task<IEnumerable<UserSubscription>> GetUserSubscriptionsAsync(Guid userId) {
    return await context.UserSubscriptions.Where(s => s.UserId == userId).Include(s => s.SubscriptionPlan).OrderByDescending(s => s.CreatedAt).ToListAsync();
  }

  public async Task<UserSubscription?> GetActiveSubscriptionAsync(Guid userId) {
    return await context.UserSubscriptions.Where(s => s.UserId == userId && s.Status == SubscriptionStatus.Active).Include(s => s.SubscriptionPlan).FirstOrDefaultAsync();
  }

  public async Task<UserSubscription?> GetSubscriptionByIdAsync(Guid id) { return await context.UserSubscriptions.Include(s => s.SubscriptionPlan).Include(s => s.User).FirstOrDefaultAsync(s => s.Id == id); }
  ...
  public async Task<IEnumerable<UserSubscription>> GetSubscriptionsAsync(int skip = 0, int take = 50, SubscriptionStatus? status = null) {
    var query = context.UserSubscriptions.Include(s => s.SubscriptionPlan).Include(s => s.User).AsQueryable();

    if (status.HasValue) query = query.Where(s => s.Status == status.Value);

    return await query.Skip(skip).Take(take).OrderByDescending(s => s.CreatedAt).ToListAsync();
  }
  ...
  public async Task<UserSubscription?> CancelSubscriptionAsync(Guid subscriptionId, Guid userId) {
    var subscription = await context.UserSubscriptions.FirstOrDefaultAsync(s => s.Id == subscriptionId && s.UserId == userId);
  ...
  public async Task<UserSubscription?> RenewSubscriptionAsync(Guid subscriptionId) {
    var subscription = await context.UserSubscriptions.FirstOrDefaultAsync(s => s.Id == subscriptionId);
  ...
  public async Task<bool> IsSubscriptionActiveAsync(Guid userId) { return await context.UserSubscriptions.AnyAsync(s => s.UserId == userId && s.Status == SubscriptionStatus.Active && s.CurrentPeriodEnd > DateTime.UtcNow); }
  ...
  public async Task<bool> HasAccessToProductAsync(Guid userId, Guid productId) {
    ...
    return await context.UserSubscriptions.Include(s => s.UserProducts)
                        .AnyAsync(s => s.UserId == userId && s.Status == SubscriptionStatus.Active && s.CurrentPeriodEnd > DateTime.UtcNow && s.UserProducts.Any(up => up.ProductId == productId));
  }
