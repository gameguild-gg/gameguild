...
public class ProgramCertificatesController(ApplicationDbContext db) : ControllerBase {
  ...
  public async Task<ActionResult<CertificateTag>> AddCertificateTag(Guid programId, Guid certificateId, [FromBody] AddCertificateTagDto dto) {
    ...
    var exists = await db.CertificateTags.Where(ct => ct.CertificateId == certificateId && ct.TagId == dto.TagId).AnyAsync();

    if (exists) return Conflict("Tag already added to certificate");

    var ct = new CertificateTag { CertificateId = certificateId, TagId = dto.TagId, RelationshipType = dto.RelationshipType, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow };
  ...
  public async Task<ActionResult> RemoveCertificateTag(Guid programId, Guid certificateId, Guid tagId) {
    ...
    var ct = await db.CertificateTags.Where(x => x.CertificateId == certificateId && x.TagId == tagId).FirstOrDefaultAsync();
