...
public abstract class EntityBase<TKey> : IEntity<TKey>, ITenantScoped where TKey : IEquatable<TKey>
{
    ...
    protected EntityBase(object partial) : this() { ApplyPartial(partial); }
    ...
    private void SetPropertiesInternal(Dictionary<string, object?> properties, bool isFromConstructor)
    {
        ...
        foreach (var property in properties)
        {
            ...
            try
            {
                ...
                if (targetType == typeof(Guid) && value is string guidString && Guid.TryParse(guidString, out var guid)) value = guid;

                if (targetType == typeof(TenantId) || targetType == typeof(TenantId?))
                {
                    if (value is string tenantIdString && Guid.TryParse(tenantIdString, out var parsedGuid))
                        value = new TenantId(parsedGuid);
                    else if (value is Guid tenantIdGuid) value = new TenantId(tenantIdGuid);
                }
            ...
            {
                // TODO: Handle or log conversion errors as needed
