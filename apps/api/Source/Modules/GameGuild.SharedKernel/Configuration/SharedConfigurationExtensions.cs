...
public static class SharedConfigurationExtensions
{
    ...
    {
        var options = OptionBuilderUtilities.CreateBindAndValidate(configuration, sectionName, defaultFactory, validator);
    ...
    /// <param name="validator">Optional validation action</param>
    /// <returns>The service collection for chaining</returns>
    public static IServiceCollection ConfigureOptions<TOptions>(this IServiceCollection services, IConfiguration configuration, Func<TOptions> defaultFactory, Action<TOptions>? validator = null) where TOptions : class
    {
