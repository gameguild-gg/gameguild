...
public class UserMetadataRepository(IApplicationDbContext context) : IUserMetadataRepository
{
    ...
    {
        return await context.Set<UserMetadata>().FirstOrDefaultAsync(um => um.UserId == userId && !um.IsDeleted, cancellationToken).ConfigureAwait(false);
    }

    public async Task<UserMetadata?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        return await context.Set<UserMetadata>().FirstOrDefaultAsync(um => um.Id == id && !um.IsDeleted, cancellationToken).ConfigureAwait(false);
    }
