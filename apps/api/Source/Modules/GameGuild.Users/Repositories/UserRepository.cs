...
public class UserRepository(IApplicationDbContext context) : IUserRepository
{
    ...
    {
        return await context.Set<User>().FirstOrDefaultAsync(u => u.Id == id && !u.IsDeleted, cancellationToken).ConfigureAwait(false);
    }

    public async Task<User?> GetByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        return await context.Set<User>().FirstOrDefaultAsync(u => u.Email == email && !u.IsDeleted, cancellationToken).ConfigureAwait(false);
    }

    public async Task<IEnumerable<User>> GetAllAsync(CancellationToken cancellationToken = default) { return await context.Set<User>().Where(u => !u.IsDeleted).ToListAsync(cancellationToken).ConfigureAwait(false); }
    ...
    {
        var query = context.Set<User>().Where(u => !u.IsDeleted);
    ...
    {
        return await context.Set<User>().Where(u => ids.Contains(u.Id) && !u.IsDeleted).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await context.Set<User>().Where(u => emails.Contains(u.Email) && !u.IsDeleted).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await context.Set<User>().Where(u => u.IsActive && !u.IsDeleted).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        var existingEmails = await context.Set<User>().Where(u => emails.Contains(u.Email) && !u.IsDeleted).Select(u => u.Email).ToListAsync(cancellationToken).ConfigureAwait(false);

        return emails.ToDictionary(email => email, email => existingEmails.Contains(email));
    }

    public async Task<bool> ExistsAsync(Guid id, CancellationToken cancellationToken = default) { return await context.Set<User>().AnyAsync(u => u.Id == id && !u.IsDeleted, cancellationToken).ConfigureAwait(false); }

    public async Task<bool> ExistsByEmailAsync(string email, CancellationToken cancellationToken = default)
    {
        return await context.Set<User>().AnyAsync(u => u.Email == email && !u.IsDeleted, cancellationToken).ConfigureAwait(false);
    }
