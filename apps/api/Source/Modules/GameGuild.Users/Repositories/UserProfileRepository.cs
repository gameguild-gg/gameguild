...
public class UserProfileRepository(IApplicationDbContext context) : IUserProfileRepository
{
    ...
    {
        var query = context.Set<UserProfile>().Where(up => up.UserId == userId && !up.IsDeleted);
        return await query.FirstOrDefaultAsync(cancellationToken).ConfigureAwait(false);
    }

    public async Task<UserProfile?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        var query = context.Set<UserProfile>().Where(up => up.Id == id && !up.IsDeleted);
        return await query.FirstOrDefaultAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    public async Task<(List<UserProfile> Profiles, int TotalCount)> GetProfilesPagedAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = context.Set<UserProfile>().Where(up => !up.IsDeleted);
