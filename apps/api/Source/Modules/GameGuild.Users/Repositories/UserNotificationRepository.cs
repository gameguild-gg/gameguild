...
public class UserNotificationRepository(IApplicationDbContext context) : IUserNotificationRepository
{
    ...
    {
        return await context.Set<UserNotification>().FirstOrDefaultAsync(n => n.Id == id && !n.IsDeleted, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        var query = context.Set<UserNotification>().Where(n => n.UserId == userId && !n.IsDeleted);
    ...
    {
        return await context.Set<UserNotification>().CountAsync(n => n.UserId == userId && !n.IsRead && !n.IsDeleted && !n.IsArchived, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await context.Set<UserNotification>()
            .Where(n => n.UserId == userId && !n.IsRead && !n.IsDeleted && !n.IsArchived)
            .GroupBy(n => n.Type)
    ...
    public async Task ArchiveAllAsync(Guid userId, int olderThanDays = 30, CancellationToken cancellationToken = default)
    {
        ...
        await context.Set<UserNotification>()
            .Where(n => n.UserId == userId && !n.IsArchived && !n.IsDeleted && n.CreatedAt < cutoffDate)
