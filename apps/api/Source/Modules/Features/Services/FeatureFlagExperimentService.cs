...
public class ABTestResult
{
    ...
    public List<ExperimentStatistics> VariantStats { get; set; } = new();
}
...
public class FeatureFlagExperimentService : IFeatureFlagExperimentService
{
    public ABTestResult EvaluateABTest(Dictionary<string, (int observations, int conversions)> variants, double confidenceLevel = 0.95)
    {
        ...
        var pooledProportion = (double)(control.Value.conversions + treatment.Value.conversions) /
                               (control.Value.observations + treatment.Value.observations);

        var standardError = Math.Sqrt(pooledProportion * (1 - pooledProportion) *
                           (1.0 / control.Value.observations + 1.0 / treatment.Value.observations));
    ...
    {
        if (!isSignificant && totalSampleSize < 1000)
        {
            return $"Inconclusive - Need more data. Current sample size: {totalSampleSize}. Continue test.";
        }

        if (!isSignificant)
        {
            return $"No significant difference detected (p={pValue:F4}). Consider stopping test.";
        }

        if (uplift > 0)
        {
            return $"Treatment variant shows {uplift:F2}% improvement (p={pValue:F4}). Recommend rollout.";
        }
        else
        {
    ...
    private double InverseStandardNormalCDF(double p)
    {
        ...
        var a = new[] { 2.50662823884, -18.61500062529, 41.39119773534, -25.44106049637 };
        var b = new[] { -8.47351093090, 23.08336743743, -21.06224101826, 3.13082909833 };
