...
/// <summary> Service implementation for feature flag management and evaluation </summary>
public class FeatureFlagService : IFeatureFlagService {
  ...
  public async Task<FeatureEvaluationResult> EvaluateFeatureAsync(string featureKey, FeatureContext context, CancellationToken cancellationToken = default) {
    try {
      ...
      if (featureFlag == null) {
        ...
        return new FeatureEvaluationResult { FeatureKey = featureKey, IsEnabled = false, Reason = "Feature flag not found" };
      }

      // Check environment
      if (!string.IsNullOrEmpty(featureFlag.Environment) && featureFlag.Environment != context.Environment) {
        return new FeatureEvaluationResult { FeatureKey = featureKey, IsEnabled = false, Reason = $"Environment mismatch: expected {featureFlag.Environment}, got {context.Environment}" };
      }

      // Check tenant-specific flags
      if (!featureFlag.IsGlobal && featureFlag.TenantId.HasValue && featureFlag.TenantId != context.TenantId) { return new FeatureEvaluationResult { FeatureKey = featureKey, IsEnabled = false, Reason = "Tenant mismatch" }; }
      ...
      if (featureFlag.RolloutPercentage < 100) {
        ...
        if (!isInRollout) {
          var result = new FeatureEvaluationResult {
            FeatureKey = featureKey, IsEnabled = false, Value = GetTypedValue(featureFlag.DefaultValue, featureFlag.Type), Reason = $"User not in rollout percentage ({featureFlag.RolloutPercentage}%)",
          };
      ...
      var globalResult = new FeatureEvaluationResult {
        ...
        Value = featureFlag.IsEnabled ? GetTypedValue(featureFlag.EnabledValue, featureFlag.Type) : GetTypedValue(featureFlag.DefaultValue, featureFlag.Type),
      ...
      return new FeatureEvaluationResult { FeatureKey = featureKey, IsEnabled = false, Reason = $"Evaluation error: {ex.Message}" };
    }
  }

  public async Task<bool> GetBooleanAsync(string featureKey, bool defaultValue = false, FeatureContext? context = null, CancellationToken cancellationToken = default) {
  ...
  public async Task<int> GetIntAsync(string featureKey, int defaultValue = 0, FeatureContext? context = null, CancellationToken cancellationToken = default) {
    context ??= new FeatureContext();
    var result = await EvaluateFeatureAsync(featureKey, context, cancellationToken);
  ...
  public async Task<double> GetDoubleAsync(string featureKey, double defaultValue = 0d, FeatureContext? context = null, CancellationToken cancellationToken = default) {
    context ??= new FeatureContext();
    var result = await EvaluateFeatureAsync(featureKey, context, cancellationToken);
  ...
  public async Task<FeatureFlag?> GetFeatureFlagByIdAsync(Guid id, CancellationToken cancellationToken = default) { return await _context.FeatureFlags.Include(f => f.Targets).FirstOrDefaultAsync(f => f.Id == id, cancellationToken); }
  ...
  public async Task<FeatureFlag?> GetFeatureFlagByKeyAsync(string key, CancellationToken cancellationToken = default) {
    ...
    var featureFlag = await _context.FeatureFlags.Include(f => f.Targets).FirstOrDefaultAsync(f => f.Key == key, cancellationToken);

    if (featureFlag != null) { _cache[key] = featureFlag; }

    return featureFlag;
  }

  public async Task<IEnumerable<FeatureFlag>> GetFeatureFlagsAsync(Guid? tenantId = null, string? environment = null, CancellationToken cancellationToken = default) {
    var query = _context.FeatureFlags.Include(f => f.Targets).AsQueryable();

    if (tenantId.HasValue) { query = query.Where(f => f.IsGlobal || f.TenantId == tenantId.Value); }

    if (!string.IsNullOrEmpty(environment)) { query = query.Where(f => f.Environment == environment); }

    return await query.OrderBy(f => f.Key).ToListAsync(cancellationToken);
  }

  public async Task<IEnumerable<FeatureFlagUsage>> GetUsageAnalyticsAsync(Guid featureFlagId, DateTime? fromDate = null, DateTime? toDate = null, CancellationToken cancellationToken = default) {
    ...
    foreach (var target in featureFlag.Targets.OrderBy(t => t.CreatedAt)) {
      ...
      if (matches) {
        return new FeatureEvaluationResult {
          ...
          Value = target.IsIncluded ? GetTypedValue(target.Value ?? featureFlag.EnabledValue, featureFlag.Type) : GetTypedValue(featureFlag.DefaultValue, featureFlag.Type),
  ...
  private async Task RecordUsageAsync(FeatureFlag featureFlag, FeatureContext context, bool wasEnabled, object? value, string? reason, CancellationToken cancellationToken) {
    try {
      var usage = new FeatureFlagUsage {
        ...
        Reason = reason,
        ContextData = JsonSerializer.Serialize(new { context.UserRoles, context.CustomAttributes, context.IpAddress, context.UserAgent }),
      };

      _context.FeatureFlagUsage.Add(usage);
      await _context.SaveChangesAsync(cancellationToken);
    }
    catch (Exception ex) { _logger.LogError(ex, "Error recording feature flag usage for '{FeatureKey}'", featureFlag.Key); }
  }
  ...
  private object? GetTypedValue(string? value, FeatureFlagType type) {
    ...
    try {
      return type switch {
        FeatureFlagType.Toggle => bool.Parse(value), FeatureFlagType.Numeric => double.Parse(value), FeatureFlagType.String => value, FeatureFlagType.Percentage => double.Parse(value), FeatureFlagType.UserSegment => value, _ => value,
      };
    }
    catch { return value; }
  }
  ...
  private async Task RefreshCacheIfNeeded(CancellationToken cancellationToken) {
    if (DateTime.UtcNow - _lastCacheUpdate > TimeSpan.FromMinutes(5)) { await RefreshCache(cancellationToken); }
  }

  private async Task RefreshCache(CancellationToken cancellationToken) {
    try {
      var flags = await _context.FeatureFlags.Include(f => f.Targets).ToListAsync(cancellationToken);

      _cache.Clear();

      foreach (var flag in flags) { _cache[flag.Key] = flag; }

      _lastCacheUpdate = DateTime.UtcNow;
      _logger.LogDebug("Refreshed feature flag cache with {Count} flags", flags.Count);
    }
    catch (Exception ex) { _logger.LogError(ex, "Error refreshing feature flag cache"); }
  }
