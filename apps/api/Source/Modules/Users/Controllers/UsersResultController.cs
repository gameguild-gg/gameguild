...
public class UsersResultController(IMediator mediator, ILogger<UsersResultController> logger) : ControllerBase {
  ...
  public async Task<ActionResult> DeleteUser(int id, CancellationToken cancellationToken) {
    ...
    return StatusCode(StatusCodes.Status501NotImplemented, new ProblemDetails { Status = StatusCodes.Status501NotImplemented, Title = "Not Implemented", Detail = "Delete user functionality is not yet implemented with Result pattern" });
  }
}
