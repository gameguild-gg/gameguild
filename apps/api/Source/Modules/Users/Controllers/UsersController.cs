...
public class UsersController(IMediator mediator, ILogger<UsersController> logger) : ControllerBase {
  ...
  public async Task<ActionResult<IEnumerable<UserResponseDto>>> GetUsers([FromQuery] bool includeDeleted = false, [FromQuery] int skip = 0, [FromQuery] int take = 50, [FromQuery] bool? isActive = null) {
    ...
    var userDtos = users.Select(u => new UserResponseDto {
      ...
      Email = u.Email,
      IsActive = u.IsActive,
      Balance = u.Balance.ToDecimal(),
      AvailableBalance = u.AvailableBalance.ToDecimal(),
  ...
  public async Task<ActionResult<UserResponseDto>> GetUser(Guid id, [FromQuery] bool includeDeleted = false) {
    ...
    var userDto = new UserResponseDto {
      ...
      AvailableBalance = user.AvailableBalance.ToDecimal(),
  ...
  public async Task<ActionResult<UserResponseDto>> UpdateUser(Guid id, [FromBody] UpdateUserDto updateDto) {
    var command = new UpdateUserCommand { UserId = id, Name = updateDto.Name, Username = updateDto.Username, Email = updateDto.Email, IsActive = updateDto.IsActive, ExpectedVersion = updateDto.ExpectedVersion };
  ...
  /// <summary> Update user balance </summary>
  ...
  public async Task<ActionResult<UserResponseDto>> UpdateUserBalance(Guid id, [FromBody] UpdateUserBalanceDto updateDto) {
    var command = new UpdateUserBalanceCommand { UserId = id, Balance = updateDto.Balance, AvailableBalance = updateDto.AvailableBalance, Reason = updateDto.Reason, ExpectedVersion = updateDto.ExpectedVersion };
