...
/// <summary> FluentValidation validator for UpdateUserBalanceCommand </summary>
public class UpdateUserBalanceCommandValidator : AbstractValidator<UpdateUserBalanceCommand> {
  ...
  public UpdateUserBalanceCommandValidator(ApplicationDbContext context) {
    ...
    RuleFor(x => x.AvailableBalance).GreaterThanOrEqualTo(0).WithMessage("Available balance must be greater than or equal to 0");

    RuleFor(x => x.Reason).MaximumLength(500).WithMessage("Reason cannot exceed 500 characters");

    RuleFor(x => x.ExpectedVersion).GreaterThan(0).When(x => x.ExpectedVersion.HasValue).WithMessage("Expected version must be greater than 0");

    // Business rule: Available balance should not exceed total balance
    RuleFor(x => x.AvailableBalance).LessThanOrEqualTo(x => x.Balance).WithMessage("Available balance cannot exceed total balance");
  }
