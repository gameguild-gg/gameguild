...
/// <summary> Handler for getting achievement by ID </summary>
public class GetAchievementByIdQueryHandler : IQueryHandler<GetAchievementByIdQuery, Result<Achievement>> {
  ...
  public async Task<Result<Achievement>> Handle(GetAchievementByIdQuery request, CancellationToken cancellationToken) {
    try {
      var query = _context.Achievements.Where(a => a.Id == request.AchievementId && a.TenantId == request.TenantId);

      if (request.IncludeLevels) { query = query.Include(a => a.Levels); }

      if (request.IncludePrerequisites) { query = query.Include(a => a.Prerequisites).ThenInclude(p => p.PrerequisiteAchievement); }
...
/// <summary> Handler for getting user achievements </summary>
public class GetUserAchievementsQueryHandler : IQueryHandler<GetUserAchievementsQuery, Result<UserAchievementsPageDto>> {
  ...
  public async Task<Result<UserAchievementsPageDto>> Handle(GetUserAchievementsQuery request, CancellationToken cancellationToken) {
    try {
      var query = _context.UserAchievements.Include(ua => ua.Achievement).ThenInclude(a => a!.Levels).Where(ua => ua.UserId == request.UserId && ua.TenantId == request.TenantId);

      // Apply filters
      if (!string.IsNullOrEmpty(request.Category)) { query = query.Where(ua => ua.Achievement!.Category == request.Category); }

      if (!string.IsNullOrEmpty(request.Type)) { query = query.Where(ua => ua.Achievement!.Type == request.Type); }

      if (request.IsCompleted.HasValue) { query = query.Where(ua => ua.IsCompleted == request.IsCompleted.Value); }

      if (request.EarnedAfter.HasValue) { query = query.Where(ua => ua.EarnedAt >= request.EarnedAfter.Value); }

      if (request.EarnedBefore.HasValue) { query = query.Where(ua => ua.EarnedAt <= request.EarnedBefore.Value); }
...
/// <summary> Handler for getting user achievement progress </summary>
public class GetUserAchievementProgressQueryHandler : IQueryHandler<GetUserAchievementProgressQuery, Result<List<AchievementProgressDto>>> {
  ...
  public async Task<Result<List<AchievementProgressDto>>> Handle(GetUserAchievementProgressQuery request, CancellationToken cancellationToken) {
    try {
      var query = _context.AchievementProgress.Include(ap => ap.Achievement).Where(ap => ap.UserId == request.UserId && ap.TenantId == request.TenantId);

      // Apply filters
      if (!string.IsNullOrEmpty(request.Category)) { query = query.Where(ap => ap.Achievement!.Category == request.Category); }

      if (request.OnlyInProgress) { query = query.Where(ap => ap.CurrentProgress > 0 && !ap.IsCompleted); }
...
/// <summary> Handler for getting user achievement summary </summary>
public class GetUserAchievementSummaryQueryHandler : IQueryHandler<GetUserAchievementSummaryQuery, Result<UserAchievementSummaryDto>> {
  ...
  public async Task<Result<UserAchievementSummaryDto>> Handle(GetUserAchievementSummaryQuery request, CancellationToken cancellationToken) {
    try {
      var userAchievements = await _context.UserAchievements.Include(ua => ua.Achievement).Where(ua => ua.UserId == request.UserId && ua.TenantId == request.TenantId).ToListAsync(cancellationToken);

      var progressRecords = await _context.AchievementProgress.Include(ap => ap.Achievement).Where(ap => ap.UserId == request.UserId && ap.TenantId == request.TenantId).ToListAsync(cancellationToken);
      ...
      // Recent achievements
      var recentAchievements = userAchievements.Where(ua => ua.IsCompleted)
      ...
      var achievementsByCategory = userAchievements.Where(ua => ua.Achievement != null).GroupBy(ua => ua.Achievement!.Category).ToDictionary(g => g.Key, g => g.Count());
...
/// <summary> Handler for getting available achievements for a user </summary>
public class GetAvailableAchievementsQueryHandler : IQueryHandler<GetAvailableAchievementsQuery, Result<AchievementsPageDto>> {
  ...
  public async Task<Result<AchievementsPageDto>> Handle(GetAvailableAchievementsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      var earnedAchievementIds = await _context.UserAchievements.Where(ua => ua.UserId == request.UserId && ua.TenantId == request.TenantId && ua.IsCompleted).Select(ua => ua.AchievementId).ToListAsync(cancellationToken);

      var query = _context.Achievements.Where(a => a.TenantId == request.TenantId && a.IsActive).Where(a => !earnedAchievementIds.Contains(a.Id)); // Exclude earned achievements

      // Apply filters
      if (!string.IsNullOrEmpty(request.Category)) { query = query.Where(a => a.Category == request.Category); }

      if (request.ExcludeSecret) { query = query.Where(a => !a.IsSecret); }
...
/// <summary> Handler for getting achievement leaderboard </summary>
public class GetAchievementLeaderboardQueryHandler : IQueryHandler<GetAchievementLeaderboardQuery, Result<List<UserAchievementLeaderboardDto>>> {
  ...
  public async Task<Result<List<UserAchievementLeaderboardDto>>> Handle(GetAchievementLeaderboardQuery request, CancellationToken cancellationToken) {
    try {
      var query = _context.UserAchievements.Include(ua => ua.User).Where(ua => ua.TenantId == request.TenantId);

      // Apply filters
      if (!string.IsNullOrEmpty(request.Category)) { query = query.Include(ua => ua.Achievement).Where(ua => ua.Achievement!.Category == request.Category); }

      var userStats = await query.GroupBy(ua => ua.UserId)
                                 .Select(g => new { UserId = g.Key, TotalAchievements = g.Count(), TotalPoints = g.Sum(ua => ua.PointsEarned), g.First().User })
...
/// <summary> Handler for getting achievement statistics </summary>
public class GetAchievementStatisticsQueryHandler : IQueryHandler<GetAchievementStatisticsQuery, Result<AchievementStatisticsDto>> {
  ...
  public async Task<Result<AchievementStatisticsDto>> Handle(GetAchievementStatisticsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      var lastEarned = await _context.UserAchievements.Where(ua => ua.AchievementId == request.AchievementId && ua.TenantId == request.TenantId && ua.IsCompleted)
                                     .OrderByDescending(ua => ua.EarnedAt)
                                     .Select(ua => ua.EarnedAt)
                                     .FirstOrDefaultAsync(cancellationToken);
...
/// <summary> Handler for checking achievement prerequisites </summary>
public class CheckAchievementPrerequisitesQueryHandler : IQueryHandler<CheckAchievementPrerequisitesQuery, Result<AchievementPrerequisiteCheckDto>> {
  ...
  public async Task<Result<AchievementPrerequisiteCheckDto>> Handle(CheckAchievementPrerequisitesQuery request, CancellationToken cancellationToken) {
    try {
      ...
      var userAchievements = await _context.UserAchievements.Where(ua => ua.UserId == request.UserId && ua.TenantId == request.TenantId && ua.IsCompleted).Select(ua => ua.AchievementId).ToListAsync(cancellationToken);
      ...
      foreach (var prerequisite in prerequisites) {
        ...
        if (!isMet) { canEarn = false; }

        prerequisiteStatuses.Add(new PrerequisiteStatusDto { PrerequisiteAchievementId = prerequisite.PrerequisiteAchievementId, Name = prerequisite.PrerequisiteAchievement?.Name ?? "Unknown", IsMet = isMet });
      }

      var result = new AchievementPrerequisiteCheckDto { AchievementId = request.AchievementId, CanEarn = canEarn, Prerequisites = prerequisiteStatuses };
