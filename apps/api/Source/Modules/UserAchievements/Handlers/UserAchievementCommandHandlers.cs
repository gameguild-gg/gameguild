  ...
  public async Task<Result<UserAchievement>> Handle(AwardAchievementCommand request, CancellationToken cancellationToken) {
    try {
      ...
      await _publisher.Publish(
        new AchievementEarnedEvent(userAchievement.Id) {
...
/// <summary> Handler for updating achievement progress </summary>
public class UpdateAchievementProgressCommandHandler : ICommandHandler<UpdateAchievementProgressCommand, Result<AchievementProgress>> {
  private readonly ApplicationDbContext _context;
  ...
  public UpdateAchievementProgressCommandHandler(ApplicationDbContext context, ILogger<UpdateAchievementProgressCommandHandler> logger, IMediator mediator, IPublisher publisher) {
    _context = context;
    _logger = logger;
    _mediator = mediator;
    _publisher = publisher;
  ...
  public async Task<Result<AchievementProgress>> Handle(UpdateAchievementProgressCommand request, CancellationToken cancellationToken) {
    try {
      ...
      await _publisher.Publish(
        new AchievementProgressUpdatedEvent(request.AchievementId) {
          UserId = request.UserId,
          AchievementId = request.AchievementId,
          AchievementName = achievement.Name,
      ...
      _logger.LogInformation("Updated achievement progress for user {UserId} on achievement {AchievementId}: {Progress}/{Target}", request.UserId, request.AchievementId, progress.CurrentProgress, progress.TargetProgress);
...
/// <summary> Handler for revoking achievements </summary>
public class RevokeAchievementCommandHandler : ICommandHandler<RevokeAchievementCommand, Result> {
  private readonly ApplicationDbContext _context;
  ...
  public RevokeAchievementCommandHandler(ApplicationDbContext context, ILogger<RevokeAchievementCommandHandler> logger, IPublisher publisher) {
    _context = context;
    _logger = logger;
    _publisher = publisher;
  }
  ...
  public async Task<Result> Handle(RevokeAchievementCommand request, CancellationToken cancellationToken) {
    try {
      ...
      var progress = await _context.AchievementProgress.FirstOrDefaultAsync(ap => ap.UserId == userAchievement.UserId && ap.AchievementId == userAchievement.AchievementId, cancellationToken);
      ...
      // Publish event
      await _publisher.Publish(
        new AchievementRevokedEvent(userAchievement.Id) {
          UserAchievementId = userAchievement.Id,
      ...
      _logger.LogInformation("Revoked achievement {AchievementName} from user {UserId}. Reason: {Reason}", achievement.Name, userAchievement.UserId, request.Reason);
