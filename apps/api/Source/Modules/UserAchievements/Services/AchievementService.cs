...
/// <summary> Service interface for achievement management </summary>
...
/// <summary> Service for achievement management and gamification logic </summary>
public class AchievementService : IAchievementService {
  ...
  public async Task<Result<AchievementProgress>> UpdateProgressAsync(Guid userId, Guid achievementId, int progressIncrement = 1, string? context = null, Guid? tenantId = null) {
    try {
      ...
      return Result.Failure<AchievementProgress>(CQRS.Error.Failure("UpdateProgress", "Failed to update achievement progress"));
    }
  }
  ...
  public async Task<Result<List<Achievement>>> GetEligibleAchievementsAsync(Guid userId, Guid? tenantId = null) {
    try {
      ...
      return Result.Failure<List<Achievement>>(CQRS.Error.Failure("GetEligibleAchievements", "Failed to get eligible achievements"));
    }
  }
  ...
  public async Task<Result<bool>> CheckPrerequisitesAsync(Guid userId, Guid achievementId, Guid? tenantId = null) {
    try {
      ...
      if (achievement == null) { return Result.Failure<bool>(CQRS.Error.NotFound("Achievement", "Achievement not found")); }
      ...
      return Result.Failure<bool>(CQRS.Error.Failure("CheckPrerequisites", "Failed to check prerequisites"));
    }
  }
  ...
  public async Task<Result<List<UserAchievement>>> GetUnnotifiedAchievementsAsync(Guid userId, Guid? tenantId = null) {
    try {
      ...
      return Result.Failure<List<UserAchievement>>(CQRS.Error.Failure("GetUnnotifiedAchievements", "Failed to get unnotified achievements"));
    }
  }
  ...
  public async Task<Result> MarkNotifiedAsync(Guid userAchievementId) {
    try {
      var command = new MarkAchievementNotifiedCommand {
        UserAchievementId = userAchievementId, UserId = Guid.Empty, // This should be set from user context
      };
      ...
      return Result.Failure(CQRS.Error.Failure("MarkAsNotified", "Failed to mark achievement as notified"));
    }
  }
