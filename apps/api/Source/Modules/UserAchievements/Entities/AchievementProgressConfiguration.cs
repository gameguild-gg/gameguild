...
internal class AchievementProgressConfiguration : IEntityTypeConfiguration<AchievementProgress> {
  public void Configure(EntityTypeBuilder<AchievementProgress> builder) {
    ...
    builder.HasOne(ap => ap.User).WithMany().HasForeignKey(ap => ap.UserId).IsRequired(false).OnDelete(DeleteBehavior.SetNull);

    // Configure the relationship with Achievement
    builder.HasOne(ap => ap.Achievement).WithMany().HasForeignKey(ap => ap.AchievementId).IsRequired().OnDelete(DeleteBehavior.Cascade);
  }
}
