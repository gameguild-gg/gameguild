    ...
    IRequestHandler<DeleteProductCommand, DeleteProductResult>
{
    ...
    public async Task<CreateProductResult> Handle(CreateProductCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return new CreateProductResult { Success = false, ErrorMessage = "User must be authenticated" };
            }

            // Check if user can create products
            if (!_userContext.IsInRole("Admin") && !_userContext.IsInRole("ContentCreator"))
            {
                return new CreateProductResult { Success = false, ErrorMessage = "User does not have permission to create products" };
            }
    ...
    public async Task<UpdateProductResult> Handle(UpdateProductCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (product == null)
            {
                return new UpdateProductResult { Success = false, ErrorMessage = "Product not found" };
            }

            // Check permissions
            if (!_userContext.IsInRole("Admin") && product.CreatorId != _userContext.UserId)
            {
                return new UpdateProductResult { Success = false, ErrorMessage = "User does not have permission to update this product" };
            }
            ...
            if (request.Visibility.HasValue)
                product.Visibility = request.Visibility.Value;
    ...
    public async Task<DeleteProductResult> Handle(DeleteProductCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (product == null)
            {
                return new DeleteProductResult { Success = false, ErrorMessage = "Product not found" };
            }

            // Check permissions
            if (!_userContext.IsInRole("Admin") && product.CreatorId != _userContext.UserId)
            {
                return new DeleteProductResult { Success = false, ErrorMessage = "User does not have permission to delete this product" };
            }
