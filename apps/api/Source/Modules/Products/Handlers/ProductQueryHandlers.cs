...
public class ProductQueryHandlers
  ...
  public async Task<Product?> Handle(GetProductByIdQuery request, CancellationToken cancellationToken) {
    try {
      ...
      var query = _context.Products.Where(p => p.Id == request.ProductId && p.DeletedAt == null);
  ...
  public async Task<IEnumerable<Product>> Handle(GetProductsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      if (!string.IsNullOrEmpty(request.SearchTerm)) {
        ...
        query = query.Where(p => p.Name.ToLower().Contains(searchTerm) || p.ShortDescription != null && p.ShortDescription.ToLower().Contains(searchTerm) || p.Title.ToLower().Contains(searchTerm));
      }
      ...
      // Apply pagination
      query = query.Skip(request.Skip).Take(Math.Min(request.Take, 100)); // Limit to prevent abuse
  ...
  public async Task<ProductStats> Handle(GetProductStatsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      if (request.ProductId.HasValue) { query = query.Where(p => p.Id == request.ProductId.Value); }
      ...
      var productsByType = await query.GroupBy(p => p.Type).Select(g => new { Type = g.Key.ToString(), Count = g.Count() }).ToDictionaryAsync(x => x.Type, x => x.Count, cancellationToken);
      ...
      if (_userContext.IsAuthenticated) {
        try {
          ...
          if (request.ProductId.HasValue) { userProductsQuery = userProductsQuery.Where(up => up.ProductId == request.ProductId.Value); }
          else {
          ...
          acquisitionsByType = await userProductsQuery.GroupBy(up => up.AcquisitionType).Select(g => new { Type = g.Key.ToString(), Count = g.Count() }).ToDictionaryAsync(x => x.Type, x => x.Count, cancellationToken);
      ...
      _logger.LogDebug("Retrieved product statistics: {TotalProducts} products, {PublishedProducts} published", stats.TotalProducts, stats.PublishedProducts);
  ...
  public async Task<IEnumerable<UserProduct>> Handle(GetUserProductsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      var query = _context.UserProducts.Include(up => up.Product).Where(up => up.UserId == request.UserId && up.Product.DeletedAt == null);

      // Apply optional filters
      if (request.AcquisitionType.HasValue) { query = query.Where(up => up.AcquisitionType == request.AcquisitionType.Value); }

      if (request.IsActive.HasValue) {
        if (request.IsActive.Value) { query = query.Where(up => up.AccessStatus == ProductAccessStatus.Active); }
        else { query = query.Where(up => up.AccessStatus != ProductAccessStatus.Active); }
      }
  ...
  private IQueryable<Product> ApplyAccessControl(IQueryable<Product> query) {
    ...
    var accessibleQuery = query.Where(p => p.Status == ContentStatus.Published && p.Visibility == AccessLevel.Public);
