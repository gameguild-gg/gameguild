    ...
    IRequestHandler<GetUserProductsQuery, IEnumerable<UserProduct>>
{
    ...
    public async Task<Product?> Handle(GetProductByIdQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                query = query.Include(p => p.ProductPricings);
            }

            if (request.IncludePrograms)
            {
                query = query.Include(p => p.ProductPrograms);
            }
            ...
            if (product == null)
            {
                _logger.LogWarning("Product not found or not accessible: {ProductId}", request.ProductId);
            }
    ...
    public async Task<IEnumerable<Product>> Handle(GetProductsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogDebug("Getting products with filters - Type: {Type}, Status: {Status}, Skip: {Skip}, Take: {Take}", 
                request.Type, request.Status, request.Skip, request.Take);
            ...
            {
                "name" => request.SortDirection?.ToUpper() == "DESC" ? 
                    query.OrderByDescending(p => p.Name) : query.OrderBy(p => p.Name),
                "createdat" => request.SortDirection?.ToUpper() == "DESC" ? 
    ...
    public async Task<IEnumerable<UserProduct>> Handle(GetUserProductsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // Apply access control
            if (!_userContext.IsInRole("Admin") && _userContext.UserId != request.UserId)
            {
                // Users can only see their own products unless they're an admin
