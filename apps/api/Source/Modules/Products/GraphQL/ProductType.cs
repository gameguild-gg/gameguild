...
public class ProductType : ObjectType<Product> {
  protected override void Configure(IObjectTypeDescriptor<ProductEntity> descriptor) {
    ...
    descriptor.Field(p => p.Type).Type<NonNullType<EnumType<ProductTypeEnum>>>().Description("The type of the product.");

    descriptor.Field(p => p.Status).Type<NonNullType<EnumType<ContentStatus>>>().Description("The publication status of the product.");

    descriptor.Field(p => p.Visibility).Type<NonNullType<EnumType<AccessLevel>>>().Description("The access level of the product.");
    ...
    descriptor.Field(p => p.Creator).Type<ObjectType<User>>().Description("The user who created this product.");

    descriptor.Field(p => p.Tenant).Type<ObjectType<Tenant>>().Description("The tenant this product belongs to.");
    ...
    descriptor.Field("canEdit")
              ...
              .Resolve(async context => {
                ...
                try {
                  ...
                  var hasResourcePermission = await permissionService.HasResourcePermissionAsync<ProductPermission, ProductEntity>(userId.Value, product.Tenant?.Id, product.Id, PermissionType.Edit);
                ...
                var hasContentTypePermission = await permissionService.HasContentTypePermissionAsync(userId.Value, product.Tenant?.Id, "Product", PermissionType.Edit);

                if (hasContentTypePermission) return true;

                // 3. Check tenant permission
                var hasTenantPermission = await permissionService.HasTenantPermissionAsync(userId.Value, product.Tenant?.Id, PermissionType.Edit);
    ...
    descriptor.Field("canDelete")
              ...
              .Resolve(async context => {
                ...
                try {
                  var hasResourcePermission = await permissionService.HasResourcePermissionAsync<ProductPermission, ProductEntity>(userId.Value, product.Tenant?.Id, product.Id, PermissionType.Delete);

                  if (hasResourcePermission) return true;
                }
                catch { }

                var hasContentTypePermission = await permissionService.HasContentTypePermissionAsync(userId.Value, product.Tenant?.Id, "Product", PermissionType.Delete);

                if (hasContentTypePermission) return true;

                var hasTenantPermission = await permissionService.HasTenantPermissionAsync(userId.Value, product.Tenant?.Id, PermissionType.Delete);
    ...
    descriptor.Field("canPublish")
              ...
              .Resolve(async context => {
                ...
                try {
                  var hasResourcePermission = await permissionService.HasResourcePermissionAsync<ProductPermission, ProductEntity>(userId.Value, product.Tenant?.Id, product.Id, PermissionType.Publish);

                  if (hasResourcePermission) return true;
                }
                catch { }

                var hasContentTypePermission = await permissionService.HasContentTypePermissionAsync(userId.Value, product.Tenant?.Id, "Product", PermissionType.Publish);

                if (hasContentTypePermission) return true;

                var hasTenantPermission = await permissionService.HasTenantPermissionAsync(userId.Value, product.Tenant?.Id, PermissionType.Publish);
