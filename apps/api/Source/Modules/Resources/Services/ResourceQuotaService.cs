...
public class ResourceQuotaService : IResourceQuotaService {
  ...
  public async Task<ResourceQuota> SetQuotaAsync(Guid tenantId, ResourceUsageType type, long? softLimit, long? hardLimit, ResourceQuotaPeriod period = ResourceQuotaPeriod.Monthly, CancellationToken cancellationToken = default) {
    var existingQuota = await _context.ResourceQuotas.FirstOrDefaultAsync(q => q.TenantId == tenantId && q.Type == type, cancellationToken);
    ...
    _logger.LogInformation("Set quota for tenant {TenantId}, type {Type}: Soft={SoftLimit}, Hard={HardLimit}", tenantId, type, softLimit, hardLimit);
  ...
  public async Task<IEnumerable<ResourceQuota>> GetTenantQuotasAsync(Guid tenantId, CancellationToken cancellationToken = default) {
    return await _context.ResourceQuotas.Where(q => q.TenantId == tenantId).OrderBy(q => q.Type).ToListAsync(cancellationToken);
  }
  ...
  public async Task<bool> RecordUsageAsync(Guid tenantId, ResourceUsageType type, long amount = 1, Guid? userId = null, string? source = null, Dictionary<string, string>? metadata = null, CancellationToken cancellationToken = default) {
    try {
      // Update quota usage
      var quota = await GetQuotaAsync(tenantId, type, cancellationToken);
      ...
      var usageRecord = await _context.ResourceUsageRecords.FirstOrDefaultAsync(r => r.TenantId == tenantId && r.Type == type && r.PeriodStart == today, cancellationToken);
      ...
      _logger.LogDebug("Recorded usage for tenant {TenantId}, type {Type}: amount={Amount}", tenantId, type, amount);
  ...
  public async Task<ResourceLimitCheckResponse> CheckLimitsAsync(Guid tenantId, ResourceUsageType type, long requestedAmount = 1, CancellationToken cancellationToken = default) {
    ...
    var message = softLimitExceeded ? $"Soft limit exceeded but within hard limit. Requested: {requestedAmount}, Current: {currentUsage}, Soft Limit: {quota.SoftLimit}" : "Within limits";

    return ResourceLimitCheckResponse.Success(type, currentUsage, quota.SoftLimit, quota.HardLimit, message);
  }

  public async Task<Dictionary<ResourceUsageType, ResourceLimitCheckResponse>> CheckMultipleLimitsAsync(Guid tenantId, Dictionary<ResourceUsageType, long> requestedAmounts, CancellationToken cancellationToken = default) {
  ...
  public async Task<MultiResourceUsageResponse> GetTenantUsageOverviewAsync(Guid tenantId, CancellationToken cancellationToken = default) {
    ...
    foreach (var quota in quotas) {
      ...
      else if (usage.SoftLimit.HasValue && usage.CurrentUsage >= usage.SoftLimit.Value) { response.SoftLimitExceeded.Add(quota.Type); }
    }
  ...
  public async Task<int> ResetExpiredQuotasAsync(CancellationToken cancellationToken = default) {
    var expiredQuotas = await _context.ResourceQuotas.Where(q => q.IsActive && q.LastReset.HasValue).ToListAsync(cancellationToken);
  ...
  public async Task<int> CleanupOldUsageRecordsAsync(DateTime olderThan, CancellationToken cancellationToken = default) {
    var oldRecords = await _context.ResourceUsageRecords.Where(r => r.PeriodEnd < olderThan).ToListAsync(cancellationToken);
  ...
  public async Task<bool> RecalculateUsageAsync(Guid tenantId, ResourceUsageType type, CancellationToken cancellationToken = default) {
    try {
      ...
      var totalUsage = await _context.ResourceUsageRecords.Where(r => r.TenantId == tenantId && r.Type == type && r.PeriodStart >= periodStart).SumAsync(r => r.Count, cancellationToken);
      ...
      _logger.LogInformation("Recalculated usage for tenant {TenantId}, type {Type}: {Usage}", tenantId, type, totalUsage);
