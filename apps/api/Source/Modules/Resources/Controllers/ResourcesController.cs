...
/// <summary>
/// API controller for resource quota management and usage tracking
/// </summary>
[ApiController]
...
public class ResourcesController : ControllerBase
{
    ...
    public ResourcesController(IResourceQuotaService resourceQuotaService)
    {
        _resourceQuotaService = resourceQuotaService;
    }

    /// <summary>
    /// Get resource usage overview for current tenant
    /// </summary>
    [HttpGet("usage")]
    ...
    /// </summary>
    [HttpGet("usage/{type}")]
    public async Task<ActionResult<ResourceUsageResponse>> GetResourceUsageDetails(
    ...
    /// <summary>
    /// Check if a resource usage request would exceed limits
    /// </summary>
    ...
    public async Task<ActionResult<Dictionary<ResourceUsageType, ResourceLimitCheckResponse>>> CheckMultipleLimits(
        [FromBody] Dictionary<ResourceUsageType, long> requestedAmounts)
    {
        var tenantId = User.GetTenantId();
    ...
    public async Task<ActionResult<ResourceLimitCheckResponse>> ConsumeResource([FromBody] ConsumeResourceRequest request)
    {
        ...
        var result = await _resourceQuotaService.TryConsumeResourceAsync(
            tenantId, request.Type, request.Amount, userId, request.Source);

        return Ok(result);
    }

    /// <summary>
    /// Record resource usage (for batch operations)
    /// </summary>
    ...
    public async Task<ActionResult<bool>> RecordUsage([FromBody] RecordUsageRequest request)
    {
        ...
        var success = await _resourceQuotaService.RecordUsageAsync(
            tenantId, request.Type, request.Amount, userId, request.Source, request.Metadata);
    ...
    public async Task<ActionResult<IEnumerable<ResourceUsageRecord>>> GetUsageHistory(
        ResourceUsageType type,
    ...
    public async Task<ActionResult<ResourceQuota>> SetQuota([FromBody] SetQuotaRequest request)
    {
        var quota = await _resourceQuotaService.SetQuotaAsync(
            request.TenantId, request.Type, request.SoftLimit, request.HardLimit, request.Period);
