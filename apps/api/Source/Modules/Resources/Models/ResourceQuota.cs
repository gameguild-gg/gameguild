...
public class ResourceQuota : EntityBase
{
    ...
    {
        return SoftLimit.HasValue && CurrentUsage >= SoftLimit.Value;
    }
    ...
    {
        return HardLimit.HasValue && CurrentUsage >= HardLimit.Value;
    }
    ...
    {
        if (!LastReset.HasValue || Period == ResourceQuotaPeriod.Never)
            return false;
