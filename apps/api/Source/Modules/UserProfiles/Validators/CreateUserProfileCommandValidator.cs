...
public class CreateUserProfileCommandValidator : AbstractValidator<CreateUserProfileCommand> {
  ...
  public CreateUserProfileCommandValidator(ApplicationDbContext context) {
    ...
    RuleFor(x => x.DisplayName).NotEmpty().WithMessage("Display name is required").Length(2, 100).WithMessage("Display name must be between 2 and 100 characters").MustAsync(BeUniqueDisplayName).WithMessage("Display name must be unique");
  ...
  private async Task<bool> BeUniqueDisplayName(string displayName, CancellationToken cancellationToken) {
    return !await _context.Resources.OfType<UserProfile>().AnyAsync(x => x.DisplayName == displayName && x.DeletedAt == null, cancellationToken);
  }

  private async Task<bool> BeValidUser(Guid userId, CancellationToken cancellationToken) { return await _context.Users.AnyAsync(x => x.Id == userId && x.DeletedAt == null, cancellationToken); }

  private async Task<bool> NotHaveExistingProfile(Guid userId, CancellationToken cancellationToken) { return !await _context.Resources.OfType<UserProfile>().AnyAsync(x => x.Id == userId && x.DeletedAt == null, cancellationToken); }
}
