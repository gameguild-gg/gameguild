...
/// <summary> Handles UserProfileDeletedEvent - manages cleanup and notifications </summary>
public class UserProfileDeletedEventHandler : IDomainEventHandler<UserProfileDeletedEvent> {
  ...
  public async Task Handle(UserProfileDeletedEvent domainEvent, CancellationToken cancellationToken) {
    _logger.LogInformation("User profile deleted: {UserProfileId} for user {UserId} (soft delete: {IsSoftDelete})", domainEvent.UserProfileId, domainEvent.UserId, domainEvent.IsSoftDelete);
