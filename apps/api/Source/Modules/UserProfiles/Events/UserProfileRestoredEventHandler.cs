...
/// <summary> Handles UserProfileRestoredEvent - manages restoration and notifications </summary>
public class UserProfileRestoredEventHandler : IDomainEventHandler<UserProfileRestoredEvent> {
  ...
  public async Task Handle(UserProfileRestoredEvent domainEvent, CancellationToken cancellationToken) {
    _logger.LogInformation("User profile restored: {UserProfileId} for user {UserId}", domainEvent.UserProfileId, domainEvent.UserId);
