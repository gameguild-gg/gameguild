...
public class GetUserProfileStatisticsHandler(ApplicationDbContext context, ILogger<GetUserProfileStatisticsHandler> logger) : IQueryHandler<GetUserProfileStatisticsQuery, Common.Result<UserProfileStatistics>> {
  public async Task<Common.Result<UserProfileStatistics>> Handle(GetUserProfileStatisticsQuery request, CancellationToken cancellationToken) {
    try {
      ...
      statistics.NewUserProfiles = await query.Where(up => up.CreatedAt >= fromDate && up.CreatedAt <= toDate).CountAsync(cancellationToken);
      statistics.UpdatedUserProfiles = await query.Where(up => up.UpdatedAt >= fromDate && up.UpdatedAt <= toDate && up.UpdatedAt != up.CreatedAt).CountAsync(cancellationToken);
      ...
      logger.LogError(ex, "ErrorMessage generating user profile statistics");

      return Result.Failure<UserProfileStatistics>(
        Common.ErrorMessage.Failure("UserProfile.StatisticsFailed", "Failed to generate user profile statistics")
      );
    }
  }
}
