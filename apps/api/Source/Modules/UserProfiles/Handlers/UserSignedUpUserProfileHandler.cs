...
) : INotificationHandler<GameGuild.Modules.Authentication.UserSignedUpNotification>
{
    public async Task Handle(GameGuild.Modules.Authentication.UserSignedUpNotification notification, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (result.IsSuccess)
            {
                logger.LogInformation("Successfully created UserProfile for user {UserId}", notification.UserId);
            }
            else
            {
                logger.LogWarning("Failed to create UserProfile for user {UserId}: {Error}", 
    ...
    private static (string GivenName, string FamilyName) ExtractNamesFromUserInfo(string username, string email)
    {
        ...
        if (!string.IsNullOrEmpty(email))
        {
            ...
            if (emailLocalPart.Contains('.'))
            {
                ...
                if (parts.Length >= 2)
                {
                    return (CapitalizeFirst(parts[0]), CapitalizeFirst(string.Join(" ", parts.Skip(1))));
                }
            }
