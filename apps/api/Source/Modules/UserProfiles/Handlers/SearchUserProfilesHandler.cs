    ...
    public async Task<GameGuild.Common.Result<IEnumerable<UserProfile>>> Handle(SearchUserProfilesQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (request.CreatedAfter.HasValue)
                query = query.Where(up => up.CreatedAt >= request.CreatedAfter.Value);

            if (request.CreatedBefore.HasValue)
                query = query.Where(up => up.CreatedAt <= request.CreatedBefore.Value);

            if (request.UpdatedAfter.HasValue)
                query = query.Where(up => up.UpdatedAt >= request.UpdatedAfter.Value);

            if (request.UpdatedBefore.HasValue)
                query = query.Where(up => up.UpdatedAt <= request.UpdatedBefore.Value);

            // Apply specific field filters
            if (!string.IsNullOrWhiteSpace(request.GivenName))
                query = query.Where(up => up.GivenName != null && up.GivenName.ToLower().Contains(request.GivenName.ToLower()));

            if (!string.IsNullOrWhiteSpace(request.FamilyName))
                query = query.Where(up => up.FamilyName != null && up.FamilyName.ToLower().Contains(request.FamilyName.ToLower()));

            if (!string.IsNullOrWhiteSpace(request.DisplayName))
                query = query.Where(up => up.DisplayName != null && up.DisplayName.ToLower().Contains(request.DisplayName.ToLower()));

            if (!string.IsNullOrWhiteSpace(request.Title))
                query = query.Where(up => up.Title != null && up.Title.ToLower().Contains(request.Title.ToLower()));
