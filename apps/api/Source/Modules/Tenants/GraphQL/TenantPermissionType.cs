...
/// <summary> GraphQL type for TenantPermission entity </summary>
public class TenantPermissionType : ObjectType<TenantPermission> {
  protected override void Configure(IObjectTypeDescriptor<TenantPermission> descriptor) {
    ...
    descriptor.Field(tp => tp.Id).Type<NonNullType<UuidType>>().Description("The unique identifier of the tenant permission");
    ...
    descriptor.Field(tp => tp.CreatedAt).Type<NonNullType<DateTimeType>>().Description("The date and time when the user joined the tenant");

    descriptor.Field(tp => tp.ExpiresAt).Type<DateTimeType>().Description("The date and time when the permission expires");

    descriptor.Field(tp => tp.PermissionFlags1).Type<NonNullType<LongType>>().Description("Permission flags for bits 0-63");

    descriptor.Field(tp => tp.PermissionFlags2).Type<NonNullType<LongType>>().Description("Permission flags for bits 64-127");
