...
public static class TenantDomainMappingExtensions
{
    ...
    {
        if (dto.TopLevelDomain != null)
            domain.TopLevelDomain = dto.TopLevelDomain;
        
        if (dto.Subdomain != null)
            domain.Subdomain = dto.Subdomain;
        
        if (dto.IsMainDomain.HasValue)
            domain.IsMainDomain = dto.IsMainDomain.Value;
        
        if (dto.IsSecondaryDomain.HasValue)
            domain.IsSecondaryDomain = dto.IsSecondaryDomain.Value;
        
        if (dto.UserGroupId.HasValue)
            domain.UserGroupId = dto.UserGroupId.Value;
    ...
    public static TenantUserGroup UpdateTenantUserGroup(this UpdateTenantUserGroupDto dto, TenantUserGroup userGroup)
    {
        ...
        if (dto.ParentGroupId.HasValue)
            userGroup.ParentGroupId = dto.ParentGroupId;
        
        if (dto.IsActive.HasValue)
            userGroup.IsActive = dto.IsActive.Value;
        
        if (dto.IsDefault.HasValue)
            userGroup.IsDefault = dto.IsDefault.Value;
