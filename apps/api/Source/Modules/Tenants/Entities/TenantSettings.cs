...
public class TenantSettings : Resource {
  ...
  /// <summary> Date format preference (e.g., "MM/dd/yyyy", "dd/MM/yyyy") </summary>
  [MaxLength(20)]
  public string DateFormat { get; set; } = "MM/dd/yyyy";

  /// <summary> Time format preference (12-hour vs 24-hour) </summary>
  ...
  /// <summary> Tenant logo URL or base64 data </summary>
  [MaxLength(2000)]
  public string? LogoUrl { get; set; }

  /// <summary> Custom CSS/styling overrides </summary>
  [Column(TypeName = "TEXT")]
  public string? CustomCss { get; set; }

  /// <summary> Default theme ("light", "dark", "auto") </summary>
  [MaxLength(10)]
  public string DefaultTheme { get; set; } = "auto";

  // === FEATURE FLAGS/SETTINGS ===

  /// <summary> JSON configuration for tenant-specific feature flags </summary>
  [Column(TypeName = "JSON")]
  public string? FeatureFlags { get; set; } = "{}";

  /// <summary> JSON configuration for module-specific settings </summary>
  [Column(TypeName = "JSON")]
  ...
  /// <summary> Default notification email address </summary>
  [MaxLength(255)]
  public string? DefaultNotificationEmail { get; set; }

  // === SECURITY SETTINGS ===

  /// <summary> Require two-factor authentication for all users </summary>
  public bool RequireTwoFactorAuth { get; set; }

  /// <summary> Password minimum length requirement </summary>
  public int MinPasswordLength { get; set; } = 8;

  /// <summary> Password complexity requirements (JSON) </summary>
  [Column(TypeName = "JSON")]
  ...
  /// <summary> Maximum number of users allowed for this tenant (null = unlimited) </summary>
  public int? MaxUsers { get; set; }

  /// <summary> Storage quota in MB (null = unlimited) </summary>
  public long? StorageQuotaMB { get; set; }

  /// <summary> Subscription plan/tier </summary>
  [MaxLength(50)]
  public string? SubscriptionPlan { get; set; }

  /// <summary> Subscription expires at (null = no expiration) </summary>
  public DateTime? SubscriptionExpiresAt { get; set; }

  // === CONTACT INFORMATION ===

  /// <summary> Support email for this tenant </summary>
  [MaxLength(255)]
  ...
  /// <summary> Physical address (JSON structure) </summary>
  [Column(TypeName = "JSON")]
  public string? Address { get; set; }

  // === HELPER METHODS ===

  /// <summary> Get feature flag value as boolean </summary>
  public bool GetFeatureFlag(string key, bool defaultValue = false) {
    if (string.IsNullOrEmpty(FeatureFlags)) return defaultValue;

    try {
      var flags = JsonSerializer.Deserialize<Dictionary<string, object>>(FeatureFlags);

      return flags?.TryGetValue(key, out var value) == true && value is bool boolValue ? boolValue : defaultValue;
    }
    catch { return defaultValue; }
  }

  /// <summary> Set feature flag value </summary>
  public void SetFeatureFlag(string key, bool value) {
    var flags = string.IsNullOrEmpty(FeatureFlags) ? new Dictionary<string, object>() : JsonSerializer.Deserialize<Dictionary<string, object>>(FeatureFlags) ?? new Dictionary<string, object>();
  ...
  /// <summary> Get module setting value </summary>
  public T? GetModuleSetting<T>(string module, string key, T? defaultValue = default) {
    ...
    try {
      var settings = JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, object>>>(ModuleSettings);

      return settings?.TryGetValue(module, out var moduleDict) == true && moduleDict.TryGetValue(key, out var value) && value is T typedValue ? typedValue : defaultValue;
    }
    catch { return defaultValue; }
  }

  /// <summary> Set module setting value </summary>
  public void SetModuleSetting<T>(string module, string key, T value) {
    var settings = string.IsNullOrEmpty(ModuleSettings)
                     ? new Dictionary<string, Dictionary<string, object>>()
                     : JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, object>>>(ModuleSettings) ?? new Dictionary<string, Dictionary<string, object>>();

    if (!settings.ContainsKey(module)) { settings[module] = new Dictionary<string, object>(); }

    settings[module][key] = value!;
    ModuleSettings = JsonSerializer.Serialize(settings);
    Touch();
  }

  /// <summary> Create default settings for a tenant </summary>
