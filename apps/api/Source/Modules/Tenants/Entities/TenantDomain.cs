...
public class TenantDomain : EntityBase {
  ...
  /// <summary> The top-level domain name (e.g., "champlain.edu", "university.edu") </summary>
  [Required]
  [MaxLength(255)]
  ...
  /// <summary> Whether this is the main/principal domain for the tenant (only one per tenant) </summary>
  ...
  /// <summary> Navigation property to the user group </summary>
  ...
  [NotMapped]
  public string FullDomainName { get => string.IsNullOrEmpty(Subdomain) ? TopLevelDomain : $"{Subdomain}.{TopLevelDomain}"; }

  /// <summary> Gets the domain type as a string for display purposes </summary>
  [NotMapped]
  ...
  /// <summary> Checks if an email matches this domain </summary>
