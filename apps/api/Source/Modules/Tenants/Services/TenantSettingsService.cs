...
public class TenantSettingsService : ITenantSettingsService {
  ...
  public async Task<TenantSettings> GetTenantSettingsAsync(Guid? tenantId = null, CancellationToken cancellationToken = default) {
    var settings = await _context.TenantSettings.Include(s => s.Tenant).FirstOrDefaultAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);
  ...
  public async Task<TenantSettings> SaveTenantSettingsAsync(Guid? tenantId, TenantSettings settings, CancellationToken cancellationToken = default) {
    ...
    var existing = await _context.TenantSettings.FirstOrDefaultAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);
    ...
    foreach (var update in updates) {
      ...
      if (property != null && property.CanWrite) {
        try {
          ...
          _logger.LogDebug("Updated property {PropertyName} to {Value} for tenant {TenantId}", update.Key, update.Value, tenantId);
        }
        catch (Exception ex) { _logger.LogWarning(ex, "Failed to update property {PropertyName} for tenant {TenantId}", update.Key, tenantId); }
      }
    }
  ...
  public async Task DeleteTenantSettingsAsync(Guid tenantId, CancellationToken cancellationToken = default) {
    var settings = await _context.TenantSettings.FirstOrDefaultAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);
  ...
  public async Task SetModuleSettingAsync<T>(Guid? tenantId, string module, string key, T value, CancellationToken cancellationToken = default) {
    ...
    _logger.LogInformation("Set module setting {Module}.{Key} to {Value} for tenant {TenantId}", module, key, value, tenantId);
  }
  ...
  public async Task<Dictionary<string, object>> GetModuleSettingsAsync(Guid? tenantId, string module, CancellationToken cancellationToken = default) {
    ...
    try {
      ...
      _logger.LogWarning(ex, "Failed to deserialize module settings for tenant {TenantId}, module {Module}", tenantId, module);
  ...
  public async Task<bool> TenantSettingsExistAsync(Guid? tenantId, CancellationToken cancellationToken = default) { return await _context.TenantSettings.AnyAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken); }
  ...
  public async Task<TenantSettings> GetEffectiveTenantSettingsAsync(Guid tenantId, CancellationToken cancellationToken = default) {
    ...
    var tenantSettings = await _context.TenantSettings.Include(s => s.Tenant).FirstOrDefaultAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);
