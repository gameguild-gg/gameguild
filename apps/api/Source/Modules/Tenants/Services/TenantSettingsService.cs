...
/// <summary>
/// Implementation of tenant settings service
/// Provides complete tenant settings management with caching and fallback support
/// </summary>
public class TenantSettingsService : ITenantSettingsService
{
    ...
    public TenantSettingsService(ApplicationDbContext context, ILogger<TenantSettingsService> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<TenantSettings> GetTenantSettingsAsync(Guid? tenantId = null, CancellationToken cancellationToken = default)
    {
        var settings = await _context.TenantSettings
            .Include(s => s.Tenant)
            .FirstOrDefaultAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);

        if (settings == null)
        {
            _logger.LogDebug("No settings found for tenant {TenantId}, creating defaults", tenantId);
    ...
    public async Task<TenantSettings> SaveTenantSettingsAsync(Guid? tenantId, TenantSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Saved settings for tenant {TenantId}", tenantId);
        ...
        foreach (var update in updates)
        {
            ...
            if (property != null && property.CanWrite)
            {
                try
                {
                    ...
                    _logger.LogDebug("Updated property {PropertyName} to {Value} for tenant {TenantId}",
                        update.Key, update.Value, tenantId);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to update property {PropertyName} for tenant {TenantId}",
                        update.Key, tenantId);
                }
            }
        }
    ...
    public async Task<TenantSettings> ResetTenantSettingsAsync(Guid? tenantId = null, CancellationToken cancellationToken = default)
    {
        ...
        TenantSettings defaultSettings = TenantSettings.CreateDefault(tenantId);
    ...
    public async Task DeleteTenantSettingsAsync(Guid tenantId, CancellationToken cancellationToken = default)
    {
        ...
        if (settings != null)
        {
            settings.Delete();
            await _context.SaveChangesAsync(cancellationToken);
            _logger.LogInformation("Deleted settings for tenant {TenantId}", tenantId);
        }
    }
    ...
    public async Task SetFeatureFlagAsync(Guid? tenantId, string key, bool value, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Set feature flag {FlagKey} to {Value} for tenant {TenantId}", key, value, tenantId);
    }
    ...
    public async Task<Dictionary<string, bool>> GetAllFeatureFlagsAsync(Guid? tenantId, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(settings.FeatureFlags))
        {
        ...
        try
        {
            var flags = JsonSerializer.Deserialize<Dictionary<string, object>>(settings.FeatureFlags) ??
                       new Dictionary<string, object>();

            return flags.Where(kvp => kvp.Value is bool)
                       .ToDictionary(kvp => kvp.Key, kvp => (bool)kvp.Value);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to deserialize feature flags for tenant {TenantId}", tenantId);
            return new Dictionary<string, bool>();
        }
    }
    ...
    public async Task SetModuleSettingAsync<T>(Guid? tenantId, string module, string key, T value, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Set module setting {Module}.{Key} to {Value} for tenant {TenantId}",
            module, key, value, tenantId);
    }
    ...
    public async Task<Dictionary<string, object>> GetModuleSettingsAsync(Guid? tenantId, string module, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(settings.ModuleSettings))
        {
        ...
        try
        {
            ...
            return allSettings?.TryGetValue(module, out var moduleSettings) == true ?
                   moduleSettings : new Dictionary<string, object>();
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to deserialize module settings for tenant {TenantId}, module {Module}",
                tenantId, module);
            return new Dictionary<string, object>();
        }
    }
    ...
    public async Task<bool> TenantSettingsExistAsync(Guid? tenantId, CancellationToken cancellationToken = default)
    {
        return await _context.TenantSettings
            .AnyAsync(s => s.TenantId == tenantId && s.DeletedAt == null, cancellationToken);
    }
    ...
    public async Task<TenantSettings> CloneTenantSettingsAsync(Guid? sourceTenantId, Guid targetTenantId, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ArgumentException($"Target tenant with ID {targetTenantId} not found", nameof(targetTenantId));
        }
