...
/// <summary> Handler for updating an existing tenant </summary>
public class UpdateTenantHandler(ApplicationDbContext context, ILogger<UpdateTenantHandler> logger, IDomainEventPublisher eventPublisher) : ICommandHandler<UpdateTenantCommand, Result<Tenant>> {
  public async Task<Result<Tenant>> Handle(UpdateTenantCommand request, CancellationToken cancellationToken) {
    try {
      ...
      if (!string.IsNullOrEmpty(request.Name) && tenant.Name != request.Name) {
        var existingTenant = await context.Resources.OfType<Tenant>().FirstOrDefaultAsync(t => t.Name == request.Name && t.Id != request.Id && t.DeletedAt == null, cancellationToken);
      ...
      if (!string.IsNullOrEmpty(request.Slug) && tenant.Slug != request.Slug) {
        var existingSlug = await context.Resources.OfType<Tenant>().FirstOrDefaultAsync(t => t.Slug == request.Slug && t.Id != request.Id && t.DeletedAt == null, cancellationToken);
      ...
      await eventPublisher.PublishAsync(new TenantUpdatedEvent(tenant.Id, tenant.Name, tenant.Description, tenant.IsActive, tenant.Slug), cancellationToken);
      ...
      return Result.Failure<Tenant>(Error.Failure("Tenant.UpdateFailed", "Failed to update tenant"));
    }
  }
}
