...
/// <summary> Handler for bulk deleting tenants </summary>
public class BulkDeleteTenantsHandler(ApplicationDbContext context, ILogger<BulkDeleteTenantsHandler> logger, IDomainEventPublisher eventPublisher) : ICommandHandler<BulkDeleteTenantsCommand, Result<int>> {
  public async Task<Result<int>> Handle(BulkDeleteTenantsCommand request, CancellationToken cancellationToken) {
    try {
      ...
      return Result.Failure<int>(Error.Failure("Tenant.BulkDeleteFailed", "Failed to bulk delete tenants"));
    }
  }
}
