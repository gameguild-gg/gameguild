...
/// <summary> Handler for creating a new tenant </summary>
public class CreateTenantHandler(ApplicationDbContext context, ILogger<CreateTenantHandler> logger, IDomainEventPublisher eventPublisher) : ICommandHandler<CreateTenantCommand, Result<Tenant>> {
  public async Task<Result<Tenant>> Handle(CreateTenantCommand request, CancellationToken cancellationToken) {
    try {
      ...
      var existingTenant = await context.Resources.OfType<Tenant>().FirstOrDefaultAsync(t => t.Name == request.Name && t.DeletedAt == null, cancellationToken);
      ...
      var existingSlug = await context.Resources.OfType<Tenant>().FirstOrDefaultAsync(t => t.Slug == request.Slug && t.DeletedAt == null, cancellationToken);
      ...
      var tenant = new Tenant { Name = request.Name, Description = request.Description, IsActive = request.IsActive, Slug = request.Slug };
      ...
      await eventPublisher.PublishAsync(new TenantCreatedEvent(tenant.Id, tenant.Name, tenant.Description, tenant.IsActive, tenant.Slug), cancellationToken);
      ...
      return Result.Failure<Tenant>(Error.Failure("Tenant.CreateFailed", "Failed to create tenant"));
    }
  }
}
