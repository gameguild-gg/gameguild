...
) : IFeatureFlagAnalyticsService
{
    ...
    public async Task RecordUsageAsync(string featureKey, FeatureContext context, bool wasEnabled, string? value = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var usage = new FeatureFlagUsage
            {
                ...
                Environment = context.Environment ?? _options.DefaultEnvironment,
            ...
            await _analyticsRepository.RecordUsageAsync(usage, cancellationToken);
        }
        catch (Exception ex)
        {
    ...
    public async Task<FeatureFlagAnalytics> GetAnalyticsAsync(string featureKey, DateTime? startDate = null, DateTime? endDate = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var usageRecords = await _analyticsRepository.GetUsageAnalyticsAsync(featureKey, start, end, cancellationToken);

            var stats = await _analyticsRepository.GetAggregatedStatsAsync(featureKey, start, end, cancellationToken);
    ...
    {
        ArgumentNullException.ThrowIfNull(featureKeys, nameof(featureKeys));
        ...
        try
        {
            var tasks = keysList.Select(async key =>
                {
                    var analytics = await GetAnalyticsAsync(key, start, end, cancellationToken);
    ...
    public async Task<TenantFeatureAnalytics> GetTenantAnalyticsAsync(Guid tenantId, DateTime? startDate = null, DateTime? endDate = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var usageRecords = await _analyticsRepository.GetUsageByTenantAsync(tenantId, start, end, cancellationToken);
            ...
            {
                var featureFlag = await _queryRepository.GetByIdAsync(group.Key, cancellationToken);
    ...
    public async Task<IEnumerable<FeatureUsageRanking>> GetTopFeaturesAsync(int topCount = 10, DateTime? startDate = null, DateTime? endDate = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var topFeatureKeys = await _analyticsRepository.GetMostAccessedFeaturesAsync(topCount, start, end, cancellationToken);
    ...
    public async Task<AnalyticsExportResult> ExportAnalyticsAsync(AnalyticsExportRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Get data to export
            var featureKeys = request.FeatureKeys?.ToList() ?? (await _queryRepository.GetAllAsync(cancellationToken)).Select(f => f.Key).ToList();

            var analyticsData = await GetBulkAnalyticsAsync(featureKeys, start, end, cancellationToken);
    ...
    private static AnalyticsExportResult ExportAsCsv(IDictionary<string, FeatureFlagAnalytics> analyticsData, AnalyticsExportRequest request)
    {
