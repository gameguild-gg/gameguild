    ...
    : IFeatureFlagConfigurationService
{
    ...
    public async Task<FeatureFlagConfig?> GetConfigAsync(string featureKey, string? environment = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var featureFlag = await GetCachedFeatureFlagAsync(featureKey, cancellationToken);
    ...
    public async Task<IEnumerable<FeatureFlagConfig>> GetAllConfigsAsync(string? environment = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var featureFlags = await GetCachedFeatureFlagsByEnvironmentAsync(environment, cancellationToken);
    ...
    public async Task<IDictionary<string, FeatureFlagConfig>> GetConfigsAsync(IEnumerable<string> featureKeys, string? environment = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var featureFlags = await _queryRepository.GetByKeysAsync(keysList, cancellationToken);
    ...
    public async Task<string> GetConfigHashAsync(string? environment = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var configs = await GetAllConfigsAsync(environment, cancellationToken);
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(currentHash, nameof(currentHash));

        try
        {
            var latestHash = await GetConfigHashAsync(environment, cancellationToken);
    ...
    {
        if (!_options.EnableCaching || cache == null) { return await _queryRepository.GetByKeyAsync(key, cancellationToken); }
        ...
        return await cache.GetOrCreateAsync(
            ...
            async entry =>
            {
                ...
                return await _queryRepository.GetByKeyAsync(key, cancellationToken);
            }
        );
    }
    ...
    {
        if (!_options.EnableCaching || cache == null) { return await _queryRepository.GetByEnvironmentAsync(environment, cancellationToken); }
        ...
        return await cache.GetOrCreateAsync(
            ...
            async entry =>
            {
                ...
                return await _queryRepository.GetByEnvironmentAsync(environment, cancellationToken);
            }
        ) ?? [];
    }
