...
) : IFeatureFlagManagementService
{
    ...
    public async Task<Guid> CreateFeatureFlagAsync(CreateFeatureFlagRequest request, CancellationToken cancellationToken = default)
    {
        ...
        ArgumentException.ThrowIfNullOrWhiteSpace(request.Key, nameof(request.Key));
        ...
        try
        {
            ...
            var existing = await _queryRepository.GetByKeyAsync(request.Key, cancellationToken);
            ...
            var featureFlag = new FeatureFlag
            {
                ...
                Type = (FeatureFlagType) request.Type,
                DefaultValue = request.DefaultValue,
                EnabledValue = request.DefaultValue, // Start with default
                IsEnabled = request.IsEnabled,
                RolloutPercentage = request.RolloutPercentage,
                Environment = request.Environment ?? _options.DefaultEnvironment,
                IsGlobal = true // Default to global
            };

            var id = await _queryRepository.AddAsync(featureFlag, cancellationToken);
    ...
    public async Task UpdateFeatureFlagAsync(Guid featureFlagId, UpdateFeatureFlagRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var featureFlag = await _queryRepository.GetByIdAsync(featureFlagId, cancellationToken);
            ...
            await _queryRepository.UpdateAsync(featureFlag, cancellationToken);
    ...
    public async Task DeleteFeatureFlagAsync(Guid featureFlagId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await _targetingRepository.DeleteTargetsByFeatureFlagAsync(featureFlagId, cancellationToken);

            // Delete the feature flag
            await _queryRepository.RemoveAsync(featureFlagId, cancellationToken);
            await _queryRepository.SaveChangesAsync(cancellationToken);
    ...
    /// <inheritdoc />
    public async Task EnableFeatureAsync(Guid featureFlagId, CancellationToken cancellationToken = default) { await SetFeatureEnabledStateAsync(featureFlagId, true, cancellationToken); }

    /// <inheritdoc />
    public async Task DisableFeatureAsync(Guid featureFlagId, CancellationToken cancellationToken = default) { await SetFeatureEnabledStateAsync(featureFlagId, false, cancellationToken); }
    ...
    {
        ArgumentNullException.ThrowIfNull(request, nameof(request));
        ArgumentException.ThrowIfNullOrWhiteSpace(request.TargetType, nameof(request.TargetType));
        ArgumentException.ThrowIfNullOrWhiteSpace(request.TargetIdentifier, nameof(request.TargetIdentifier));
        ...
        try
        {
            ...
            var featureFlag = await _queryRepository.GetByIdAsync(request.FeatureFlagId, cancellationToken);
            ...
            var targetId = await _targetingRepository.CreateTargetAsync(target, cancellationToken);
    ...
    public async Task UpdateTargetingRuleAsync(Guid targetId, FeatureFlagTargetingRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var target = await _targetingRepository.GetTargetByIdAsync(targetId, cancellationToken);
            ...
            await _targetingRepository.UpdateTargetAsync(target, cancellationToken);
    ...
    public async Task DeleteTargetingRuleAsync(Guid targetId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await _targetingRepository.DeleteTargetAsync(targetId, cancellationToken);
    ...
    {
        try { return await _queryRepository.GetByIdAsync(featureFlagId, cancellationToken); }
        catch (Exception ex)
        {
    ...
    public async Task<FeatureFlag?> GetByKeyAsync(string featureKey, CancellationToken cancellationToken = default)
    {
        ...
        try { return await _queryRepository.GetByKeyAsync(featureKey, cancellationToken); }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<FeatureFlag>> GetAllAsync(string? environment = null, bool enabledOnly = false, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(environment)) { features = await _queryRepository.GetByEnvironmentAsync(environment, cancellationToken); }
            else if (enabledOnly) { features = await _queryRepository.GetEnabledAsync(cancellationToken); }
            else { features = await _queryRepository.GetAllAsync(cancellationToken); }
