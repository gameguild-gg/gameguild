...
namespace GameGuild.Features.Services.Utilities;
...
public static class RolloutHashCalculator
{
    ...
    public static bool IsInRollout(string identifier, int percentage, string? salt = null)
    {
        ...
        if (percentage is < FeatureFlagConstants.MinRolloutPercentage or > FeatureFlagConstants.MaxRolloutPercentage)
        {
            throw new ArgumentOutOfRangeException(nameof(percentage), percentage, $"Percentage must be between {FeatureFlagConstants.MinRolloutPercentage} and {FeatureFlagConstants.MaxRolloutPercentage}");
        }
    ...
    private static uint CalculateBucket(string hashInput)
    {
        ...
        try
        {
            ...
            var bytesWritten = SHA256.HashData(inputBytes, buffer.AsSpan(0, 32));
