...
public class TenantDomainService : ITenantDomainService {
  ...
  public async Task<TenantDomain?> GetDomainByIdAsync(Guid id) { return await _context.TenantDomains.Include(d => d.Tenant).Include(d => d.UserGroup).FirstOrDefaultAsync(d => d.Id == id && d.DeletedAt == null); }

  public async Task<IEnumerable<TenantDomain>> GetDomainsByTenantAsync(Guid tenantId) {
    return await _context.TenantDomains.Where(d => d.TenantId == tenantId && d.DeletedAt == null).Include(d => d.UserGroup).OrderBy(d => d.IsMainDomain ? 0 : 1).ThenBy(d => d.TopLevelDomain).ToListAsync();
  }
  ...
  public async Task<bool> SetMainDomainAsync(Guid tenantId, Guid domainId) {
    var domain = await _context.TenantDomains.FirstOrDefaultAsync(d => d.Id == domainId && d.TenantId == tenantId && d.DeletedAt == null);
  ...
  private async Task UnsetMainDomainsForTenantAsync(Guid tenantId) {
    var mainDomains = await _context.TenantDomains.Where(d => d.TenantId == tenantId && d.IsMainDomain && d.DeletedAt == null).ToListAsync();
  ...
  public async Task<TenantUserGroup?> GetUserGroupByIdAsync(Guid id) {
    return await _context.TenantUserGroups.Include(g => g.Tenant).Include(g => g.ParentGroup).Include(g => g.SubGroups).Include(g => g.Domains).FirstOrDefaultAsync(g => g.Id == id && g.DeletedAt == null);
  }

  public async Task<IEnumerable<TenantUserGroup>> GetUserGroupsByTenantAsync(Guid tenantId) {
    return await _context.TenantUserGroups.Where(g => g.TenantId == tenantId && g.DeletedAt == null).Include(g => g.ParentGroup).Include(g => g.SubGroups).OrderBy(g => g.Name).ToListAsync();
  }

  public async Task<IEnumerable<TenantUserGroup>> GetRootUserGroupsByTenantAsync(Guid tenantId) {
    return await _context.TenantUserGroups.Where(g => g.TenantId == tenantId && g.ParentGroupId == null && g.DeletedAt == null).Include(g => g.SubGroups).OrderBy(g => g.Name).ToListAsync();
  }
  ...
  public async Task<TenantUserGroupMembership> AddUserToGroupAsync(Guid userId, Guid userGroupId, bool isAutoAssigned = false) {
    ...
    var existingMembership = await _context.TenantUserGroupMemberships.FirstOrDefaultAsync(m => m.UserId == userId && m.UserGroupId == userGroupId);
    ...
    return await _context.TenantUserGroupMemberships.Include(m => m.User).Include(m => m.UserGroup).FirstAsync(m => m.Id == membership.Id);
  }

  public async Task<bool> RemoveUserFromGroupAsync(Guid userId, Guid userGroupId) {
    var membership = await _context.TenantUserGroupMemberships.FirstOrDefaultAsync(m => m.UserId == userId && m.UserGroupId == userGroupId);
  ...
  public async Task<IEnumerable<TenantUserGroupMembership>> GetUserGroupMembershipsAsync(Guid userId) {
    return await _context.TenantUserGroupMemberships.Where(m => m.UserId == userId).Include(m => m.UserGroup).ThenInclude(g => g.Tenant).OrderBy(m => m.UserGroup.Name).ToListAsync();
  }

  public async Task<IEnumerable<TenantUserGroupMembership>> GetGroupMembersAsync(Guid userGroupId) {
    return await _context.TenantUserGroupMemberships.Where(m => m.UserGroupId == userGroupId).Include(m => m.User).OrderBy(m => m.User.Name).ToListAsync();
  }
  ...
  public async Task<int> AutoAssignTenantUsersAsync(Guid tenantId) {
    ...
    var users = await _context.Users.Where(u => u.DeletedAt == null && domainStrings.Any(domain => u.Email.ToLower().EndsWith("@" + domain))).ToListAsync();
  ...
  public async Task<IEnumerable<UserModel>> GetUsersInGroupAsync(Guid userGroupId, bool includeSubGroups = false) {
    ...
    return await _context.TenantUserGroupMemberships.Where(m => groupIds.Contains(m.UserGroupId)).Select(m => m.User).Distinct().OrderBy(u => u.Name).ToListAsync();
  }

  public async Task<IEnumerable<TenantUserGroup>> GetUserGroupsForUserAsync(Guid userId) {
    return await _context.TenantUserGroupMemberships.Where(m => m.UserId == userId).Select(m => m.UserGroup).Include(g => g.Tenant).Include(g => g.ParentGroup).OrderBy(g => g.Tenant.Name).ThenBy(g => g.Name).ToListAsync();
  }
