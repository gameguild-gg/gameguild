...
/// <summary>
/// Infrastructure implementation of the three-layer permission service
/// Implements Clean Architecture and Domain-Driven Design principles
/// </summary>
public class PermissionService : IPermissionService {
    ...
    public async Task<TenantPermission> GrantTenantPermissionAsync(Guid? userId, Guid? tenantId, PermissionType[] permissions) {
        ...
        if (existingPermission != null) {
            ...
            _logger.LogInformation("Updated tenant permissions for User:{UserId}, Tenant:{TenantId}, Permissions:{Permissions}",
                userId, tenantId, combinedPermissions);
        }
        else {
            ...
            _logger.LogInformation("Granted new tenant permissions for User:{UserId}, Tenant:{TenantId}, Permissions:{Permissions}",
                userId, tenantId, combinedPermissions);
        }
    ...
    public async Task<List<TenantPermission>> BulkGrantTenantPermissionAsync(Guid[] userIds, Guid tenantId, PermissionType[] permissions) {
        ...
        _logger.LogInformation("Bulk granted tenant permissions for {UserCount} users in Tenant:{TenantId}, Permissions:{Permissions}",
            userIds.Length, tenantId, combinedPermissions);
    ...
    public async Task<bool> HasTenantPermissionAsync(Guid? userId, Guid? tenantId, PermissionType permission) {
        ...
        if (tenantId.HasValue) {
            var tenantDefault = await _context.TenantPermissions
                .AsNoTracking()
                .FirstOrDefaultAsync(tp => tp.UserId == null &&
                                         tp.TenantId == tenantId &&
                                         tp.DeletedAt == null);
    ...
    public async Task<IEnumerable<PermissionType>> GetTenantPermissionsAsync(Guid? userId, Guid? tenantId) {
        ...
        if (userId.HasValue) {
            ...
            if (userPermission != null)
                AddPermissionFlags(permissions, userPermission.Permissions);
        }
    ...
    public async Task<ContentTypePermission> GrantContentTypePermissionAsync(Guid userId, Guid? tenantId, string contentTypeName, PermissionType[] permissions) {
        if (string.IsNullOrWhiteSpace(contentTypeName))
            throw new ArgumentException("Content type name cannot be empty", nameof(contentTypeName));
        ...
        _logger.LogInformation("Granted content type permissions for User:{UserId}, Tenant:{TenantId}, ContentType:{ContentType}, Permissions:{Permissions}",
            userId, tenantId, contentTypeName, combinedPermissions);
    ...
    public async Task<IEnumerable<PermissionType>> GetContentTypePermissionsAsync(Guid userId, Guid? tenantId, string contentTypeName) {
        ...
        if (contentTypePermission != null)
            AddPermissionFlags(permissions, contentTypePermission.Permissions);
    ...
    public async Task<TPermission> GrantResourcePermissionAsync<TPermission, TResource>(
        Guid userId, Guid? tenantId, Guid resourceId, PermissionType[] permissions)
        ...
        _logger.LogInformation("Granted resource permissions for User:{UserId}, Tenant:{TenantId}, Resource:{ResourceId}, Type:{ResourceType}, Permissions:{Permissions}",
            userId, tenantId, resourceId, typeof(TResource).Name, combinedPermissions);

        return existingPermission;
    }

    public async Task<bool> HasResourcePermissionAsync<TPermission, TResource>(
        Guid userId, Guid? tenantId, Guid resourceId, PermissionType permission)
        where TPermission : ResourcePermission<TResource>, new()
        where TResource : EntityBase {
    ...
    public async Task<IEnumerable<PermissionType>> GetResourcePermissionsAsync<TPermission, TResource>(
        ...
        if (resourcePermission != null)
            AddPermissionFlags(permissions, resourcePermission.Permissions);

        return permissions;
    }

    public async Task BulkGrantResourcePermissionAsync<TPermission, TResource>(
        Guid userId, Guid? tenantId, Guid[] resourceIds, PermissionType[] permissions)
        where TPermission : ResourcePermission<TResource>, new()
        where TResource : EntityBase {
        if (resourceIds == null || resourceIds.Length == 0)
            throw new ArgumentException("At least one resource ID must be specified", nameof(resourceIds));
        ...
        var existingPermissions = await _context.Set<TPermission>()
            .Where(rp => resourceIds.Contains(rp.ResourceId) &&
                        rp.UserId == userId &&
                        rp.TenantId == tenantId &&
                        rp.DeletedAt == null)
            .ToListAsync();
        ...
        _logger.LogInformation("Bulk granted resource permissions for User:{UserId}, Tenant:{TenantId}, Resources:{ResourceCount}, Type:{ResourceType}, Permissions:{Permissions}",
            userId, tenantId, resourceIds.Length, typeof(TResource).Name, combinedPermissions);
    }

    public async Task ShareResourceAsync<TPermission, TResource>(
        Guid resourceId, Guid targetUserId, Guid? tenantId, PermissionType[] permissions, DateTime? expiresAt = null)
        where TPermission : ResourcePermission<TResource>, new()
        where TResource : EntityBase {
        var resourcePermission = await GrantResourcePermissionAsync<TPermission, TResource>(
            targetUserId, tenantId, resourceId, permissions);
        ...
        _logger.LogInformation("Shared resource {ResourceId} with User:{TargetUserId}, Permissions:{Permissions}, ExpiresAt:{ExpiresAt}",
            resourceId, targetUserId, permissions, expiresAt);
    }

    public async Task RevokeResourceAccessAsync<TPermission, TResource>(
        Guid userId, Guid? tenantId, Guid resourceId)
        where TPermission : ResourcePermission<TResource>, new()
        where TResource : EntityBase {
        var resourcePermission = await _context.Set<TPermission>()
        ...
        if (resourcePermission != null) {
            ...
            _logger.LogInformation("Revoked resource access for User:{UserId}, Tenant:{TenantId}, Resource:{ResourceId}",
    ...
    private static void AddPermissionFlags(HashSet<PermissionType> permissions, PermissionType permissionFlags) {
        foreach (PermissionType permission in Enum.GetValues<PermissionType>()) {
            if ((permissionFlags & permission) == permission)
