...
/// <summary>
/// Enhanced DAC Permission Resolver implementing three-layer permission system
...
public class DacPermissionResolver : IDacPermissionResolver {
    ...
    public async Task<PermissionResult> ResolvePermissionAsync<TResource>(
        ...
        try {
            ...
            _logger.LogDebug(
                "Permission {Permission} resolved for user {UserId} in tenant {TenantId}: {IsGranted} from {Source}",
                permission, userId, tenantId, hierarchy.FinalResult.IsGranted, hierarchy.FinalResult.Source);

            return hierarchy.FinalResult;
        }
        catch (Exception ex) {
            _logger.LogError(ex,
                "Error resolving permission {Permission} for user {UserId} in tenant {TenantId}",
                permission, userId, tenantId);

            return new PermissionResult {
                IsGranted = false,
                Source = PermissionSource.None,
                Reason = "Error during permission resolution",
                Priority = 0
            };
        }
    }
    ...
    public async Task<IEnumerable<EffectivePermission>> GetEffectivePermissionsAsync<TResource>(
        ...
        try {
            ...
            _logger.LogError(ex,
                "Error getting effective permissions for user {UserId} in tenant {TenantId}",
                userId, tenantId);
    ...
    public async Task<bool> CanGrantPermissionsAsync(
        ...
        try {
            ...
            foreach (var permission in permissions) {
                var canGrant = await HasPermissionInContext(
                    grantorUserId, tenantId, permission, resourceId, contentTypeName);
            ...
            _logger.LogError(ex,
                "Error checking grant permissions for user {UserId} in tenant {TenantId}",
                grantorUserId, tenantId);
            return false;
        }
    }
    ...
    public async Task<PermissionHierarchy> GetPermissionHierarchyAsync<TResource>(
        ...
        try {
            ...
            if (tenantId.HasValue) {
                await AddTenantDefaultLayer(layers, tenantId.Value, permission);
            }

            // Layer 3: Content Type Defaults (Priority 3)
            if (contentTypeName != null) {
                await AddContentTypeDefaultLayer(layers, tenantId, contentTypeName, permission);
            }

            // Layer 4: User Tenant Permissions (Priority 4)
            if (tenantId.HasValue) {
                await AddUserTenantLayer(layers, userId, tenantId.Value, permission);
            }

            // Layer 5: User Content Type Permissions (Priority 5)
            if (contentTypeName != null) {
                await AddUserContentTypeLayer(layers, userId, tenantId, contentTypeName, permission);
            }

            // Layer 6: Resource Defaults (Priority 6)
            if (resourceId.HasValue) {
                await AddResourceDefaultLayer<TResource>(layers, tenantId, resourceId.Value, permission);
            }

            // Layer 7: User Resource Permissions (Priority 7 - Highest)
            if (resourceId.HasValue) {
                await AddUserResourceLayer<TResource>(layers, userId, tenantId, resourceId.Value, permission);
            }
            ...
            _logger.LogError(ex,
                "Error building permission hierarchy for user {UserId} in tenant {TenantId}",
                userId, tenantId);

            hierarchy.FinalResult = new PermissionResult {
                IsGranted = false,
                Source = PermissionSource.None,
                Reason = "Error building permission hierarchy",
                Priority = 0
            };
    ...
    public async Task<Dictionary<Guid, Dictionary<PermissionType, PermissionResult>>> BulkResolvePermissionsAsync<TResource>(
        ...
        try {
            ...
            _logger.LogError(ex,
                "Error in bulk permission resolution for user {UserId} in tenant {TenantId}",
                userId, tenantId);
    ...
    private async Task<bool> HasPermissionInContext(
        Guid userId,
        Guid? tenantId,
        PermissionType permission,
        Guid? resourceId,
        ...
        if (resourceId.HasValue) {
            return await _permissionService.HasResourcePermissionAsync<ResourcePermission<EntityBase>, EntityBase>(
                userId, tenantId, resourceId.Value, permission);
        }

        if (contentTypeName != null) {
            return await _permissionService.HasContentTypePermissionAsync(
                userId, tenantId, contentTypeName, permission);
        }
    ...
    private async Task AddContentTypeDefaultLayer(
    ...
    private async Task AddUserTenantLayer(
        List<PermissionLayer> layers,
        Guid userId,
        Guid tenantId,
        PermissionType permission) {
    ...
    private async Task AddUserContentTypeLayer(
        List<PermissionLayer> layers,
        Guid userId,
        Guid? tenantId,
        string contentTypeName,
    ...
    private async Task AddResourceDefaultLayer<TResource>(
        List<PermissionLayer> layers,
        Guid? tenantId,
        Guid resourceId,
        PermissionType permission) where TResource : EntityBase {

        var resourceDefaults = await _permissionService.GetResourcePermissionsAsync<ResourcePermission<TResource>, TResource>(
            null, tenantId, resourceId);
    ...
    private async Task AddUserResourceLayer<TResource>(
        List<PermissionLayer> layers,
        Guid userId,
        Guid? tenantId,
        Guid resourceId,
    ...
    private static PermissionLayer? DetermineFinalResult(List<PermissionLayer> layers) {
        return layers
