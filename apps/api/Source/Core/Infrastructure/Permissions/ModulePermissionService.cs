...
public class ModulePermissionService : IModulePermissionService {
    ...
    public ModulePermissionService(
        ...
        ILogger<ModulePermissionService> logger) {
    ...
    public async Task<UserRoleAssignment> AssignRoleAsync(
        Guid userId,
        Guid? tenantId,
        ModuleType module,
        string roleName,
        ...
        try {
            ...
            _logger.LogInformation(
                "Assigned role {RoleName} to user {UserId} for module {Module} in tenant {TenantId}",
            ...
            _logger.LogError(ex,
                "Error assigning role {RoleName} to user {UserId} for module {Module} in tenant {TenantId}",
                roleName, userId, module, tenantId);
            throw;
        }
    }
    ...
    public async Task<bool> RevokeRoleAsync(Guid userId, Guid? tenantId, ModuleType module, string roleName) {
        ...
        try {
            ...
            if (assignmentsToRemove.Count != 0) {
                ...
                _logger.LogInformation(
                    "Revoked role {RoleName} from user {UserId} for module {Module} in tenant {TenantId}",
                    roleName, userId, module, tenantId);
                return true;
            }
            ...
            _logger.LogError(ex,
                "Error revoking role {RoleName} from user {UserId} for module {Module} in tenant {TenantId}",
                roleName, userId, module, tenantId);
            throw;
        }
    }
    ...
    public async Task<List<UserRoleAssignment>> GetUserRolesAsync(Guid userId, Guid? tenantId, ModuleType module) {
        try {
            ...
            _logger.LogError(ex,
                "Error getting roles for user {UserId} in module {Module} for tenant {TenantId}",
                userId, module, tenantId);
            return [];
        }
    }
    ...
    public async Task<List<UserRoleAssignment>> GetUsersWithRoleAsync(Guid? tenantId, ModuleType module, string roleName) {
        ...
        try {
            ...
            _logger.LogError(ex,
                "Error getting users with role {RoleName} in module {Module} for tenant {TenantId}",
                roleName, module, tenantId);
            return [];
        }
    }
    ...
    public async Task<bool> HasModulePermissionAsync(
        ...
        try {
            ...
            foreach (var roleAssignment in userRoles) {
                ...
                if (hasPermission) {
                    // Check constraints
                    if (await CheckConstraintsAsync(roleAssignment.Constraints.ToList(), userId, tenantId, resourceId)) {
                        return true;
                    }
                }
            }
            ...
            _logger.LogError(ex,
                "Error checking module permission {Action} for user {UserId} in module {Module} for tenant {TenantId}",
                action, userId, module, tenantId);
            return false;
        }
    }

    public async Task<List<ModulePermissionDefinition>> GetUserModulePermissionsAsync(
        Guid userId,
        Guid? tenantId,
        ModuleType module) {
        ...
        try {
            ...
            foreach (var roleAssignment in userRoles) {
                ...
                var rolePermissions = roleDefinition.Permissions
                    .Where(p => p.IsGranted && (p.ExpiresAt == null || p.ExpiresAt > DateTime.UtcNow))
                    .ToList();
            ...
            _logger.LogError(ex,
                "Error getting module permissions for user {UserId} in module {Module} for tenant {TenantId}",
                userId, module, tenantId);
            return [];
        }
    }

    public async Task<List<Guid>> GetUsersWithPermissionAsync(
        Guid? tenantId,
        ModuleType module,
        ModuleAction action) {
        ...
        try {
            ...
            _logger.LogError(ex,
                "Error getting users with permission {Action} in module {Module} for tenant {TenantId}",
                action, module, tenantId);
            return [];
        }
    }
    ...
    public async Task<bool> CanCreateTestingSessionsAsync(Guid userId, Guid? tenantId) =>
        await HasModulePermissionAsync(userId, tenantId, ModuleType.TestingLab, ModuleAction.CreateSession);

    public async Task<bool> CanDeleteTestingSessionsAsync(Guid userId, Guid? tenantId) =>
        await HasModulePermissionAsync(userId, tenantId, ModuleType.TestingLab, ModuleAction.DeleteSession);

    public async Task<bool> CanManageTestersAsync(Guid userId, Guid? tenantId) =>
        await HasModulePermissionAsync(userId, tenantId, ModuleType.TestingLab, ModuleAction.ManageTesters);

    public async Task<bool> CanViewTestingReportsAsync(Guid userId, Guid? tenantId) =>
        await HasModulePermissionAsync(userId, tenantId, ModuleType.TestingLab, ModuleAction.ViewReports);

    public async Task<bool> CanExportTestingDataAsync(Guid userId, Guid? tenantId) =>
        await HasModulePermissionAsync(userId, tenantId, ModuleType.TestingLab, ModuleAction.ExportData);
    ...
    public async Task<TestingLabPermissions> GetUserTestingLabPermissionsAsync(Guid userId, Guid? tenantId) {
        try {
            ...
            var constraints = userRoles
                .SelectMany(r => r.Constraints)
                .ToList();
            ...
            _logger.LogError(ex,
                "Error getting Testing Lab permissions for user {UserId} in tenant {TenantId}",
                userId, tenantId);
            return new TestingLabPermissions();
        }
    }
    ...
    public async Task<ModuleRole> CreateRoleDefinitionAsync(
        string roleName,
        ModuleType module,
        string description,
        List<ModulePermissionDefinition> permissions,
        int priority = 0) {
        ...
        try {
            ...
            _logger.LogInformation(
                "Created role definition {RoleName} for module {Module}",
                roleName, module);
            ...
            _logger.LogError(ex,
                "Error creating role definition {RoleName} for module {Module}",
                roleName, module);
            throw;
        }
    }

    public async Task<ModuleRole> UpdateRoleDefinitionAsync(
        string roleName,
        ModuleType module,
        List<ModulePermissionDefinition> permissions) {
        ...
        try {
            ...
            _logger.LogInformation(
                "Updated role definition {RoleName} for module {Module}",
                roleName, module);
            ...
            _logger.LogError(ex,
                "Error updating role definition {RoleName} for module {Module}",
                roleName, module);
            throw;
        }
    }
    ...
    public async Task<List<ModuleRole>> GetModuleRoleDefinitionsAsync(ModuleType module) {
        try {
            ...
            _logger.LogError(ex,
                "Error getting role definitions for module {Module}",
                module);
            return [];
        }
    }
    ...
    public async Task<bool> DeleteRoleDefinitionAsync(string roleName, ModuleType module) {
        ...
        try {
            ...
            if (hasAssignments) {
                _logger.LogWarning(
                    "Cannot delete role {RoleName} for module {Module} because it has active assignments",
                    roleName, module);
                return false;
            }
            ...
            _logger.LogInformation(
                "Deleted role definition {RoleName} for module {Module}",
                roleName, module);
            ...
            _logger.LogError(ex,
                "Error deleting role definition {RoleName} for module {Module}",
                roleName, module);
            throw;
        }
    }
    ...
    private async Task<bool> CheckConstraintsAsync(
        ...
        try {
            ...
            foreach (var constraint in constraints) {
                if (!await EvaluateConstraint(constraint, userId, tenantId, resourceId)) {
                    return false;
                }
            }
            ...
            _logger.LogError(ex,
                "Error checking constraints for user {UserId} in tenant {TenantId}",
                userId, tenantId);
            return false;
        }
    }

    private async Task<bool> EvaluateConstraint(
    ...
    private async Task EnsureTestingLabDefaultRoles() {
        ...
        foreach (var roleInfo in testingLabRoles) {
            var existingRole = await _context.ModuleRoles
                .FirstOrDefaultAsync(r => r.Name == roleInfo.Name && r.Module == ModuleType.TestingLab);

            if (existingRole == null) {
                var permissions = roleInfo.Permissions.Select(action => new ModulePermissionDefinition {
