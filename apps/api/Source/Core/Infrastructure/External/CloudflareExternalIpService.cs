...
public class CloudflareExternalIpService : ICloudflareExternalIpService, IDisposable {
    ...
    public CloudflareExternalIpService(
        ...
        if (_isEnabled) {
            ...
            _logger.LogInformation(
                "Cloudflare Dynamic DNS service initialized with {IntervalMinutes} minute interval",
    ...
    public async Task UpdateExternalIpAsync(CancellationToken cancellationToken = default) {
        ...
        try {
            ...
            _logger.LogInformation(
                "External IP address changed from {OldIp} to {NewIp}. Updating DNS records...",
                _lastKnownIp ?? "unknown", currentIp);
            ...
            if (success) {
                ...
                _logger.LogError("Failed to update Cloudflare DNS records");
            }
        }
        catch (Exception ex) {
            _logger.LogError(ex, "Error during external IP update process");
        }
        finally {
            _semaphore.Release();
        }
    ...
    public async Task<string?> GetExternalIpAsync(CancellationToken cancellationToken = default) {
        ...
        foreach (var service in services) {
            try {
                ...
                _logger.LogWarning(ex, "Failed to get IP from service {Service}", service);
            }
        }
    ...
    private void ValidateConfiguration() {
        _isEnabled = !string.IsNullOrWhiteSpace(_options.ApiToken) &&
                    !string.IsNullOrWhiteSpace(_options.ZoneId) &&
                    _options.Records != null &&
                    _options.Records.Count != 0;
        ...
        _logger.LogInformation(
            "Cloudflare Dynamic DNS service configured for zone {ZoneId} with {RecordCount} records",
            _options.ZoneId, _options.Records.Count);
    }
    ...
    private static string CleanIpAddress(string response) {
    ...
    private async Task<bool> UpdateSingleRecordAsync(
        ...
        try {
            ...
            if (!response.IsSuccessStatusCode) {
                _logger.LogError(
                    "Cloudflare API returned error {StatusCode} for record {RecordName}: {Response}",
                    response.StatusCode, record.Name, responseBody);
                return false;
            }
    ...
    private async Task<string?> GetRecordIdAsync(string recordName, CancellationToken cancellationToken) {
        try {
            ...
            if (!response.IsSuccessStatusCode) {
                _logger.LogError(
                    "Failed to get record ID for {RecordName}. Status: {StatusCode}, Response: {Response}",
                    recordName, response.StatusCode, responseBody);
                return null;
            }
