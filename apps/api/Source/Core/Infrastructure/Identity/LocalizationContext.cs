...
public class LocalizationContext : ILocalizationContext {
  ...
  public LocalizationContext(IUserContext userContext, ITenantContext tenantContext, ILogger<LocalizationContext> logger) {
  ...
  public CultureInfo CurrentCulture {
    get {
      ...
      if (_userContext.Claims.TryGetValue("culture", out var userCulture) && !string.IsNullOrEmpty(userCulture?.ToString())) {
        ...
        catch (CultureNotFoundException) { _logger.LogWarning("Invalid culture '{Culture}' found in user context", userCulture); }
      }
      ...
      if (_tenantContext.Settings.TryGetValue("DefaultCulture", out var tenantCulture) && !string.IsNullOrEmpty(tenantCulture?.ToString())) {
        ...
        catch (CultureNotFoundException) { _logger.LogWarning("Invalid culture '{Culture}' found in tenant context", tenantCulture); }
      }
  ...
  public TimeZoneInfo CurrentTimeZone {
    get {
      ...
      if (_userContext.Claims.TryGetValue("timezone", out var userTimezone) && !string.IsNullOrEmpty(userTimezone?.ToString())) {
        ...
        catch (TimeZoneNotFoundException) { _logger.LogWarning("Invalid timezone '{TimeZone}' found in user context", userTimezone); }
      }
      ...
      if (_tenantContext.Settings.TryGetValue("DefaultTimeZone", out var tenantTimezone) && !string.IsNullOrEmpty(tenantTimezone?.ToString())) {
        ...
        catch (TimeZoneNotFoundException) { _logger.LogWarning("Invalid timezone '{TimeZone}' found in tenant context", tenantTimezone); }
      }
  ...
  public DateTime ConvertToLocalTime(DateTime utcTime) {
    if (utcTime.Kind != DateTimeKind.Utc) { _logger.LogWarning("Converting non-UTC time {Time} to local time, this may produce incorrect results", utcTime); }
