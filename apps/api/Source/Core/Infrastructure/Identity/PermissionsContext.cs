...
public class PermissionsContext : IPermissionsContext {
    ...
    public async Task<bool> HasContentTypePermissionAsync(PermissionType permission, string contentType, Guid? tenantId = null) {
        ...
        try {
            return await _permissionService.HasContentTypePermissionAsync(UserId.Value, effectiveTenantId, contentType, permission);
        }
        catch (Exception ex) {
            _logger.LogWarning(ex, "Error checking content type permission {Permission} for user {UserId} in tenant {TenantId} for content type {ContentType}",
              permission, UserId, effectiveTenantId, contentType);
            return false;
        }
    }
    ...
    public async Task<bool> HasResourcePermissionAsync(PermissionType permission, string resourceType, Guid resourceId, Guid? tenantId = null) {
        ...
        try {
            return await _permissionService.HasResourcePermissionAsync(UserId.Value, effectiveTenantId, resourceType, resourceId, permission);
        }
        catch (Exception ex) {
            _logger.LogWarning(ex, "Error checking resource permission {Permission} for user {UserId} in tenant {TenantId} for resource {ResourceType}:{ResourceId}",
              permission, UserId, effectiveTenantId, resourceType, resourceId);
            return false;
        }
    }

    public async Task<bool> HasAnyTenantPermissionAsync(PermissionType[] permissions, Guid? tenantId = null) {
        if (permissions?.Length > 0 != true) return false;
