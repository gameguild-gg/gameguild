...
/// Provides access to current tenant information from claims, headers, and query parameters
/// </summary>
public class TenantContext : GameGuild.Core.Domain.Identity.ITenantContext {
    ...
    public string? TenantName => _user?.FindFirst("tenant_name")?.Value ??
    ...
    public IDictionary<string, object> Settings {
        get {
            ...
            if (_user != null) {
                foreach (var claim in _user.Claims.Where(c => c.Type.StartsWith("tenant_"))) {
                    settings[claim.Type] = claim.Value;
                }
            }
    ...
    public string? SubscriptionPlan => _user?.FindFirst("subscription_plan")?.Value ??
                                       _user?.FindFirst("tenant_plan")?.Value;
}
