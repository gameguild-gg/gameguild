...
/// <summary> Interface for the three-layer permission service Layer 1: Tenant-wide permissions with default support Layer 2: Content-Type permissions (to be implemented) Layer 3: Resource-specific permissions (to be implemented) </summary>
public interface IPermissionService {
  // ===== LAYER 1: TENANT-WIDE PERMISSIONS =====

  /// <summary> Grant permissions to a user in a tenant, or set default permissions </summary>
  ...
  /// <summary> Grant permissions to multiple users in a tenant efficiently in a single transaction </summary>
  ...
  /// <summary> Add user to a tenant </summary>
  Task<TenantPermission> JoinTenantAsync(Guid userId, Guid tenantId);

  /// <summary> Remove user from a tenant </summary>
  Task LeaveTenantAsync(Guid userId, Guid tenantId);

  /// <summary> Check if user is a member of a tenant </summary>
  Task<bool> IsUserInTenantAsync(Guid userId, Guid tenantId);

  /// <summary> Update user's membership expiration in a tenant </summary>
  ...
  /// <summary> Grant content-type permissions to a user in a tenant, or set default permissions </summary>
  /// <param name="userId"> User ID (null for default permissions) </param>
  /// <param name="tenantId"> Tenant ID (null for global defaults) </param>
  ...
  /// <summary> Check if user has a specific content-type permission </summary>
  ...
  /// <summary> Get all content-type permissions for a user in a tenant </summary>
  /// <param name="userId"> User ID (null for default permissions) </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="contentTypeName"> Name of the content type </param>
  Task<IEnumerable<PermissionType>> GetContentTypePermissionsAsync(Guid? userId, Guid? tenantId, string contentTypeName);

  /// <summary> Revoke specific content-type permissions from a user in a tenant </summary>
  /// <param name="userId"> User ID (null for default permissions) </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="contentTypeName"> Name of the content type </param>
  ...
  /// <summary> Grant resource-specific permissions to a user </summary>
  ...
  /// <param name="permissions"> Permissions to grant </param>
  Task GrantResourcePermissionAsync<TPermission, TResource>(Guid? userId, Guid? tenantId, Guid resourceId, PermissionType[ ] permissions) where TPermission : ResourcePermission<TResource>, new() where TResource : EntityBase;

  /// <summary> Check if user has a specific resource permission </summary>
  /// <typeparam name="TPermission"> The resource permission type </typeparam>
  /// <typeparam name="TResource"> The resource entity type </typeparam>
  /// <param name="userId"> User ID </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="resourceId"> ID of the specific resource </param>
  /// <param name="permission"> Permission to check </param>
  Task<bool> HasResourcePermissionAsync<TPermission, TResource>(Guid userId, Guid? tenantId, Guid resourceId, PermissionType permission) where TPermission : ResourcePermission<TResource> where TResource : EntityBase;

  /// <summary> Get all resource permissions for a user on a specific resource </summary>
  /// <typeparam name="TPermission"> The resource permission type </typeparam>
  /// <typeparam name="TResource"> The resource entity type </typeparam>
  /// <param name="userId"> User ID (null for default permissions) </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="resourceId"> ID of the specific resource </param>
  Task<IEnumerable<PermissionType>> GetResourcePermissionsAsync<TPermission, TResource>(Guid? userId, Guid? tenantId, Guid resourceId) where TPermission : ResourcePermission<TResource> where TResource : EntityBase;

  /// <summary> Revoke specific resource permissions from a user </summary>
  /// <typeparam name="TPermission"> The resource permission type </typeparam>
  /// <typeparam name="TResource"> The resource entity type </typeparam>
  /// <param name="userId"> User ID (null for default permissions) </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="resourceId"> ID of the specific resource </param>
  /// <param name="permissions"> Permissions to revoke </param>
  Task RevokeResourcePermissionAsync<TPermission, TResource>(Guid? userId, Guid? tenantId, Guid resourceId, PermissionType[ ] permissions) where TPermission : ResourcePermission<TResource> where TResource : EntityBase;

  /// <summary> Get resource permissions for multiple resources at once (bulk operation) </summary>
  /// <typeparam name="TPermission"> The resource permission type </typeparam>
  /// <typeparam name="TResource"> The resource entity type </typeparam>
  /// <param name="userId"> User ID </param>
  /// <param name="tenantId"> Tenant ID </param>
  /// <param name="resourceIds"> IDs of the resources </param>
  Task<Dictionary<Guid, IEnumerable<PermissionType>>> GetBulkResourcePermissionsAsync<TPermission, TResource>(Guid userId, Guid? tenantId, Guid[ ] resourceIds) where TPermission : ResourcePermission<TResource> where TResource : EntityBase;

  /// <summary> Share a resource with another user with specific permissions </summary>
  /// <typeparam name="TPermission"> The resource permission type </typeparam>
  /// <typeparam name="TResource"> The resource entity type </typeparam>
  ...
  /// <summary> Grant permissions to multiple resources efficiently in a single transaction </summary>
  ...
  /// <summary> Get the tenant context for a specific resource </summary>
  Task<Guid?> GetResourceTenantIdAsync(Guid resourceId);

  /// <summary> Get the content type name for a specific resource </summary>
