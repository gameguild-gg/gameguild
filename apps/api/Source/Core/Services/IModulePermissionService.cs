...
/// <summary> Service for managing module-based permissions Provides granular permission management for different modules like TestingLab, Projects, etc. </summary>
public interface IModulePermissionService {
  ...
  /// <summary> Check if a user has a specific permission in a module </summary>
  Task<bool> HasModulePermissionAsync(Guid userId, Guid? tenantId, ModuleType module, ModuleAction action, Guid? resourceId = null);

  /// <summary> Get all effective permissions for a user in a module </summary>
  ...
  /// <summary> Check if user can create testing sessions </summary>
  Task<bool> CanCreateTestingSessionsAsync(Guid userId, Guid? tenantId);

  /// <summary> Check if user can delete testing sessions </summary>
  Task<bool> CanDeleteTestingSessionsAsync(Guid userId, Guid? tenantId);

  /// <summary> Check if user can manage testers </summary>
  Task<bool> CanManageTestersAsync(Guid userId, Guid? tenantId);

  /// <summary> Check if user can view testing reports </summary>
  Task<bool> CanViewTestingReportsAsync(Guid userId, Guid? tenantId);

  /// <summary> Check if user can export testing data </summary>
  Task<bool> CanExportTestingDataAsync(Guid userId, Guid? tenantId);

  /// <summary> Get all Testing Lab permissions for a user </summary>
  ...
  /// <summary> Update permissions for an existing role definition </summary>
  ...
  /// <summary> Delete a role definition (only if no users are assigned to it) </summary>
...
public class TestingLabPermissions {
  ...
  public List<PermissionConstraint> Constraints { get; set; } = new List<PermissionConstraint>();
}
