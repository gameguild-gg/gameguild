...
/// <summary> Implementation of module-based permission service using EntityBase Framework </summary>
public class ModulePermissionService(ApplicationDbContext context, ILogger<ModulePermissionService> logger) : IModulePermissionService {
  ...
  public async Task<List<UserRoleAssignment>> GetUserRolesAsync(Guid userId, Guid? tenantId, ModuleType module) {
    return await _context.UserRoleAssignments.Where(r => r.UserId == userId && r.TenantId == tenantId && r.Module == module && r.IsActive).Where(r => r.ExpiresAt == null || r.ExpiresAt > DateTime.UtcNow).ToListAsync();
  }

  public async Task<List<UserRoleAssignment>> GetUsersWithRoleAsync(Guid? tenantId, ModuleType module, string roleName) {
    return await _context.UserRoleAssignments.Where(r => r.TenantId == tenantId && r.Module == module && r.RoleName == roleName && r.IsActive).Where(r => r.ExpiresAt == null || r.ExpiresAt > DateTime.UtcNow).ToListAsync();
  }
  ...
  public async Task<bool> HasModulePermissionAsync(Guid userId, Guid? tenantId, ModuleType module, ModuleAction action, Guid? resourceId = null) {
    ...
    foreach (var roleAssignment in userRoles) {
      ...
      if (hasPermission) {
        // Check constraints
        if (await CheckConstraintsAsync(roleAssignment.Constraints.ToList(), userId, tenantId, resourceId).ConfigureAwait(false)) { return true; }
      }
    }
  ...
  public async Task<List<ModulePermissionDefinition>> GetUserModulePermissionsAsync(Guid userId, Guid? tenantId, ModuleType module) {
    ...
    foreach (var roleAssignment in userRoles) {
      ...
      if (roleDefinition != null) { permissions.AddRange(roleDefinition.Permissions.Where(p => p.IsGranted)); }
    }
  ...
  public async Task<List<Guid>> GetUsersWithPermissionAsync(Guid? tenantId, ModuleType module, ModuleAction action) {
    ...
    foreach (var assignment in roleAssignments) {
      ...
      if (roleDefinition?.Permissions.Any(p => p.Module == module && p.Action == action && p.IsGranted) == true) { usersWithPermission.Add(assignment.UserId); }
    }
  ...
  public async Task<ModuleRole> CreateRoleDefinitionAsync(string roleName, ModuleType module, string description, List<ModulePermissionDefinition> permissions, int priority = 0) {
    ...
    var role = new ModuleRole { Name = roleName, Module = module, Description = description, Permissions = permissions, Priority = priority, IsSystemRole = false };
  ...
  public async Task<ModuleRole> UpdateRoleDefinitionAsync(string roleName, ModuleType module, List<ModulePermissionDefinition> permissions) {
    ...
    if (role.IsSystemRole) { throw new InvalidOperationException($"Cannot modify system role {roleName}"); }
  ...
  public async Task<List<ModuleRole>> GetModuleRoleDefinitionsAsync(ModuleType module) { return await _context.ModuleRoles.Where(r => r.Module == module).OrderBy(r => r.Priority).ToListAsync(); }
  ...
  public async Task<bool> DeleteRoleDefinitionAsync(string roleName, ModuleType module) {
    ...
    if (role.IsSystemRole) { throw new InvalidOperationException($"Cannot delete system role {roleName}"); }

    // Check if any users are assigned to this role
    var hasAssignments = await _context.UserRoleAssignments.AnyAsync(r => r.RoleName == roleName && r.Module == module && r.IsActive);

    if (hasAssignments) { throw new InvalidOperationException($"Cannot delete role {roleName} because users are still assigned to it"); }
  ...
  /// <summary> Ensures default module roles exist in the database. Should be called during application startup. </summary>
  public async Task EnsureDefaultRolesExistAsync() {
    var existingRoles = await _context.ModuleRoles.Where(r => r.Module == ModuleType.TestingLab).ToListAsync();

    if (existingRoles.Any()) return; // Already initialized

    // Testing Lab Roles
    var testingLabAdminPermissions = new List<ModulePermissionDefinition> {
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Create, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Read, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Edit, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Delete, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Administer, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.CreateSession, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.DeleteSession, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.ManageTesters, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.ViewReports, IsGranted = true },
      new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.ExportData, IsGranted = true },
    };
    ...
    var testingLabTesterPermissions = new List<ModulePermissionDefinition> { new ModulePermissionDefinition { Module = ModuleType.TestingLab, Action = ModuleAction.Read, IsGranted = true } };

    var defaultRoles = new[ ] {
      new ModuleRole { Name = "TestingLabAdmin", Module = ModuleType.TestingLab, Description = "Full administrative access to Testing Lab", Permissions = testingLabAdminPermissions, Priority = 100, IsSystemRole = true },
      new ModuleRole {
        Name = "TestingLabManager", Module = ModuleType.TestingLab, Description = "Can create and edit sessions, manage testers, but cannot delete sessions", Permissions = testingLabManagerPermissions, Priority = 80, IsSystemRole = true,
      },
      new ModuleRole { Name = "TestingLabCoordinator", Module = ModuleType.TestingLab, Description = "Can create sessions and manage testers", Permissions = testingLabCoordinatorPermissions, Priority = 60, IsSystemRole = true },
      new ModuleRole { Name = "TestingLabTester", Module = ModuleType.TestingLab, Description = "Can only participate in testing sessions", Permissions = testingLabTesterPermissions, Priority = 40, IsSystemRole = true },
    };
