...
/// <summary> Implementation of the enhanced DAC Permission Resolver </summary>
public class DacPermissionResolver : IDacPermissionResolver {
  ...
  public async Task<bool> CanGrantPermissionsAsync(Guid grantorUserId, Guid? tenantId, PermissionType[ ] permissions, Guid? resourceId = null, string? contentTypeName = null) {
    ...
    foreach (var permission in permissions) {
      var canGrant = resourceId.HasValue ? await _permissionService.HasResourcePermissionAsync<ResourcePermission<EntityBase>, EntityBase>(grantorUserId, tenantId, resourceId.Value, permission) :
                     contentTypeName != null ? await _permissionService.HasContentTypePermissionAsync(grantorUserId, tenantId, contentTypeName, permission) :
  ...
  public async Task<PermissionHierarchy> GetPermissionHierarchyAsync<TResource>(Guid userId, Guid? tenantId, PermissionType permission, Guid? resourceId = null, string? contentTypeName = null) where TResource : EntityBase {
    ...
    if (resourceId.HasValue) {
      var resourceDefaults = await _permissionService.GetResourcePermissionsAsync<ResourcePermission<TResource>, TResource>(null, tenantId, resourceId.Value);
    ...
    if (resourceId.HasValue) {
      var userResourcePermissions = await _permissionService.GetResourcePermissionsAsync<ResourcePermission<TResource>, TResource>(userId, tenantId, resourceId.Value);
    ...
    var highestPriorityLayer = hierarchy.Layers.Where(l => l.IsGranted.HasValue).OrderByDescending(l => l.Priority).FirstOrDefault();
