...
public class CloudflareController : ControllerBase {
  ...
  public CloudflareController(ILogger<CloudflareController> logger, ICloudflareExternalIpService cloudflareService) {
    _logger = logger;
    _cloudflareService = cloudflareService;
  }

  /// <summary> Gets the current status of the Cloudflare Dynamic DNS service. </summary>
  [HttpGet("status")]
  public ActionResult<CloudflareServiceStatus> GetStatus() {
    try {
      var status = new CloudflareServiceStatus { IsRunning = _cloudflareService.IsRunning, LastKnownIp = _cloudflareService.LastKnownIp, LastUpdate = _cloudflareService.LastUpdate };
  ...
  /// <summary> Manually triggers an IP check and DNS update. </summary>
  [HttpPost("update")]
  ...
  public async Task<ActionResult<string>> GetExternalIp(CancellationToken cancellationToken = default) {
    try {
      ...
      if (string.IsNullOrEmpty(ip)) { return NotFound("Unable to retrieve external IP"); }
