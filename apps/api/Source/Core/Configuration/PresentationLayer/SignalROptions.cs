...
public class SignalROptions
{
    ...
    public void Validate()
    {
        ...
        if (ClientTimeoutInterval <= TimeSpan.Zero)
            throw new InvalidOperationException("Client timeout interval must be greater than zero.");

        if (ClientTimeoutInterval <= KeepAliveInterval)
            throw new InvalidOperationException("Client timeout interval must be greater than keep alive interval.");
