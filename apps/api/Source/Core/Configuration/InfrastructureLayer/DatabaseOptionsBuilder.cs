...
/// <summary> Static builder for database configuration options. </summary>
public static class DatabaseOptionsBuilder {
  /// <summary> Creates a new database options builder. </summary>
  ...
  /// <summary> Creates default database options from configuration. </summary>
  ...
  /// <summary> Creates database options with in-memory database for testing. </summary>
  public static DatabaseOptions CreateInMemory() { return new DatabaseOptions { UseInMemoryDatabase = true, EnableSensitiveDataLogging = true, EnableDetailedErrors = true }; }

  /// <summary> Creates database options with SQLite for development. </summary>
  public static DatabaseOptions CreateSqLite(string connectionString) {
    return new DatabaseOptions { ConnectionString = connectionString, UseInMemoryDatabase = false, EnableSensitiveDataLogging = IsEnvironment("Development"), EnableDetailedErrors = IsEnvironment("Development") };
  }

  /// <summary> Checks if the current environment matches the specified environment name. </summary>
  ...
  public class DatabaseOptionsBuilderInstance {
    ...
    public DatabaseOptionsBuilderInstance UseConnectionStringFromConfiguration(string configurationKey = "DefaultConnection") {
      ...
      if (!string.IsNullOrEmpty(connectionString)) { _options.ConnectionString = connectionString; }
    ...
    private static string GetDatabaseConnectionString(IConfiguration configuration) {
      ...
      if (string.IsNullOrEmpty(connectionString)) {
        throw new InvalidOperationException("Database connection string not found. Please set DB_CONNECTION_STRING environment variable " + "or configure 'ConnectionStrings:DB_CONNECTION_STRING' in appSettings.json");
      }
    ...
    private static bool ShouldUseInMemoryDatabase() {
      ...
      return !string.IsNullOrEmpty(useInMemoryEnv) && useInMemoryEnv.Equals("true", StringComparison.OrdinalIgnoreCase);
    }
  }
}
