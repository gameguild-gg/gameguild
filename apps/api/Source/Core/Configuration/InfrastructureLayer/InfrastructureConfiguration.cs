...
public static class InfrastructureConfiguration
{
    ...
    public class DatabaseOptions
    {
        ...
        public string MigrationsHistoryTable { get; set; } = "migrations_history";

        public string SchemaName { get; set; } = "public";

        public void Validate()
        {
            if (string.IsNullOrEmpty(ConnectionString)) { throw new InvalidOperationException("ConnectionString must be provided for PostgreSQL database."); }
        }
    }
    ...
    public static DatabaseOptions CreateDatabaseOptions(IConfiguration configuration)
    {
        ...
        {
            ConnectionString = GetDatabaseConnectionString(configuration), EnableSensitiveDataLogging = IsEnvironment("Development"), EnableDetailedErrors = IsEnvironment("Development"),
        };
    ...
    private static string GetDatabaseConnectionString(IConfiguration configuration)
    {
        ...
        // Fallback to configuration connection strings (important for development)
        if (string.IsNullOrEmpty(connectionString))
        {
            ...
            if (!string.IsNullOrEmpty(connectionString))
            {
                Console.WriteLine("Using connection string from appsettings.Development.json DefaultConnection");
        ...
        // Additional fallback to configuration (PostgreSQL connection strings only)
        ...
        if (string.IsNullOrEmpty(connectionString))
        {
            ...
            if (isDevelopment)
            {
                ...
                connectionString = $"Host={defaultHost};Port=5432;Database=postgres;Username=postgres;Password=postgres;";
                Console.WriteLine($"Using default development connection string with {defaultHost}/postgres/postgres.");
