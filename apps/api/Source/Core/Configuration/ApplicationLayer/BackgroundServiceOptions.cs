...
public class BackgroundServiceOptions
{
    ...
    public void Validate()
    {
        if (MaxConcurrentTasks <= 0)
            throw new InvalidOperationException("Max concurrent tasks must be greater than zero.");

        if (TaskTimeoutSeconds <= 0)
            throw new InvalidOperationException("Task timeout seconds must be greater than zero.");
    }
}
