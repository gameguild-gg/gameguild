...
public partial class SlugCaseTransformer : CachedCaseTransformer
{
    ...
    {
        var config = new SlugHelperConfiguration { ForceLowerCase = true, CollapseDashes = true, TrimWhitespace = true, StringReplacements = new Dictionary<string, string> { { "&", "and" }, { "+", "plus" } }, };

        SlugHelper = new SlugHelper(config);
    }

    protected override string CacheKeyPrefix { get => "slug"; }
    ...
    public string GenerateUnique(string text, IEnumerable<string> existingSlugs, CaseTransformOptions? options = null)
    {
        ...
        do
        {
            ...
            {
                var truncatedBase = baseSlug.Length > availableLength ? baseSlug[..availableLength].TrimEnd('-') : baseSlug;
