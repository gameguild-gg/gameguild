...
public class PermissionTelemetryService : IPermissionTelemetryService
{
    ...
    public PermissionTelemetryService(ILogger<PermissionTelemetryService> logger) { _logger = logger; }

    public async Task<bool> RecordPermissionCheckAsync(string permission, string? resourceType, Guid? resourceId, Func<Task<bool>> permissionCheck)
    {
        ...
        try
        {
            _logger.LogDebug("Checking permission {Permission} for resource {ResourceType}:{ResourceId}", permission, resourceType, resourceId);
        ...
        finally
        {
            ...
            // Record metrics
            OpenTelemetryConfiguration.PermissionCheckCounter.Add(
                1,
            ...
            OpenTelemetryConfiguration.PermissionCheckDuration.Record(
                durationMs,
                new KeyValuePair<string, object?>("permission.name", permission),
                new KeyValuePair<string, object?>("permission.resource_type", resourceType ?? "global"),
                new KeyValuePair<string, object?>("success", success)
            ...
            if (success) { _logger.LogDebug("Permission check for {Permission} completed in {Duration}ms", permission, durationMs); }
            else { _logger.LogWarning("Permission check for {Permission} failed in {Duration}ms: {Error}", permission, durationMs, exception?.Message ?? "Check failed"); }
        }
    }
}
