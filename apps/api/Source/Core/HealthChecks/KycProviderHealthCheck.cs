...
public class KycProviderHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        var data = new Dictionary<string, object>
        {
            ...
            ["providers"] = results.Select(r => new { name = r.Provider, healthy = r.IsHealthy, error = r.Error, response_time_ms = r.ResponseTime }).ToArray()
        };

        if (totalCount == 0) { return HealthCheckResult.Healthy("No KYC providers configured", data); }
        else if (healthyCount == 0) { return HealthCheckResult.Unhealthy("No KYC providers are healthy", null, data); }
        else if (healthyCount < totalCount) { return HealthCheckResult.Degraded($"Only {healthyCount}/{totalCount} KYC providers are healthy", null, data); }
        else { return HealthCheckResult.Healthy("All KYC providers are healthy", data); }
    }
    ...
    private async Task CheckKycProviderHealth(string providerName, string healthEndpoint, List<(string Provider, bool IsHealthy, string? Error, long ResponseTime)> results, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode) { results.Add((providerName, true, null, stopwatch.ElapsedMilliseconds)); }
            else { results.Add((providerName, false, $"HTTP {response.StatusCode}", stopwatch.ElapsedMilliseconds)); }
        }
        catch (Exception ex)
        {
