...
public class PaymentProviderHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (healthyCount == 0) { return HealthCheckResult.Unhealthy("No payment providers are healthy", null, data); }
        else if (healthyCount < totalCount) { return HealthCheckResult.Degraded($"Only {healthyCount}/{totalCount} payment providers are healthy", null, data); }
        else { return HealthCheckResult.Healthy("All payment providers are healthy", data); }
    }
    ...
    private async Task CheckStripeHealth(List<(string Provider, bool IsHealthy, string? Error, long ResponseTime)> results, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode) { results.Add(("Stripe", true, null, stopwatch.ElapsedMilliseconds)); }
            else { results.Add(("Stripe", false, $"HTTP {response.StatusCode}", stopwatch.ElapsedMilliseconds)); }
        }
        catch (Exception ex)
        {
