...
public record Error(string Code, string Message, ErrorType Type, Dictionary<string, object>? Metadata = null) {
    ...
    public static Error RequiredField(string propertyName) => ValidationFailure(propertyName, $"{propertyName} is required");

    public static Error InvalidFormat(string propertyName, object? attemptedValue = null) => ValidationFailure(propertyName, $"{propertyName} has invalid format", attemptedValue);
