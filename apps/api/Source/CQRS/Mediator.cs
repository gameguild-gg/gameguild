  ...
  public async Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default) {
    ...
    var metadata = _handlerCache.GetOrAdd(
      ...
      static ht => {
        ...
        return new HandlerMetadata { HandlerType = ht, HandleMethod = method, CompiledInvoker = CreateCompiledInvoker<TResponse>(method) };
      }
    );
  ...
  public async Task Send<TRequest>(TRequest request, CancellationToken cancellationToken = default) where TRequest : IRequest {
    ...
    var metadata = _handlerCache.GetOrAdd(
      ...
      static ht => {
        ...
        return new HandlerMetadata { HandlerType = ht, HandleMethod = method, CompiledInvoker = CreateCompiledInvokerUnit(method) };
      }
    );
    ...
    if (result is Task<Unit> task) { await task.ConfigureAwait(false); }
  ...
  public async Task Publish<TNotification>(TNotification notification, CancellationToken cancellationToken = default) where TNotification : INotification {
    ...
    if (handlers != null) {
      ...
      if (handlerArray.Length > 0) {
        ...
        for (var i = 0; i < handlerArray.Length; i++) { executors[i] = new NotificationHandlerExecutor<TNotification>(handlerArray[i]); }
