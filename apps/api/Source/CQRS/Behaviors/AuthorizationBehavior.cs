...
/// <summary> Interface for requests that require authorization </summary>
public interface IAuthorizedRequest {
  /// <summary> Required roles for this request </summary>
  string[ ]? RequiredRoles { get; }

  /// <summary> Required permissions for this request </summary>
  string[ ]? RequiredPermissions { get; }

  /// <summary> Custom authorization logic for this request </summary>
...
/// <summary> Pipeline behavior for authorizing requests using Result<T> pattern </summary>
/// <typeparam name="TRequest"> Request type </typeparam>
/// <typeparam name="TResponse"> Response type </typeparam>
public class AuthorizationBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : IBaseRequest {
  private readonly IHttpContextAccessor _httpContextAccessor;

  private readonly ILogger<AuthorizationBehavior<TRequest, TResponse>> _logger;

  /// <summary> Initializes a new instance of the AuthorizationBehavior class </summary>
  ...
  /// <summary> Handles the request pipeline with authorization </summary>
  ...
  public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegateBase<TResponse> next, CancellationToken cancellationToken) {
    ...
    try {
      ...
      if (authorizedRequest.RequiredRoles?.Length > 0) {
        ...
        if (!isAuthorized) {
          _logger.LogWarning("User {UserId} failed role authorization for {RequestType}. Required roles: {RequiredRoles}", user?.Identity?.Name, typeof(TRequest).Name, string.Join(", ", authorizedRequest.RequiredRoles));
        }
      }

      // Check permission-based authorization
      if (!isAuthorized && authorizedRequest.RequiredPermissions?.Length > 0) {
        isAuthorized = authorizedRequest.RequiredPermissions.Any(permission => user?.HasClaim("permission", permission) == true);

        if (!isAuthorized) {
          _logger.LogWarning(
            "User {UserId} failed permission authorization for {RequestType}. Required permissions: {RequiredPermissions}",
            user?.Identity?.Name,
      ...
      if (!isAuthorized) {
        ...
        if (!isAuthorized) { _logger.LogWarning("User {UserId} failed custom authorization for {RequestType}", user?.Identity?.Name, typeof(TRequest).Name); }
      }
    }
    catch (Exception ex) {
    ...
    if (!isAuthorized) {
      ...
      if (typeof(TResponse) == typeof(Result)) { return (TResponse) (object) Result.Failure(error); }

      // Fallback to throwing exception for non-Result responses (backward compatibility)
      throw user?.Identity?.IsAuthenticated == true ? new UnauthorizedAccessException("You do not have permission to perform this action") : new UnauthorizedAccessException("You must be authenticated to perform this action");
    }
