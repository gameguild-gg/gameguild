...
/// <summary>
/// Interface for requests that require a database transaction
/// </summary>
...
/// <summary>
/// Pipeline behavior for wrapping commands in database transactions
/// </summary>
    ...
    where TRequest : IBaseRequest
{
    ...
    /// <summary>
    /// Handles the request pipeline with transaction management
    /// </summary>
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegateBase<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (request is not ICommand && request is not ITransactionalRequest)
        {
        ...
        _logger.LogDebug("Starting transaction for {RequestType} with isolation level {IsolationLevel}",
            typeof(TRequest).Name, isolationLevel);
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger.LogError(rollbackEx, "Error rolling back transaction for {RequestType}", typeof(TRequest).Name);
                }
    ...
    /// <summary>
    /// Determines if a response indicates failure (for Result types)
    /// </summary>
    ...
    private static bool IsFailureResponse(TResponse response)
    {
        ...
        {
            IResult result => result.IsFailure,
            _ => false
        };
    }
}
