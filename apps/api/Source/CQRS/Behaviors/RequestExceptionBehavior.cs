    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegateBase<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        catch (Exception exception)
        {
            ...
            var handlerType = HandlerTypeCache.GetOrAdd(
                exceptionType,
                static et => typeof(IRequestExceptionHandler<,,>).MakeGenericType(typeof(TRequest), typeof(TResponse), et)
            );
            ...
            var actionType = ActionTypeCache.GetOrAdd(
                exceptionType,
                static et => typeof(IRequestExceptionAction<,>).MakeGenericType(typeof(TRequest), et)
            );

            var enumerableActionType = EnumerableActionTypeCache.GetOrAdd(
                actionType,
                static at => typeof(IEnumerable<>).MakeGenericType(at)
            );
            ...
            {
                // Cached method lookup - O(1)
                var executeMethod = ExecuteMethodCache.GetOrAdd(
                    actionType,
                    static at => at.GetMethod("Execute")
                );
