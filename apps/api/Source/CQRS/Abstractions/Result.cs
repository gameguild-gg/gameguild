...
/// <summary> Represents the result of an operation that can either succeed or fail </summary>
public class Result : IResult {
  protected Result(bool isSuccess, Error? error, IEnumerable<ValidationError>? validationErrors = null) {
    ...
    if (!isSuccess && error == Error.None) throw new InvalidOperationException("Failed result must have an error");
  ...
  /// <summary> Creates a failed result with validation errors </summary>
  /// <param name="validationErrors"> The validation errors that occurred </param>
  /// <returns> A failed result </returns>
  public static Result ValidationFailure(IEnumerable<ValidationError> validationErrors) { return new Result(false, Error.Validation("ValidationFailed", "One or more validation errors occurred"), validationErrors); }

  /// <summary> Creates a failed result with validation errors </summary>
  /// <param name="validationErrors"> The validation errors that occurred </param>
  /// <returns> A failed result </returns>
  public static Result ValidationFailure(params ValidationError[ ] validationErrors) { return ValidationFailure(validationErrors.AsEnumerable()); }
  ...
  public static Result<TValue> ValidationFailure<TValue>(IEnumerable<ValidationError> validationErrors) {
    return new Result<TValue>(default(TValue?), false, Error.Validation("ValidationFailed", "One or more validation errors occurred"), validationErrors);
  }
  ...
  /// <param name="validationErrors"> The validation errors that occurred </param>
  /// <returns> A failed result </returns>
  public static Result<TValue> ValidationFailure<TValue>(params ValidationError[ ] validationErrors) { return ValidationFailure<TValue>(validationErrors.AsEnumerable()); }
}

/// <summary> Represents the result of an operation that can either succeed with a value or fail </summary>
/// <typeparam name="TValue"> The type of the value </typeparam>
public class Result<TValue> : Result, IResult<TValue> {
  private readonly TValue? _value;

  internal Result(TValue? value, bool isSuccess, Error? error, IEnumerable<ValidationError>? validationErrors = null) : base(isSuccess, error, validationErrors) { _value = value; }

  /// <inheritdoc />
  public TValue? Value { get => IsSuccess ? _value : default(TValue?); }
  ...
  public static implicit operator Result<TValue>(Error error) { return Failure<TValue>(error); }

  /// <summary> Maps the value of a successful result to a new type </summary>
  ...
  /// <summary> Binds the result to another operation that returns a result </summary>
  /// <typeparam name="TNew"> The new type </typeparam>
  ...
  /// <summary> Executes an action if the result is failed </summary>
  /// <param name="action"> The action to execute </param>
  /// <returns> The current result </returns>
  ...
  public TResult Match<TResult>(Func<TValue, TResult> onSuccess, Func<Error, TResult> onFailure) { return IsSuccess ? onSuccess(Value!) : onFailure(Error!); }
}
