...
/// <summary> Publisher that executes handlers without waiting for completion (fire and forget) with optimized O(n) enumeration </summary>
public class NoWaitPublisher : INotificationPublisher {
  /// <summary> Publishes a notification to all handlers without waiting </summary>
  ...
  public Task Publish(IEnumerable<NotificationHandlerExecutorBase> handlerExecutors, INotification notification, CancellationToken cancellationToken) {
    ...
    foreach (var handler in handlerExecutors) { Task.Run(() => handler.ExecuteHandler(notification, cancellationToken), cancellationToken); }
