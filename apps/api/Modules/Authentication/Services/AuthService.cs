...
) : IAuthService
{
    public async Task<SignInResponse> LocalSignInAsync(LocalSignInRequest request)
    {
        ...
        try
        {
            ...
            await mediator.Publish(new UserSignedInEvent(user.Id, user.Email, "local", ipAddress, userAgent, DateTime.UtcNow), CancellationToken.None);
    ...
    public async Task<SignInResponse> LocalSignUpAsync(LocalSignUpRequest request)
    {
        ...
        try
        {
            ...
            if (request.TenantId.HasValue)
            {
                ...
                catch (Exception ex) { logger.LogWarning(ex, "Failed to add user {UserId} to tenant {TenantId}", user.Id, request.TenantId.Value); }
            }
            ...
            await mediator.Publish(new UserSignedUpEvent(user.Id, user.Email, "local", DateTime.UtcNow), CancellationToken.None);
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                ...
                if (tenantId.HasValue)
                {
                    ...
                    if (activeTenants.Any(t => t.TenantId.HasValue && t.TenantId.Value == tenantId.Value)) { tenantClaims = await tenantAuthService.GetTenantClaimsAsync(user, tenantId.Value); }
                    else
                    {
    ...
    public async Task<SignInResponse> GitHubSignInAsync(OAuthSignInRequest request)
    {
        ...
        await mediator.Publish(
            new UserSignedInEvent(user.Id, user.Email, "github", GetClientIpAddress(httpContextAccessor.HttpContext), httpContextAccessor.HttpContext?.Request.Headers.UserAgent.ToString(), DateTime.UtcNow),
    ...
    public async Task<SignInResponse> GoogleSignInAsync(OAuthSignInRequest request)
    {
        ...
        // Publish sign-in event
        await mediator.Publish(
            new UserSignedInEvent(user.Id, user.Email, "google", GetClientIpAddress(httpContextAccessor.HttpContext), httpContextAccessor.HttpContext?.Request.Headers.UserAgent.ToString(), DateTime.UtcNow),
            CancellationToken.None
        );
    ...
    public async Task<EmailOperationResponse> ForgotPasswordAsync(PasswordResetRequestDto request) { return await emailVerificationService.SendPasswordResetAsync(request.Email); }
