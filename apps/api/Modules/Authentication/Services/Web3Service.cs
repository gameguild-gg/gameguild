...
{
    private readonly Dictionary<string, Web3ChallengeResponse> _challenges = [];

    public async Task<Web3ChallengeResponse> GenerateChallengeAsync(Web3ChallengeRequest request)
    {
        ...
        var challengeResponse = new Web3ChallengeResponse { Challenge = challenge, Nonce = nonce, ExpiresAt = expiresAt };

        // TODO: Store challenge temporarily (in production, use Redis or database)
    ...
    public async Task<bool> VerifySignatureAsync(Web3AuthenticationVerificationRequest request)
    {
        // Check if challenge exists and is valid
        if (!_challenges.TryGetValue(request.Nonce, out Web3ChallengeResponse? challenge))
        {
        ...
        var isValidSignature = await VerifyEthereumSignature(challenge.Challenge, request.Signature, request.WalletAddress);
    ...
    public async Task<User> FindOrCreateWeb3UserAsync(string walletAddress, string chainId = "1")
    {
        ...
        var credential = await context.Credentials.Include(c => c.User).FirstOrDefaultAsync(c => c.Type == "web3_wallet" && c.Value == walletAddress.ToLower());
    ...
    private Task<bool> VerifyEthereumSignature(string message, string signature, string walletAddress)
    {
        try
        {
            ...
            if (!signature.StartsWith("0x") || !IsValidEthereumAddress(walletAddress)) return Task.FromResult(false);
