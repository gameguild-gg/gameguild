...
public class AuthorizationBehavior<TRequest, TResponse>(IHttpContextAccessor httpContextAccessor, ILogger<AuthorizationBehavior<TRequest, TResponse>> logger) : IPipelineBehavior<TRequest, TResponse>
    where TRequest : IBaseRequest
{
    ...
    private static TResponse CreateErrorResponse<T>(Error error)
    {
        ...
        if (typeof(T).IsGenericType && typeof(T).GetGenericTypeDefinition() == typeof(Result<>))
        {
            ...
            if (failureMethod != null) { return (TResponse) failureMethod.Invoke(null, [error])!; }
        }
