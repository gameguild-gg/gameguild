...
import { recordLessonEvent } from '@/lib/lesson-interaction-actions';
...
function renderLexicalNode(node: LexicalNode, key: string): ReactNode {
    ...
    switch (node.type) {
        ...
        case 'heading': {
            ...
            return createElement(tag, { key, className: 'mt-7 font-semibold text-slate-100' }, children);
        }
        case 'paragraph': return <p key={key} className="leading-7 text-slate-300">{children}</p>;
        case 'quote': return <blockquote key={key} className="border-l-2 border-violet-400 pl-4 italic text-slate-300">{children}</blockquote>;
        ...
        case 'link': return <a key={key} href={node.url ?? '#'} className="text-violet-300 underline" rel="noreferrer">{children}</a>;
...
function LexicalRenderer({ content }: { content: unknown }) {
    ...
    if (!root) return <p className="text-sm text-slate-400">This Lexical lesson has no published content.</p>;
...
function RevealRenderer({ content }: { content: unknown }) {
    ...
    if (!slides.length) return <p className="text-sm text-slate-400">This presentation has no published slides.</p>;
    return <section aria-label="Slide presentation" className="bg-[#111216] p-6 sm:p-10">
        ...
        <footer className="mt-6 flex items-center justify-between border-t border-white/10 pt-4">
            ...
            <span className="text-sm text-slate-400">{index + 1} / {slides.length}</span>
...
function VideoRenderer({ courseId, enrollmentId, itemId, content }: { courseId: string; enrollmentId?: string; itemId: string; content: unknown }) {
    ...
    if (!src) return <p className="text-sm text-slate-400">This video lesson has no published media.</p>;
...
export function LearnerLessonRenderer({ courseId, enrollmentId, itemId, format, content }: { courseId: string; enrollmentId?: string; itemId: string; format?: LearningCoursesLessonContentFormat; content: unknown }) {
    switch (format ?? 'Markdown') {
        ...
        default: {
            ...
            return markdown ? <MarkdownRenderer content={markdown} /> : <p className="text-sm text-slate-400">This lesson has no published content.</p>;
        }
    }
}