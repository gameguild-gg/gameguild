...
public class TenantDomainService : ITenantDomainService
{
    ...
    public TenantDomainService(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<TenantDomain> CreateDomainAsync(TenantDomain domain)
    {
        ...
        {
            await UnsetMainDomainsForTenantAsync(domain.TenantId);
        }
        ...
        return await _context.TenantDomains
            ...
            .FirstOrDefaultAsync(d => d.DeletedAt == null && 
                ((d.Subdomain == null && d.TopLevelDomain == normalizedDomain) ||
                 (d.Subdomain != null && $"{d.Subdomain}.{d.TopLevelDomain}" == normalizedDomain)));
    }
    ...
    public async Task<TenantDomain> UpdateDomainAsync(TenantDomain domain)
    {
        ...
        {
            throw new InvalidOperationException($"Domain with ID {domain.Id} not found.");
        }

        // If changing to main domain, unset other main domains for the tenant
        if (domain.IsMainDomain && !existingDomain.IsMainDomain)
        {
            await UnsetMainDomainsForTenantAsync(domain.TenantId);
        }
    ...
    public async Task<bool> SetMainDomainAsync(Guid tenantId, Guid domainId)
    {
        var domain = await _context.TenantDomains
            .FirstOrDefaultAsync(d => d.Id == domainId && d.TenantId == tenantId && d.DeletedAt == null);
    ...
    public async Task<TenantUserGroup> UpdateUserGroupAsync(TenantUserGroup userGroup)
    {
        ...
        {
            throw new InvalidOperationException($"User group with ID {userGroup.Id} not found.");
        }
    ...
    public async Task<TenantUserGroupMembership> AddUserToGroupAsync(Guid userId, Guid userGroupId, bool isAutoAssigned = false)
    {
        ...
        var existingMembership = await _context.TenantUserGroupMemberships
            .FirstOrDefaultAsync(m => m.UserId == userId && m.UserGroupId == userGroupId);
        ...
        var membership = new TenantUserGroupMembership(userId, userGroupId, isAutoAssigned)
        {
            Id = Guid.NewGuid(),
        ...
        return await _context.TenantUserGroupMemberships
            .Include(m => m.User)
    ...
    public async Task<bool> IsUserInGroupAsync(Guid userId, Guid userGroupId)
    {
        return await _context.TenantUserGroupMemberships
            .AnyAsync(m => m.UserId == userId && m.UserGroupId == userGroupId);
    }
    ...
    public async Task<IEnumerable<TenantUserGroupMembership>> AutoAssignUserToGroupsAsync(string userEmail)
    {
        ...
        if (user == null)
        {
            return Enumerable.Empty<TenantUserGroupMembership>();
        }
    ...
    public async Task<IEnumerable<TenantUserGroupMembership>> AutoAssignUserToGroupsAsync(Guid userId)
    {
        ...
        if (matchingDomain?.UserGroupId == null)
        {
            return Enumerable.Empty<TenantUserGroupMembership>();
        }
        ...
        return await _context.TenantDomains
            ...
            .FirstOrDefaultAsync(d => d.DeletedAt == null && 
                ...
                ((d.Subdomain == null && d.TopLevelDomain == emailDomain) ||
                 (d.Subdomain != null && $"{d.Subdomain}.{d.TopLevelDomain}" == emailDomain)));
    }
    ...
    public async Task<int> AutoAssignTenantUsersAsync(Guid tenantId)
    {
        ...
        var users = await _context.Users
            .Where(u => u.DeletedAt == null && 
                       domainStrings.Any(domain => u.Email.ToLower().EndsWith("@" + domain)))
            .ToListAsync();
