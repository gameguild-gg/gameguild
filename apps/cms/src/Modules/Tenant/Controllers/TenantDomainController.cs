...
public class TenantDomainController : ControllerBase
{
    ...
    public TenantDomainController(ITenantDomainService tenantDomainService)
    {
    ...
    public async Task<ActionResult<TenantDomain>> CreateDomain([FromBody] CreateTenantDomainDto request)
    {
        try
        {
            ...
            if (!isValid)
            {
                return BadRequest("Domain ownership could not be validated");
            }
        ...
        {
            return BadRequest($"Error creating domain: {ex.Message}");
        }
    }
    ...
    public async Task<ActionResult<TenantDomain>> UpdateDomain(Guid id, [FromBody] UpdateTenantDomainDto request)
    {
        ...
        {
            return BadRequest($"Error updating domain: {ex.Message}");
        }
    }
    ...
    // GET: api/tenant-domains/user-groups
    [HttpGet("user-groups")]
    [RequireResourcePermission<TenantUserGroup>(PermissionType.Read)]
    public async Task<ActionResult<IEnumerable<TenantUserGroup>>> GetUserGroups([FromQuery] Guid tenantId, [FromQuery] bool rootOnly = false)
    {
    ...
    [RequireResourcePermission<TenantUserGroup>(PermissionType.Read)]
    public async Task<ActionResult<TenantUserGroup>> GetUserGroup(Guid id)
    {
    ...
    public async Task<ActionResult<TenantUserGroup>> CreateUserGroup([FromBody] CreateTenantUserGroupDto request)
    {
        try
        {
            ...
            return CreatedAtAction(nameof(GetUserGroup), new { id = createdUserGroup.Id }, createdUserGroup);
        }
        catch (Exception ex)
        {
            return BadRequest($"Error creating user group: {ex.Message}");
        }
    }
    ...
    public async Task<ActionResult<TenantUserGroup>> UpdateUserGroup(Guid id, [FromBody] UpdateTenantUserGroupDto request)
    {
        try
        {
        ...
        {
            return BadRequest($"Error updating user group: {ex.Message}");
        }
    }
    ...
    public async Task<ActionResult<TenantUserGroupMembership>> AddUserToGroup([FromBody] AddUserToGroupDto request)
    {
        try
        {
            var membership = await _tenantDomainService.AddUserToGroupAsync(
                request.UserId, 
                request.UserGroupId, 
        ...
        {
            return BadRequest($"Error adding user to group: {ex.Message}");
        }
    }
    ...
    [RequireResourcePermission<TenantUserGroupMembership>(PermissionType.Read)]
    public async Task<ActionResult<IEnumerable<TenantUserGroup>>> GetUserGroups(Guid userId)
    {
    ...
    public async Task<ActionResult<object>> AutoAssignUsers([FromBody] AutoAssignUsersDto request)
    {
        ...
        {
            return BadRequest($"Error during auto-assignment: {ex.Message}");
        }
    }
    ...
    public async Task<ActionResult<TenantDomain>> FindMatchingDomain([FromQuery] string email)
    {
        ...
        if (domain == null)
        {
            return NotFound("No matching domain found for the provided email");
        }
