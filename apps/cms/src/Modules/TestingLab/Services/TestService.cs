...
public class TestService : ITestService
{
    ...
    public async Task<TestingParticipant> AddParticipantAsync(Guid testingRequestId, Guid userId)
    {
        ...
        return await _context.TestingParticipants
    ...
    public async Task<SessionWaitlist> AddToWaitlistAsync(Guid sessionId, Guid userId, RegistrationType registrationType, string? notes = null)
    {
        ...
        var existingWaitlist = await _context.SessionWaitlists
        ...
        var maxPosition = await _context.SessionWaitlists
        ...
        return await _context.SessionWaitlists
    ...
    {
        var waitlistEntry = await _context.SessionWaitlists
            .FirstOrDefaultAsync(sw => sw.SessionId == sessionId && sw.UserId == userId);
    ...
    public async Task<TestingFeedback> AddFeedbackAsync(Guid testingRequestId, Guid userId, Guid feedbackFormId, string feedbackData, TestingContext context, Guid? sessionId = null, string? additionalNotes = null)
    {
        ...
        return await _context.TestingFeedback
    ...
    {
        var participantCount = await _context.TestingParticipants
            .CountAsync(tp => tp.TestingRequestId == testingRequestId);

        var sessionCount = await _context.TestingSessions
            .CountAsync(ts => ts.TestingRequestId == testingRequestId && ts.DeletedAt == null);

        var feedbackCount = await _context.TestingFeedback
            .CountAsync(tf => tf.TestingRequestId == testingRequestId);

        var completedSessionCount = await _context.TestingSessions
            .CountAsync(ts => ts.TestingRequestId == testingRequestId && 
    ...
    public async Task<object> GetTestingSessionStatisticsAsync(Guid testingSessionId)
    {
        ...
        var waitlistCount = await _context.SessionWaitlists
            .CountAsync(sw => sw.SessionId == testingSessionId);

        var feedbackCount = await _context.TestingFeedback
            .CountAsync(tf => tf.SessionId == testingSessionId);
    ...
    {
        var participationCount = await _context.TestingParticipants
            .CountAsync(tp => tp.UserId == userId);

        var sessionRegistrationCount = await _context.SessionRegistrations
            .CountAsync(sr => sr.UserId == userId);

        var feedbackCount = await _context.TestingFeedback
            .CountAsync(tf => tf.UserId == userId);

        var managedSessionCount = await _context.TestingSessions
            .CountAsync(ts => ts.ManagerUserId == userId && ts.DeletedAt == null);

        var createdRequestCount = await _context.TestingRequests
            .CountAsync(tr => tr.CreatedById == userId && tr.DeletedAt == null);
