...
public class TestingControllerTests : IClassFixture<TestWebApplicationFactory>, IDisposable
{
    ...
    public async Task GetTestingRequests_ShouldReturnEmptyArray_WhenNoRequests()
    {
        ...
        var requests = JsonSerializer.Deserialize<TestingRequest[]>(content, new JsonSerializerOptions 
        { 
    ...
    public async Task GetTestingRequest_ShouldReturnRequest_WhenExists()
    {
        ...
        var content = await response.Content.ReadAsStringAsync();
        var request = JsonSerializer.Deserialize<TestingRequest>(content, new JsonSerializerOptions 
        { 
            PropertyNameCaseInsensitive = true 
        });
    ...
    public async Task GetTestingSessions_ShouldReturnSessions_WhenSessionsExist()
    {
        ...
        var content = await response.Content.ReadAsStringAsync();
        var sessions = JsonSerializer.Deserialize<TestingSession[]>(content, new JsonSerializerOptions 
        { 
            PropertyNameCaseInsensitive = true 
        });
    ...
    public async Task GetTestingSession_ShouldReturnSession_WhenExists()
    {
        ...
        foreach (var dbSession in sessionsInDb)
        {
            _output.WriteLine($"Session ID: {dbSession.Id}, Name: {dbSession.SessionName}, DeletedAt: {dbSession.DeletedAt}");
        }
        ...
        var content = await response.Content.ReadAsStringAsync();
        var session = JsonSerializer.Deserialize<TestingSession>(content, new JsonSerializerOptions 
        { 
            PropertyNameCaseInsensitive = true 
        });
