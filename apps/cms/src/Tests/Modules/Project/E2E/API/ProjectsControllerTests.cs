...
public class ProjectsControllerTests : IClassFixture<TestWebApplicationFactory>, IDisposable
{
    ...
    public async Task GetProjectBySlug_ShouldReturnProject_WhenExists()
    {
        ...
        {
            _output.WriteLine($"Saved project: ID={savedProject.Id}, Title={savedProject.Title}, Slug={savedProject.Slug}");
        }
    ...
    }    [Fact]    public async Task GetPublicProjects_ShouldReturnOnlyPublicPublishedProjects()
    {
        ...
        {
            Console.WriteLine($"DEBUG TEST: Project in DB: {p.Title}, Status: {p.Status}, Visibility: {p.Visibility}, TenantId: {(p.Tenant?.Id.ToString() ?? "null")}, IsDeleted: {p.DeletedAt != null}");
        }
