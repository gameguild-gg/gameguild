...
namespace GameGuild.Tests.Modules.Auth.Services
{
    public class AuthServiceTests : IDisposable
    {
        ...
        public async Task LocalSignUpAsync_DuplicateEmail_ThrowsInvalidOperationException()
        {
            ...
            {
                Email = "test@example.com", Name = "Existing User"
            };
            ...
            var request = new LocalSignUpRequestDto
            {
                Email = "test@example.com", Password = "P455W0RD", Username = "testuser"
            };
        ...
        public async Task LocalSignInAsync_ValidCredentials_ReturnsSignInResponse()
        {
            ...
            string hashedPassword = Convert.ToBase64String(
                System.Security.Cryptography.SHA256.HashData(
                    System.Text.Encoding.UTF8.GetBytes("P455W0RD")
                )
            );
            var credential = new Credential
            {
                UserId = user.Id, Type = "password", Value = hashedPassword, IsActive = true
            };
            ...
            {
                Email = "test@example.com", Password = "P455W0RD", TenantId = tenantId
            };
        ...
        public async Task LocalSignInAsync_InvalidCredentials_ThrowsUnauthorizedAccessException()
        {
            ...
            var request = new LocalSignInRequestDto
            {
                Email = "nonexistent@example.com", Password = "P455W0RD"
            };
        ...
        public async Task VerifyWeb3SignatureAsync_ValidSignature_ReturnsSignInResponse()
        {
            ...
            var user = new User
            {
                Email = "web3user@example.com", Name = "Web3 User"
            };
        ...
        public async Task Web3SignIn_WithInvalidTenant_FallsBackToDefaultTenant()
        {
            ...
            var enhancedResponse = new SignInResponseDto
            {
                ...
                AvailableTenants = new List<TenantInfoDto>
                {
                    new TenantInfoDto
                    {
                        Id = defaultTenantId, Name = "Default Tenant", IsActive = true
                    }
                }
            };
        ...
        public async Task RefreshTokenAsync_WithTenantId_IncludesTenantClaims()
        {
            ...
            var user = new User
            {
                Id = Guid.NewGuid(), Email = "test@example.com", Name = "Test User"
            };
            _context.Users.Add(user);
            ...
            {
                RefreshToken = "valid-refresh-token", TenantId = tenantId
            };
            ...
            _mockJwtService.Setup(x => x.GenerateAccessToken(It.IsAny<UserDto>(), It.IsAny<string[]>(), It.IsAny<IEnumerable<Claim>>()))
            ...
            _mockTenantAuthService.Setup(x => x.GetUserTenantsAsync(It.IsAny<User>()))
                .ReturnsAsync(tenantPermissions);

            _mockTenantAuthService.Setup(x => x.GetTenantClaimsAsync(It.IsAny<User>(), tenantId))
            ...
            _mockJwtService.Verify(
                x => x.GenerateAccessToken(
                    It.IsAny<UserDto>(),
                    It.IsAny<string[]>(),
                    It.Is<IEnumerable<Claim>>(c => c.Any(claim => claim.Type == "tenant_id" && claim.Value == tenantId.ToString()))
                ),
                Times.Once
            );
        }
