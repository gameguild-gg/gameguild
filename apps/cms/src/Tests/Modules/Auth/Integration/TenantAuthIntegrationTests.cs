...
namespace GameGuild.Tests.Modules.Auth.Integration
{
    public class TenantAuthIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        private async Task SetupTestTenantData(ApplicationDbContext db)
        {
            ...
            var credential = new Credential
            {
                ...
                Value = Convert.ToBase64String(
                    System.Security.Cryptography.SHA256.Create().ComputeHash(Encoding.UTF8.GetBytes("P455W0RD"))
                ),
                IsActive = true,
                CreatedAt = DateTime.UtcNow,
                UpdatedAt = DateTime.UtcNow
        ...
        public async Task Login_WithTenantId_ReturnsTenantInformationInResponse()
        {
            ...
            {
                Email = "tenant@example.com", Password = "P455W0RD", TenantId = Guid.Parse("22222222-2222-2222-2222-222222222222")
            };
        ...
        public async Task Login_WithoutTenantId_ReturnsFirstTenant()
        {
            ...
            var request = new LocalSignInRequestDto
            {
                Email = "tenant@example.com", Password = "P455W0RD"
            };
