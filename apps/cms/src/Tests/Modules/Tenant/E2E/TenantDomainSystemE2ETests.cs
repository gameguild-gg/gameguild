...
public class TenantDomainSystemE2ETests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task CorporateScenario_MultiDomainWithDepartments_WorksEndToEnd()
    {
        ...
        var mainDomain = await db.TenantDomains
            .FirstAsync(d => d.TenantId == tenantId && d.TopLevelDomain == "techcorp.com" && d.Subdomain == null);
        var partnersDomain = await db.TenantDomains
            .FirstAsync(d => d.TenantId == tenantId && d.Subdomain == "partners");
